        ...
        loopi(2)
        {
            ...
            case CTFF_DROPPED:
            {
                ...
                if (f.flagent->spawned)
                    rendermodel(path, ANIM_FLAG | ANIM_LOOP, 0, 0, vec(f.pos.x, f.pos.y, f.state == CTFF_INBASE ? (float)S(int(f.pos.x), int(f.pos.y))->floor : f.pos.z), 0, float(e.attr1) / ENTSCALE10, 0, 120.0f);
                break;
            }
            case CTFF_IDLE:
                break;
        ...
        if (d == player1)
        {
            ...
            */
            if (identexists("onPickup"))
            {
                ...
                if (tmp)
                    exechook(HOOK_SP, "onPickup", "%d %d", e.type - 3, m_lss && e.type == I_GRENADE ? 2 : tmp->add);
            }
        }
        else
...
void checkitems(playerent *d)
{
    ...
    if (m_flags)
        loopi(2)
        {
            flaginfo &f = flaginfos[i];
            entity &e = *f.flagent;
            if (!e.spawned || !f.ack || (f.state == CTFF_INBASE && !clentstats.flags[i]))
                continue;
            if (OUTBORD(f.pos.x, f.pos.y))
                continue;
...
void entstats_(void)
{
    ...
    loopv(ents)
    {
        ...
        {
            if (!mapconfigdata.mapsoundlines.inrange(e.attr1))
                xsounds++;
        }
    }
    string txt = "", clips = "";
    ...
    {
    case MAPMODEL:
        conoutf(" %d %s, %d clipped, %s%d unconfigured", es.entcnt[i], entnames[i], clipents, xmodels ? "\f3" : "", xmodels);
        break;
    case SOUND:
        conoutf(" %d %s, %s%d unconfigured", es.entcnt[i], entnames[i], xsounds ? "\f3" : "", xsounds);
        break;
    case PLAYERSTART:
        conoutf(" %d %s, %d CLA, %d RVSF, %d FFA%c \f3unknown %d", es.entcnt[i], entnames[i], es.spawns[0], es.spawns[1], es.spawns[2], es.unknownspawns ? ',' : '\0', es.unknownspawns);
        break;
    case CTF_FLAG:
        conoutf(" %d %s, %d CLA, %d RVSF%c \f3unknown %d", es.entcnt[i], entnames[i], es.flags[0], es.flags[1], es.unknownflags ? ',' : '\0', es.unknownflags);
        break;
    case CLIP:
    case PLCLIP:
    ...
    if (es.entcnt[CTF_FLAG])
        conoutf(" flag distance: %d", es.flagentdistance);
