...
void gl_checkextensions()
{
    ...
    if (hasext(exts, "GL_EXT_texture_env_combine")
        ...
        conoutf("WARNING: cannot use overbright lighting, using old lighting "
                "model!");
    ...
    if (strstr(vendor, "S3 Graphics")) {
        // official UniChrome drivers can't handle glDrawElements inside a
        // display list without bugs
        extern int mdldlist;
        mdldlist = 0;
    }
#endif
            ...
            int subdiv)
{
    ...
    glVertex2f(x, y);
    loopi(subdiv + 1) {
        float c = cosf(2 * M_PI * i / float(subdiv)),
              s = sinf(2 * M_PI * i / float(subdiv));
...
void blendbox(int x1, int y1, int x2, int y2, bool border, int tex, color* c)
{
    ...
    if (tex >= 0) {
        ...
        loopj(rows) {
            ...
            float yboxcut = 0.0f;
            if ((j + 1) * texh > y2
                - y1) { // cut last row to match the box height
...
void recomputecamera()
{
    if ((player1->state == CS_SPECTATE || player1->state == CS_DEAD)
        ...
        case SM_FOLLOW3RD_TRANSPARENT: {
            ...
            // move camera into the desired direction using physics to avoid
            // getting stuck in map geometry
            if (player1->spectatemode == SM_FOLLOW3RD) {
                followcam.vel.x = -(float)(cosf(RAD * (p->yaw - 90)))
                                  * p->radius;
                followcam.vel.y = -(float)(sinf(RAD * (p->yaw - 90)))
                                  * p->radius;
...
void drawminimap(int w, int h)
{
    ...
    // float gdim = max(mapdims.xspan, mapdims.yspan)+2.0f; //make 1 cube
    // smaller to give it a black edge
...
void sethudgunperspective(bool on)
{
    ...
    if (on) {
        ...
        setperspective(75.0f, aspect, 0.3f,
                       farplane); // y fov fixed at 75 degrees
    } else
...
// acurately.
// sadly many very old cards and drivers appear to fuck up on glReadPixels() and
// give false
...
void gl_drawframe(int w, int h, float changelod, float curfps)
{
    ...
    float fogc[4]
        = {(fogcolour >> 16) / 256.0f, ((fogcolour >> 8) & 255) / 256.0f,
