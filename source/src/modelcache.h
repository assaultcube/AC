...
struct modelcache {
    ...
    template <class T> T* allocate(size_t reqsize) {
        ...
        if (curalloc)
            for (bool failed = false;;) {
                uchar* nextfree = curalloc ? (uchar*)curalloc + curalloc->size
                                           : (uchar*)buf;
    ...
    template <class T> void release(modelcacheentry<T>* e) {
        e->unlink();
    }
};
