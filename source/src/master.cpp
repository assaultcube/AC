...
COMMANDF(ban, "s", (char* name)
{
    addban(bans, name);
});
COMMANDF(servban, "s", (char* name)
{
    addban(servbans, name);
});
COMMANDF(gban, "s", (char* name)
{
    addban(gbans, name);
});
...
struct messagebuf {
    ...
    messagebuf(vector<messagebuf*>& owner)
        : owner(owner)
        , refs(0) {
    }
    ...
    for (;;) {
        ...
        loopv(gameservers) {
            ...
            if (s.address.host == addr.host && s.address.port == addr.port) {
                if (s.lastping
                    && (!s.lastpong
                        || ENET_TIME_GREATER(s.lastping, s.lastpong))) {
                    client* c = findclient(s);
                    if (c) {
...
{
    loopvrev(
        gameservers) if (checkban(servbans, gameservers[i]->address.host)) {
...
void purgeauths(client& c)
{
    int expired = 0;
    loopv(c.authreqs) {
        if (ENET_TIME_DIFFERENCE(servtime, c.authreqs[i].reqtime)
            >= AUTH_TIME) {
...
bool checkclientinput(client& c)
{
    ...
    while (end) {
        *end++ = '\0';
        ...
        int port;
        if (!strncmp(c.input, "list", 4)
            && (!c.input[4] || isspace(c.input[4]))) {
            genserverlist();
...
void checkclients()
{
    ...
    loopv(clients) {
        ...
        if ((c.message || c.output.length())
            ...
            const char* data = c.output.length() ? c.output.getbuf()
                               : c.message->getbuf();
            int len = c.output.length() ? c.output.length()
                      : c.message->length();
            ENetBuffer buf;
