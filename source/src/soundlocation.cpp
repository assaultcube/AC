    ...
    , playmillis(0)
{
    ...
    if (!sounds.inrange(sound)) {
        if (lastmillis - warn_about_unregistered_sound
            > 30 * 1000) // delay message to every 30 secs so console is not
            // spammed.
        {
            // occurs when a map contains an ambient sound entity, but sound
            // entity is not found in map cfg file.
            conoutf("\f3ERROR: this map contains at least one unregistered "
                    "ambient sound (sound entity# %d)",
                    sound);
    ...
    if ((r.type == worldobjreference::WR_ENTITY && cfg->maxuses >= 0
         && cfg->uses >= cfg->maxuses) || cfg->muted
        || (cfg->audibleradius
            && dist
            > cfg->audibleradius)) { // check max-use limits and audible radius
        stale = true;
        return;
    }
...
void location::updatepos()
{
    ...
    case worldobjreference::WR_ENTITY: {
        ...
        const float vol = eref.ent->attr4 <= 0.0f ? 1.0f : eref.ent->attr4
                          / 255.0f;
        float dist = camera1->o.dist(pos);
...
{
    if (sound < 0 || sound >= soundcollection.length())
        return NULL;
        ...
        const worldobjreference& newr)
{
    loopv(*this) {
        ...
        if (!l || !l->ref)
            continue;
        if (*l->ref == oldr)
            l->attachworldobjreference(newr);
    }
}
...
void locvector::updatelocations()
{
    ...
    loopv(*this) {
        ...
        if (l->stale)
            ...
            l->evaluateworldobjref(); // cam changed, evaluate world reference
        // again
    }
}
