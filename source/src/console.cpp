        ...
        int visible_lines
            = (int)(min(fullconsole
                ...
                : FONTH * consize,
                        (VIRTH * 2 - 2 * CONSPAD - 2 * FONTH / 3))
                    / (CONSPAD + 2 * FONTH / 3)) - 1;
        conskip = clamp(conskip + n, 0, clamp(conlines.length() - visible_lines,
                                              0, conlines.length()));
    }
...
COMMANDF(strstr, "ss", (char* a, char* b)
{
...
{
    loopv(keyms) if (!strcasecmp(keyms[i].actions[type],
                                 action)) return &keyms[i];
    return NULL;
}
...
COMMANDF(keybind, "s",
         (const char* key)
{
    keybind(key, keym::ACTION_DEFAULT);
});
COMMANDF(keyspecbind, "s",
         (const char* key)
{
    keybind(key, keym::ACTION_SPECTATOR);
});
COMMANDF(keyeditbind, "s",
         (const char* key)
{
    keybind(key, keym::ACTION_EDITING);
});
...
struct hline {
    ...
    bool shouldsave() {
        return strcmp(cmdline.buf, buf)
               || (cmdaction ? !action || strcmp(cmdaction, action) : action
                   != NULL)
               || (cmdprompt ? !prompt || strcmp(cmdprompt, prompt) : prompt
                   != NULL);
    }
...
void consolekey(int code, bool isdown, int cooked, SDLMod mod)
{
    ...
    if (isdown) {
        ...
        case SDLK_ESCAPE:
            if (mod & KMOD_LSHIFT) // LSHIFT+ESC restores buffer from before
                // complete()
            {
                if (beforecomplete)
            ...
            if (cmdline.buf[0]) {
                if (history.empty() || history.last()->shouldsave()) {
                    if (maxhistory && history.length() >= maxhistory) {
                        loopi(history.length() - maxhistory
                              + 1) delete history[i];
