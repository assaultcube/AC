...
struct console : consolebuffer<cline>
{
    ...
    void render()
    {
        ...
        if (fullconsole)
            blendbox(CONSPAD, CONSPAD, conwidth + CONSPAD + 2 * FONTH / 3, conheight + CONSPAD + 2 * FONTH / 3, true);
...
COMMANDF(
    modkeypressed, "", () { intret((SDL_GetModState() & MOD_KEYS_CTRL) ? 1 : 0); });
...
COMMANDF(
    _resetallbinds, "", () { if(_defaultbinds) loopv(keyms) loopj(keym::NUMACTIONS) bindkey(&keyms[i], "", j); });
...
COMMANDF(
    keybind, "s", (const char *key) { keybind(key, keym::ACTION_DEFAULT); });
COMMANDF(
    keyspecbind, "s", (const char *key) { keybind(key, keym::ACTION_SPECTATOR); });
COMMANDF(
    keyeditbind, "s", (const char *key) { keybind(key, keym::ACTION_EDITING); });

COMMANDF(
    bind, "ss", (const char *key, const char *action) { bindk(key, action, keym::ACTION_DEFAULT); });
COMMANDF(
    specbind, "ss", (const char *key, const char *action) { bindk(key, action, keym::ACTION_SPECTATOR); });
COMMANDF(
    editbind, "ss", (const char *key, const char *action) { bindk(key, action, keym::ACTION_EDITING); });

COMMANDF(
    searchbinds, "s", (const char *action) { searchbinds(action, keym::ACTION_DEFAULT); });
COMMANDF(
    searchspecbinds, "s", (const char *action) { searchbinds(action, keym::ACTION_SPECTATOR); });
COMMANDF(
    searcheditbinds, "s", (const char *action) { searchbinds(action, keym::ACTION_EDITING); });
...
void inputcommand(char *init, char *action, char *prompt, int *nopersist)
{
    saycommand(init);
    if (action[0])
        cmdaction = newstring(action);
    if (prompt[0])
        cmdprompt = newstring(prompt);
...
void pasteconsole(char *dst)
{
    ...
    for (char *cbline = cb, *cbend; commandlen + 1 < MAXSTRLEN && cbline < &cb[cbsize]; cbline = cbend + 1)
    {
        ...
        dst[commandlen] = '\n';
        if (commandlen + 1 < MAXSTRLEN && cbend < &cb[cbsize])
            ++commandlen;
        dst[commandlen] = '\0';
    }
    XFree(cb);
...
struct hline
{
    ...
    void restore()
    {
        ...
        if (cmdline.pos >= (int)strlen(cmdline.buf))
            cmdline.pos = -1;
        DELETEA(cmdaction);
        DELETEA(cmdprompt);
        if (action)
    ...
    void save()
    {
        ...
        if (cmdaction)
            action = newstring(cmdaction);
        if (cmdprompt)
            prompt = newstring(cmdprompt);
    }
...
void consolekey(int code, bool isdown, int cooked, SDLMod mod)
{
    ...
    if (isdown)
    {
        ...
        case SDLK_ESCAPE:
            ...
            {
                if (beforecomplete)
                    copystring(cmdline.buf, beforecomplete);
                ignoreescup = true;
            }
            else
                ignoreescup = false;
