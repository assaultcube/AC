...
struct vertmodel : model {
    struct anpos {
        ...
        void setframes(const animstate& as) {
            ...
            t = (time - fr1 * as.speed)
                / as.speed; // progress of the frame, value from 0.0f to 1.0f
            ASSERT(t >= 0.0f);
            if (as.anim & ANIM_LOOP) {
                fr1 = fr1 % as.range + as.frame;
                fr2 = fr1 + 1;
                if (fr2 >= as.frame + as.range)
                    fr2 = as.frame;
            } else {
    ...
    && !defined(__ICL) && __GNUC__ >= 4 && __GNUC_MINOR__ > 4
        ...
        dyncacheentry
            * gendynverts(animstate& as, anpos& cur, anpos* prev, float ai_t) {
            ...
            vec* buf = d->verts(), *vert1 = &verts[cur.fr1 * numverts],
                 *vert2 = &verts[cur.fr2 * numverts], *pvert1 = NULL,
                 *pvert2 = NULL;
            d->cur = cur;
            d->t = ai_t;
            if (prev) {
...
#define ip_v_ai(c) ip(ip_v(pvert, c, prev->t), ip_v(vert, c, cur.t), ai_t)
            if (prev)
                iploop(v = vec(ip_v_ai(x), ip_v_ai(y), ip_v_ai(z))) else iploop(
                    v = vec(ip_v(vert, x, cur.t), ip_v(vert, y, cur.t),
                            ip_v(vert, z, cur.t)))
#undef ip
#undef ip_v
#undef ip_v_ai
        ...
        void weldverts() {
            ...
            for (int i = 1; i < owner->numframes; i++) {
                ...
                loopj(numverts) {
                    if (frame[j] != frame[shareverts[j]])
                        shareverts[j] = (ushort)j;
                }
            }
        }
        ...
        void findneighbors() {
            ...
            loopi(numtris) {
                const tri& t = tris[i];
                loopj(3) {
                    uint e1 = shareverts[t.vert[j]],
                         e2 = shareverts[t.vert[(j + 1) % 3]], shift = 0;
                    if (e1 > e2) {
                        swap(e1, e2);
                        shift = 16;
                    }
    ...
    && !defined(__ICL) && __GNUC__ >= 4 && __GNUC_MINOR__ > 4
        __attribute__((optimize(2)))
#endif
        shadowcacheentry* genshadowvolume(animstate& as, anpos& cur,
                                          anpos* prev, float ai_t, vec* buf) {
            if (!shareverts)
                return NULL;
            ...
            loopi(numtris) {
                const tri& t = tris[i];
                const vec& a = buf[t.vert[0]], &b = buf[t.vert[1]],
                           &c = buf[t.vert[2]];
                side.add(vec().cross(vec(b).sub(a), vec(c).sub(a)).dot(
                             shadowdir) <= 0
                             ? 1
                             : 0);
            }
            ...
            loopi(numtris) if (side[i]) {
                ...
                loopj(3) {
                    ...
                    if (n == 0xFFFF || !side[n]) {
                        ushort e1 = shareverts[t.vert[j]],
                               e2 = shareverts[t.vert[(j + 1) % 3]];
                        *idx++ = e2;
                        *idx++ = e1;
                        *idx++ = numverts;
                    }
            ...
            if (dbgstenc
                ...
                conoutf("%s: %d tris", owner->filename, (idx - d->idxs()) / 3);
    ...
    && !defined(__ICL) && __GNUC__ >= 4 && __GNUC_MINOR__ > 4
        __attribute__((optimize(2)))
#endif
        lightcacheentry* lightvertexes(animstate& as, anpos& cur, anpos* prev,
                                       float ai_t, vec* buf) {
            if (dbgvlight)
                return NULL;
            ...
            if (OUTBORDRAD(modelpos.x, modelpos.y, dist))
                return NULL;
            ...
            if (d->verts() == lastcolorarray)
                lastcolorarray = (void*)-1;
            return d;
        }

        void getcurbb(bb& b, animstate& as, anpos& cur, anpos* prev,
                      float ai_t) {
            b = bbs[cur.fr1];
            b.add(bbs[cur.fr2]);
        ...
        void render(animstate& as, anpos& cur, anpos* prev, float ai_t) {
            ...
            if (as.anim & ANIM_NOSKIN && (!isstat || stenciling)) {
                ...
                if (owner->model->vertexlight) {
                    if (d)
                        d->locked = true;
                    lightcacheentry* l = lightvertexes(
                        as, cur, isstat ? NULL : prev, ai_t, buf);
                    if (d)
                        d->locked = false;
                    if (l)
                        vlight = l->verts();
            ...
            if (stenciling) {
                if (d)
                    d->locked = true;
                shadowcacheentry* s
                    = genshadowvolume(as, cur, isstat ? NULL : prev, ai_t, buf);
                if (d)
                    d->locked = false;
                if (!s)
                    return;
            ...
            if (builddlist)
                glNewList(statlist = glGenLists(1), GL_COMPILE);
            loopi(numdyndraws) {
    ...
    struct part {
        ...
        bool gentag(const char* name, int* verts, int numverts,
                    mesh* m = NULL) {
            if (!m) {
                if (meshes.empty())
                    return false;
            ...
            if (numverts < 1)
                return false;
            loopi(numverts) if (verts[i] < 0
                                || verts[i] > m->numverts) return false;
        ...
        bool addemitter(const char* tag, int type, int arg1 = 0, int arg2 = 0) {
            loopi(numtags) if (!strcmp(tags[i].name, tag)) {
                if (!links[i].emitter)
                    links[i].emitter = new particleemitter;
        ...
        void gentagmatrix(anpos& cur, anpos* prev, float ai_t, int i,
                          GLfloat* matrix) {
            tag* tag1 = &tags[cur.fr1 * numtags + i];
            tag* tag2 = &tags[cur.fr2 * numtags + i];
#define ip(p1, p2, t) (p1 + t * (p2 - p1))
#define ip_ai_tag(c) \
    ip(ip(tag1p->c, tag2p->c, prev->t), ip(tag1->c, tag2->c, cur.t), ai_t)
            if (prev) {
                ...
                loopj(3) matrix[j]
                    = ip(tag1->transform[0][j], tag2->transform[0][j],
                         cur.t); // transform
                loopj(3) matrix[4 + j]
                    = ip(tag1->transform[1][j], tag2->transform[1][j], cur.t);
                loopj(3) matrix[8 + j]
                    = ip(tag1->transform[2][j], tag2->transform[2][j], cur.t);
                loopj(3) matrix[12 + j]
                    = ip(tag1->pos[j], tag2->pos[j], cur.t); // position
            }
#undef ip_ai_tag
#undef ip
        ...
        bool calcanimstate(int anim, int varseed, float speed, int basetime,
                           dynent* d, animstate& as) {
            as.anim = anim;
            ...
            if (as.frame + as.range > numframes) {
                if (as.frame >= numframes)
                    return false;
                as.range = numframes - as.frame;
            }

            if (d && index < 2) {
                if (d->lastmodel[index] != this
                    || d->lastanimswitchtime[index] == -1
                    || lastmillis - d->lastrendered
                       > animationinterpolationtime) {
                    d->current[index] = as;
                    d->lastanimswitchtime[index]
                        = lastmillis - animationinterpolationtime * 2;
                } else if (d->current[index] != as) {
                    if (lastmillis - d->lastanimswitchtime[index]
        ...
        void render(int anim, int varseed, float speed, int basetime,
            ...
            if (meshes.empty())
                return;
            animstate as;
            if (!calcanimstate(anim, varseed, speed, basetime, d, as))
                return;
            ...
            bool doai = !(anim & ANIM_NOINTERP) && d && index < 2
                        && lastmillis - d->lastanimswitchtime[index]
                           < animationinterpolationtime
                        && d->prev[index].range > 0;
            if (doai) {
                prev.setframes(d->prev[index]);
                ai_t = (lastmillis - d->lastanimswitchtime[index])
            ...
            loopi(numtags) {
                ...
                // render the linked models - interpolate rotation and position
                // of the 'link-tags'
                glmatrixf linkmat;
        ...
        void setanim(int num, int frame, int range, float speed) {
            ...
            if (!anims)
                anims = new vector<animinfo>[NUMANIMS];
...
#define FILTER(c0, c1, c2, c3, c4, c5, c6, c7, c8)                       \
    {                                                                    \
        uint c = *src,                                                   \
             val = (filter3x3[0] * (c0)+filter3x3[1] * (c1)+filter3x3[2] \
                    * (c2)+filter3x3[3] * (c3)+filter3x3[4]              \
                    * (c4)+filter3x3[5] * (c5)+filter3x3[6]              \
                    * (c6)+filter3x3[7] * (c7)+filter3x3[8] * (c8));     \
...
#undef FILTER
        ...
        void genshadow(int aasize, int frame, stream* f) {
            ...
            if (f)
                f->write(&pixels[texsize * texsize], texsize * texsize);
        ...
        void genshadows(float height, float rad) {
            ...
            if (filename && loadshadows(filename))
                return;
        ...
        bool loadshadows(const char* filename) {
            ...
            if (f->read(buf, hdr.size * hdr.size * hdr.frames)
                != hdr.size * hdr.size * hdr.frames) {
                delete f;
                delete[] buf;
                return false;
            }
        ...
        void rendershadow(int anim, int varseed, float speed, int basetime,
                          const vec& o, float yaw) {
            if (!shadows)
                return;
            animstate as;
    ...
    void setskin(int tex = 0) {
        ...
        if (parts.length() < 1 || parts[0]->meshes.length() < 1)
            return;
...
modelcache vertmodel::dynalloc(mdldyncache << 20),
    vertmodel::statalloc(mdlstatcache << 20);
