...
#endif
    shadowcacheentry *
    genshadowvolume(animstate &as, anpos &cur, anpos *prev, float ai_t,
                    vec *buf) {
      ...
      loopi(numtris) {
        ...
        side.add(vec().cross(vec(b).sub(a), vec(c).sub(a)).dot(shadowdir) <= 0
                     ? 1
                     : 0);
      }
...
#endif
    lightcacheentry *
    lightvertexes(animstate &as, anpos &cur, anpos *prev, float ai_t,
                  vec *buf) {
      ...
      for (; d != lightcache.end(); d = d->next, cachelen++) {
        if (d->lastcalclight != lastcalclight || d->pos != modelpos ||
            d->roll != modelroll || d->yaw != modelyaw ||
            d->pitch != modelpitch || d->cur != cur)
          continue;
        if (prev) {
          if (d->prev == *prev && d->t == ai_t) return d;
        } else if (d->prev.fr1 < 0)
    ...
    void render(animstate &as, anpos &cur, anpos *prev, float ai_t) {
      ...
      if (as.anim & ANIM_NOSKIN && (!isstat || stenciling)) {
        ...
        if (owner->model->vertexlight) {
          if (d) d->locked = true;
          lightcacheentry *l =
              lightvertexes(as, cur, isstat ? NULL : prev, ai_t, buf);
          if (d) d->locked = false;
          if (l) vlight = l->verts();
        }
      }
...
#undef ip
    ...
    void render(int anim, int varseed, float speed, int basetime, dynent *d) {
      ...
      loopi(numtags) {
        ...
        if (!(link.p || link.pos || (anim & ANIM_PARTICLE && link.emitter)))
          continue;
    ...
    void setanim(int num, int frame, int range, float speed) {
      if (frame < 0 || frame >= numframes || range <= 0 ||
        ...
        conoutf("mdanim: invalid frame %d, range %d in model %s", frame, range,
                model->loadname);
        return;
      }
...
#define FILTER(c0, c1, c2, c3, c4, c5, c6, c7, c8)                    \
  {                                                                   \
    uint c = *src, val = (filter3x3[0] * (c0) + filter3x3[1] * (c1) + \
                          filter3x3[2] * (c2) + filter3x3[3] * (c3) + \
                          filter3x3[4] * (c4) + filter3x3[5] * (c5) + \
                          filter3x3[6] * (c6) + filter3x3[7] * (c7) + \
                          filter3x3[8] * (c8));                       \
    *dst++ = val / filter3x3sum;                                      \
    ...
    next++;                                                           \
      ...
      for (uint y = 1; y < size - 1; y++) {
        ...
        for (uint x = 1; x < size - 1; x++)
          FILTER(prev[-1], prev[0], prev[1], src[-1], c, src[1], next[-1],
                 next[0], next[1]);
...
#undef FILTER
    ...
    void genshadow(int aasize, int frame, stream *f) {
      ...
      model->startrender();
      render(ANIM_ALL | ANIM_NOINTERP | ANIM_NOSKIN, 0, 1, lastmillis - frame,
             NULL);
      model->endrender();
      ...
      if (aasize > 1 << dynshadowsize)
        scaletexture(pixels, aasize, aasize, 1, pixels, 1 << dynshadowsize,
                     1 << dynshadowsize);
    ...
    bool loadshadows(const char *filename) {
      ...
      loopi(hdr.frames) createtexture(shadows[i], hdr.size, hdr.size,
                                      &buf[i * hdr.size * hdr.size], 3, true,
                                      false, GL_ALPHA);
    ...
    void rendershadow(int anim, int varseed, float speed, int basetime,
      ...
      ::rendershadow(int(floor(o.x - shadowrad)), int(floor(o.y - shadowrad)),
                     int(ceil(o.x + shadowrad)), int(ceil(o.y + shadowrad)),
                     texgenS, texgenT);
    }
    ...
    float calcradius(float &zradius) {
      float rad = 0, zrad = 0;
      loopv(meshes) rad = max(rad, meshes[i]->calcradius(zrad)),
                    zradius = max(zradius, zrad);
      return rad;
    }
