...
#define ip_v_ai(c) ip(ip_v(pvert, c, prev->t), ip_v(vert, c, cur.t), ai_t)
            if (prev)
                iploop(v = vec(ip_v_ai(x), ip_v_ai(y), ip_v_ai(z))) else iploop(v = vec(ip_v(vert, x, cur.t), ip_v(vert, y, cur.t), ip_v(vert, z, cur.t)))
#undef ip
#undef ip_v
#undef ip_v_ai
        ...
        lightvertexes(animstate &as, anpos &cur, anpos *prev, float ai_t, vec *buf)
        {
            ...
            {
                if (d->lastcalclight != lastcalclight || d->pos != modelpos || d->roll != modelroll || d->yaw != modelyaw || d->pitch != modelpitch || d->cur != cur)
                    continue;
                if (prev)
                {
                    if (d->prev == *prev && d->t == ai_t)
        ...
        void render(animstate &as, anpos &cur, anpos *prev, float ai_t)
        {
            ...
            loopi(numdyndraws)
            {
                ...
                if (hasDRE && !builddlist)
                    glDrawRangeElements_(d.type, d.minvert, d.maxvert, d.count, GL_UNSIGNED_SHORT, &dynidx[d.start]);
                else
