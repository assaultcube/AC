...
int cornertest(int x, int y, int &bx, int &by, int &bs, sqr *&s, sqr *&h) // iteratively collide with a mipmapped corner cube
{
    ...
    {
        if (SOLID(w))
            res = 2; // corners between solids are solid behind the wall or SPACE in front of it
        else if (SOLID(v))
            res = 3;
    }
    else if (SOLID(t))
        ...
        if (e.type == CLIP || (e.type == PLCLIP && (d->type == ENT_BOT || d->type == ENT_PLAYER || (d->type == ENT_BOUNCE && ((bounceent *)d)->plclipped)))) // don't allow bots to hack themselves into plclips - Bukz 2011/04/14
        {
            ...
            if (hitarea)
            {
                ...
                if (e.attr6)
                    ...
                    case 1:
                        cz += (floor(0.5f + clamp(d->o.x - e.x + d->radius * (e.attr6 > 0 ? 1 : -1), -float(e.attr2) / ENTSCALE5, float(e.attr2) / ENTSCALE5))) * float(e.attr6) / (4 * ENTSCALE10);
                        break; // tilt x
                    case 2:
                        cz += (floor(0.5f + clamp(d->o.y - e.y + d->radius * (e.attr6 > 0 ? 1 : -1), -float(e.attr3) / ENTSCALE5, float(e.attr3) / ENTSCALE5))) * float(e.attr6) / (4 * ENTSCALE10);
                        break; // tilt y
                    }
        ...
        for (int x = x1; x <= x2; x++) // collide with map
        {
            ...
            case CORNER:
            {
                ...
                if (!match && d->type == ENT_BOUNCE)
                    match = q == (d->vel.x < 0) * 2 + (d->vel.y * d->vel.x < 0); // when coming towards corner surface: prefer corner bounce
                if (match && matter)
                {
                    if (!h)
                ...
                if (h)
                    tagclipped = false; // for corners between non-solids, tagclips extend the corner (visualisation does not reflect that, yet)
        ...
        loopi(moveres) // discrete steps collision detection & sliding
        {
            ...
            { // brute-force check, if it is a corner hit after all
                ...
                { // try really, really hard to detect corners for bounces (to compensate for the not-so-micro bounceent-microsteps)
                    ...
                    {
                        if (j)
                            vd = vec(ca * vd.x - sa * vd.y, ca * vd.y + sa * vd.x, 0);
                        pl->o = o_trying;
                        pl->o.add(vd);
                        if (collide(pl, false, drop, rise) && cornersurface)
                            break;
                    }
...
void physicsframe() // optimally schedule physics frames inside the graphics frames
{
    ...
    else
    {
        ...
        if (!multiplayer(NULL) && physsteps > 1000)
            physsteps = 1000;
    }
}
...
void jumpn(bool on)
{
    ...
    {
        if (lastmillis - player1->lastdeath > 1000 && on)
            togglespect();
    }
}
...
{
    if (intermission || (player1->isspectating() && player1->spectatemode == SM_FOLLOW1ST))
        return;
    ...
    { //      when scoped:
        if (scopesens > 0.0001f)
            cursens = scopesens; //          if specified, use dedicated (fixed) scope sensitivity
        else
            cursens *= autoscopesens ? autoscopesensscale : scopesensscale; //          or adjust sensitivity by given (fixed) factor or based on scopefov/fov
    }
