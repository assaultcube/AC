...
// tweaked until
// they "felt right", and have no basis in reality. Collision detection is
// simplistic but
...
{
    if (o->state != CS_ALIVE || !o->cancollide)
        return false;
    ...
    if ((d->type == ENT_PLAYER && o->type == ENT_PLAYER
         ? dr.sqrxy() < r * r
         : fabs(dr.x) < r && fabs(dr.y) < r) && dr.dotxy(d->vel) >= 0.0f) {
        if (d->o.z - deyeheight < o->o.z - oeyeheight) {
            if (o->o.z - oeyeheight < hi)
                hi = o->o.z - oeyeheight - 1;
               ...
               sqr*& h) // iteratively collide with a mipmapped corner cube
{
    ...
    if (SOLID(z)) {
        if (SOLID(w))
            res = 2; // corners between solids are solid behind the wall or
        ...
        if (e.type == CLIP
            || (e.type == PLCLIP
                && (d->type == ENT_BOT || d->type == ENT_PLAYER
                    || (d->type == ENT_BOUNCE
                        && ((bounceent*)d)->plclipped)))) // don't allow bots to
            // hack themselves
            // into plclips - Bukz
            // 2011/04/14
        {
            if (fabs(e.x - d->o.x) < e.attr2 + d->radius
                && fabs(e.y - d->o.y) < e.attr3 + d->radius) {
                const float cz = float(S(e.x, e.y)->floor + e.attr1),
                            ch = float(e.attr4);
...
{
    cornersurface = 0;
    const float fx1 = d->o.x - d->radius; // figure out integer cube rectangle
    // this entity covers in map
    ...
    for (int y = y1; y <= y2; y++)
        for (int x = x1; x <= x2; x++) { // collide with map
            ...
            sqr* s = S(x, y);
            if (s->tag & applyclip)
                return true; // tagged clips feel like solids
            float ceil = s->ceil;
            float floor = s->floor;
            switch (s->type) {
            case SOLID:
            ...
            case CORNER: {
                ...
                case 3:
                    ...
                    break;
                default:
                    return true; // mapper's fault: corner with unsufficient
                    // solids: renderer can't handle those anyway:
                    // treat as solid
                }
                cornersurface = (q & 1) ? 1 : 2;
                sqr* n = h && !matter ? h : ns;
                ceil = n->ceil; // use floor & ceil from higher mips (like the
                // renderer)
                floor = n->floor;
                if (!match && d->type == ENT_BOUNCE)
                    match = q
                            == (d->vel.x < 0) * 2
                            + (d->vel.y * d->vel.x < 0); // when coming
                // towards corner
                // surface: prefer
                // corner bounce
                if (match && matter) {
                    ...
                    else if (z1 < ns->floor || z2 > ns->ceil)
                        return true; // corner is not between solids, but we hit
                    // it...
                }
                cornersurface = 0;
                break;
    ...
    headspace -= 0.01f;
    if (mmcollide(d, hi, lo))
        return true; // collide with map models
...
void moveplayer(physent* pl, int moveres, bool local, int curtime)
{
    ...
    if (pl->type == ENT_BOUNCE) {
        ...
        const float friction = water ? 20.0f
                               : (pl->onfloor || isfly ? 6.0f : 30.0f);
        ...
        if (editfly) { // just apply velocity
            ...
            if (pl->jumpnext && !pl->trycrouch) {
                pl->jumpnext
                    = true; // fly directly upwards while holding jump keybinds
                pl->vel.z = 0.5f;
            } else if (pl->trycrouch && !pl->jumpnext) {
                pl->vel.z
                    = -0.5f; // fly directly down while holding crouch keybinds
            }
        } else if (specfly) {
                ...
                if (pl->onladder) {
                    ...
                    if (pl->onfloor || water) {
                        if (pl->jumpnext) {
                            ...
                            if (water) // dampen velocity change even harder,
                                // gives correct water feel
                            {
                                pl->vel.x /= 8.0f;
                                pl->vel.y /= 8.0f;
                        ...
                        if (pl->trycrouch && !pl->crouching
                            && !pl->crouchedinair && pl->state != CS_EDITING) {
                            pl->vel.z += 0.3f;
            ...
            if (water) {
                dropf = 5; // float slowly down in water
                pl->timeinair = 0;
            }
            if (pl->onladder) {
                dropf = 0;
    ...
    if (!editfly)
        ...
        if (pl->type != ENT_BOUNCE && hitplayer) {
            ...
            if (i == 0 && pl->type == ENT_BOT)
                pl->yaw += (dr.cxy(d) > 0
                            ? 2
                            : -2); // force the bots to change direction
            if (!collide(pl, false, drop, rise))
                continue;
            pl->o.x += f * d.x * push;
            pl->o.y += f * d.y * push;
    ...
    if (pl->type != ENT_BOUNCE && pl == player1) {
        ...
        else*/ if (pl->pitchvel < 0.05f
                   && pl->pitchvel > 0.001f)
            pl->pitchvel -= ((playerent*)pl)->weaponsel->info.recoilbackfade
                            / 100.0f; // slide back
        if (pl->pitchvel)
            fixcamerarange(pl); // fix pitch if necessary
    }
    ...
    if (pl->type == ENT_PLAYER || pl->type == ENT_BOT) {
        ...
        //         50))) updatecrouch(player1, player1->trycrouch);
...
// frames
{
    ...
    if (diff <= 0)
        ...
        physframetime
            = clamp((PHYSFRAMETIME * gamespeed) / 100, 1, PHYSFRAMETIME);
...
void jumpn(bool on)
{
    ...
    if (player1->isspectating()) {
        if (lastmillis - player1->respawnoffset > 1000 && on)
            togglespect();
...
void updatecrouch(playerent* p, bool on)
{
    ...
    if (p == player1)
        audiomgr.playsoundc(on ? S_CROUCH : S_UNCROUCH);
}
...
int tsens(int x)
{
    ...
    if (x == -2000) { // RENDERING PART!!!
        if (senst) {
            draw_textf(
                ...
                "%s\n2. try Low Sens. %s\n\fJ%s :"
                "\fE\n3. choose: High Sens.\n4. choose: Low Sens.\n\fIrepeat "
                "the steps above until the training stops.\n\f35. Stop "
                "Training.",
                VIRTW / 4, VIRTH / 3, hightry ? "(TRIED)" : "",
                lowtry ? "(TRIED)" : "",
                hightry && lowtry
    ...
    if (x == SDLK_3 || x == SDLK_4) {
        if (!hightry || !lowtry) {
            if (!hightry && !lowtry) {
                conoutf("--- \f3ERROR:\f0before choosing a sensitivity, first "
                        "try both higher and lower sens.");
            } else {
            ...
            if (sensh / sensn > 1.04f) {
                ...
                conoutf("--- \f0repeat previous steps by trying both higher "
                        "and lower sens and then by choosing the one you like "
                        "most.");
                hudoutf("next step!");
                nstep++;
            }
...
inline bool zooming(playerent* plx)
{
...
void mousemove(int odx, int ody)
{
    ...
    if (mouseaccel && curtime && (dx || dy))
        cursens += 0.02f * mouseaccel * sqrtf(dx * dx + dy * dy) / curtime;
