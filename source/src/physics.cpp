...
void moveplayer(physent *pl, int moveres, bool local, int curtime)
{
    ...
    const bool isfly = editfly || specfly || player1->spectatemode==SM_FLY;
    ...
    else // fake physics for player ents to create _the_ cube movement (tm)
    {
        ...
        const float speed = curtime/(water ? 2000.0f : 1000.0f)*pl->maxspeed*(crouching ? chspeed : 1.0f)*(pl==player1 && isfly && !slowmove ? 2.0f : 1.0f);
