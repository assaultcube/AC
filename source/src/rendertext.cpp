    ...
    if (c == 's') { // save color
        ...
        if (c == 'r')
            ...
            if (allowblinkingtext && !ignoreblinkingbit)
                stack[sp] *= -1; // blinking text - only if allowed
        } else
            stack[sp] = c;
        switch (abs(stack[sp])) {
        case '0':
        ...
        case '9':
            ...
            // lowercase seem to have special meaning like 'b' (flashing text) so
            // not yet using them
        case 'A':
...
void reloadfonts()
{
    enumerate(fonts, font, f, if (!reloadtexture(*f.tex))
