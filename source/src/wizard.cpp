...
int wizardmain(int argc, char** argv)
{
    ...
    printf("Before setting up a new server, please ensure you've read the "
           "rules at:\n"
           "\thttp://masterserver.cubers.net/rules.html\n\n");
    printf(
        "You will also need to ensure that the UDP port you choose is open.\n"
        "Whatever port you choose, you will need to forward that port, plus "
        "one port after that.\n"
    ...
    printf("Now to specify some optional settings for your server.\n"
           ...
           "field blank.\n\n"
           "Read http://assault.cubers.net/docs/commandline.html for a "
    ...
    if (toupper(ispub[0]) == 'N')
        addarg(argstr, "-mlocalhost");
    string cmds = "";
    ...
    printf("Done\n\n");
    printf("Note: You can start %s directly the next time you want to use the "
           "same configuration to start the server.\n\n",
           outfile);
    ...
    if (wsname[0]) {
        if (!wsdisplayname[0])
            copystring(wsdisplayname, wsname);
        ...
        if ((r = installer.IsInstalled()) != 0) {
            ...
            if (r == -1)
                ...
                printf("A windows service with this name (%s) is already "
                       "installed: %u\n",
