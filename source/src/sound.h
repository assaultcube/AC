...
enum {  // beware: sound order is part of the game protocol
  S_JUMP = 0,
  S_SOFTLAND,
  S_HARDLAND,
  S_BULLETAIR1,
...
#define gamesound_isvalid(x) \
  ((x) >= 0 && (x) < S_NULL)  // bounds check for soundcfg[]
#define gamesound_hasflag(x, mask) \
  (gamesound_isvalid(x) && (soundcfg[x].flags & mask) != 0)
#define gamesound_isvoicecom(x) \
  gamesound_hasflag(x, 1 << SC_VOICECOM)  // all voicecoms
...
enum { M_FLAGGRAB = 0, M_LASTMINUTE1, M_LASTMINUTE2 };

// sound priorities

enum { SP_LOW = 0, SP_NORMAL, SP_HIGH, SP_HIGHEST };
...
struct locvector : vector<location *> {
  ...
  location *find(int sound, worldobjreference *ref /* = NULL*/,
...
class audiomanager {
 ...
 public:
  ...
  int addsound(const char *name, int vol, int maxuses, bool loop,
               vector<soundconfig> &sounds, bool load, int audibleradius);
  ...
  // sound handling
  location *_playsound(int n, const worldobjreference &r, int priority,
                       float offset = 0.0f, bool loop = false);
