...
enum { // To avoid a lot of work, possible bugs and allow backward compatibility
       // for players own custom
    // cfg files, it's best not to change the listed sound order, instead, just
    // add to it.
    S_JUMP = 0,
    S_SOFTLAND,
    S_HARDLAND,
...
struct soundconfig {
    ...
    enum distancemodel {
        ...
        DM_LINEAR // custom linear model (used in conjunction with
                  // audibleradius)
    };
...
struct locvector : vector<location*> {
    ...
    location*
        find(int sound, worldobjreference* ref /* = NULL*/,
...
class audiomanager {
    ...
    int addsound(char* name, int vol, int maxuses, bool loop,
