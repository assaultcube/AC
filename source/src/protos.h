...
struct keym {
  ...
  enum { ACTION_DEFAULT = 0, ACTION_EDITING, ACTION_SPECTATOR, NUMACTIONS };
...
extern void menuitemmanual(void *menu, char *text, char *action = NULL,
                           color *bgcolor = NULL, const char *desc = NULL);
extern void menuimagemanual(void *menu, const char *filename1,
                            const char *filename2, char *text,
                            char *action = NULL, color *bgcolor = NULL,
                            const char *desc = NULL);
extern void menutitlemanual(void *menu, const char *title);
extern bool needscoresreorder;
extern void menuheader(void *menu, char *header, char *footer,
                       bool heap = false);
extern bool menukey(int code, bool isdown, int unicode, SDLMod mod = KMOD_NONE);
extern void *addmenu(const char *name, const char *title = NULL,
                     bool allowinput = true,
                     void(__cdecl *refreshfunc)(void *, bool) = NULL,
                     bool(__cdecl *keyfunc)(void *, int, bool, int) = NULL,
                     bool hotkeys = false, bool forwardkeys = false);
...
extern int connectwithtimeout(ENetSocket sock, const char *hostname,
...
struct httpget {
  ...
  uint elapsedtime;
  vector<uchar>
      *outvec;        // receives data, if not NULL (must be set up manually)
  ...
  int get(const char *url1, uint timeout, uint totaltimeout, int range = 0,
...
extern void blendbox(int x1, int y1, int x2, int y2, bool border, int tex = -1,
                     color *c = NULL);
extern void quad(GLuint tex, float x, float y, float s, float tx, float ty,
                 float tsx, float tsy = 0);
extern void quad(GLuint tex, vec &c1, vec &c2, float tx, float ty, float tsx,
                 float tsy);
extern void circle(GLuint tex, float x, float y, float r, float tx, float ty,
                   float tr, int subdiv = 32);
extern void framedquadtexture(GLuint tex, int x, int y, int xs, int ys,
                              int border = 0, int backlight = 255,
                              bool blend = false);
extern void setperspective(float fovy, float aspect, float nearplane,
                           float farplane);
...
// shadow
extern bool addshadowbox(const vec &bbmin, const vec &bbmax, const vec &extrude,
                         const glmatrixf &mat);
...
extern void scaletexture(uchar *src, uint sw, uint sh, uint bpp, uchar *dst,
                         uint dw, uint dh);
extern void createtexture(int tnum, int w, int h, void *pixels, int clamp,
                          bool mipmap, bool canreduce, GLenum format);
...
extern Texture *textureload(const char *name, int clamp = 0, bool mipmap = true,
                            bool canreduce = false, float scale = 1.0f,
                            bool trydl = false);
...
extern void setuptmu(int n, const char *rgbfunc = NULL,
...
extern void drawcrosshair(playerent *p, int n, struct color *c = NULL,
                          float size = -1.0f);
...
extern void rendershadow(int x, int y, int xs, int ys, const vec &texgenS,
                         const vec &texgenT);

// water
extern void setwatercolor(const char *r = "", const char *g = "",
...
extern void dodamage(int damage, playerent *pl, playerent *actor, int gun = -1,
                     bool gib = false, bool local = true);
...
extern char *colorname(playerent *d, char *name = NULL,
                       const char *prefix = "");
...
struct votedisplayinfo {
  ...
  votedisplayinfo()
      : owner(NULL), result(VOTE_NEUTRAL), millis(0), localplayervoted(false) {
    loopi(VOTE_NUM) stats[i] = VOTE_NEUTRAL;
  }
};

extern votedisplayinfo *newvotedisplayinfo(playerent *owner, int type,
...
extern void newentity(int index, int x, int y, int z, const char *what,
...
extern void adddynlight(physent *owner, const vec &o, int reach, int expire,
                        int fade, uchar r, uchar g = 0, uchar b = 0);
...
// worldrender
extern void render_world(float vx, float vy, float vh, float changelod, int yaw,
...
extern void popfont();
extern void draw_text(const char *str, int left, int top, int r = 255,
                      int g = 255, int b = 255, int a = 255, int cursor = -1,
                      int maxwidth = -1);
...
extern void text_bounds(const char *str, int &width, int &height,
                        int maxwidth = -1);
extern int text_visible(const char *str, int hitx, int hity, int maxwidth);
extern void text_pos(const char *str, int cursor, int &cx, int &cy,
                     int maxwidth);
...
extern void netblockpastexy(ucharbuf *p, int bx, int by, int bxs, int bys,
                            int light);
...
extern void show_out_of_renderloop_progress(float bar1, const char *text1,
                                            float bar2 = 0,
...
extern void particle_emit(int type, int *args, int basetime, int seed,
                          const vec &p);
extern void particle_fireball(int type, const vec &o);
extern void addshotline(dynent *d, const vec &from, const vec &to);
extern bool addbullethole(dynent *d, const vec &from, const vec &to,
                          float radius = 1, bool noisy = true);
...
extern void save_world(char *mname, bool skipoptimise = false,
                       bool addcomfort = false);
...
extern void rlencodecubes(vector<uchar> &f, sqr *s, int len,
                          bool preservesolids);
extern bool rldecodecubes(ucharbuf &f, sqr *s, int len, int version,
                          bool silent);
...
extern void setmapinfo(const char *newlicense = NULL,
...
extern int cornertest(int x, int y, int &bx, int &by, int &bs, sqr *&s,
                      sqr *&h);
...
extern bool objcollide(physent *d, const vec &objpos, float objrad,
                       float objheight);
extern bool collide(physent *d, bool spawn = false, float drop = 0,
                    float rise = 0);
extern void attack(bool on);

// rendermodel
extern void rendermodel(const char *mdl, int anim, int tex, float rad,
                        const vec &o, float roll, float yaw, float pitch,
                        float speed = 0, int basetime = 0, playerent *d = NULL,
                        modelattach *a = NULL, float scale = 1.0f);
...
extern void renderclient(playerent *d, const char *mdlname,
                         const char *vwepname, int tex = 0);
...
extern int intersect(playerent *d, const vec &from, const vec &to,
                     vec *end = NULL);
extern bool intersect(entity *e, const vec &from, const vec &to,
                      vec *end = NULL);
...
extern const char *formatentityattributes(const persistent_entity &e,
...
extern int variable(const char *name, int min, int cur, int max, int *storage,
                    void (*fun)(), bool persist);
extern float fvariable(const char *name, float min, float cur, float max,
                       float *storage, void (*fun)(), bool persist);
extern char *svariable(const char *name, const char *cur, char **storage,
...
extern void alias(const char *name, const char *action, bool temp = false,
                  bool constant = false);
...
extern char *filtertext(char *dst, const char *src, int flags,
                        int len = sizeof(string) - 1);
extern void filterrichtext(char *dst, const char *src,
                           int len = sizeof(string) - 1);
...
extern void serverms(int mode, int numplayers, int minremain, char *smapname,
                     int millis, const ENetAddress &localaddr, int *mnum,
                     int *msend, int *mrec, int *cnum, int *csend, int *crec,
                     int protocol_version);
extern int msgsizelookup(int msg);
extern const char *genpwdhash(const char *name, const char *pwd, int salt);
extern void servermsinit(const char *master, const char *ip, int serverport,
                         bool listen);
extern bool serverpickup(int i, int sender);
extern bool valid_client(int cn);
extern void extinfo_cnbuf(ucharbuf &p, int cn);
extern void extinfo_statsbuf(ucharbuf &p, int pid, int bpos,
                             ENetSocket &pongsock, ENetAddress &addr,
                             ENetBuffer &buf, int len, int *csend);
...
extern int calcmapareastats(mapareastats_s &ms, servsqr *s, int _ssize,
                            const mapdim_s &md);
extern void calcentitystats(entitystats_s &es, const persistent_entity *pents,
                            int pentsize);
...
extern bool initlogging(const char *identity, int facility_, int consolethres,
...
struct servercommandline {
  ...
  const char *ip, *master, *logident, *serverpassword, *adminpasswd, *demopath,
      *maprot, *pwdfile, *blfile, *nbfile, *infopath, *motdpath, *forbidden,
      *demofilenameformat, *demotimestampformat;
  ...
  bool checkarg(const char *arg) {
    ...
    int ai = atoi(a);
    // client: dtwhzbsave
    switch (arg[1]) {  // todo: gjlqEGHJQUYZ
      case '-':
