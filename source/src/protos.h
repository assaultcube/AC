...
extern void menumanual(void* menu, char* text, char* action = NULL,
...
extern entity* newentity(int index, int x, int y, int z, char* what, int v1,
...
extern void rlencodecubes(vector<uchar>& f, sqr* s, int len,
                          bool preservesolids);
extern void rldecodecubes(ucharbuf& f, sqr* s, int len, int version,
                          bool silent);
...
extern void vecfromyawpitch(float yaw, float pitch, int move, int strafe,
                            vec& m);
...
extern void audiomgr.playsoundc(int n, physent *p = NULL, int priority =
SP_NORMAL);
...
extern void rendermodel(const char* mdl, int anim, int tex, float rad,
                        const vec& o, float yaw, float pitch, float speed = 0,
                        int basetime = 0, playerent* d = NULL,
...
void TraceLine(vec from, vec to, dynent* pTracer, bool CheckPlayers,
               traceresult_s* tr, bool SkipTags = false);
extern void damageeffect(int damage, playerent* d);
extern void tryreload(playerent* p);
extern void checkweaponstate();
extern struct projectile* newprojectile(vec& from, vec& to, float speed,
                                        bool local, playerent* owner, int gun,
                                        int id = lastmillis);
...
// crypto // for AUTH
extern void genprivkey(const char* seed, vector<char>& privstr,
                       vector<char>& pubstr);
extern bool hashstring(const char* str, char* result, int maxlen);
const char* genpwdhash(const char* name, const char* pwd, int salt);
extern void answerchallenge(const char* privstr, const char* challenge,
                            vector<char>& answerstr);
extern void* parsepubkey(const char* pubstr);
extern void freepubkey(void* pubkey);
extern void* genchallenge(void* pubkey, const void* seed, int seedlen,
...
extern bool exechook(int context, const char* ident, const char* body,
                     ...); // execute cubescript hook if available and allowed
                           // in current context/gamemode
...
extern char* strreplace(char* dest, const char* source, const char* search,
                        const char* replace);
...
extern char* filtertext(char* dst, const char* src, int flags = 1,
...
struct servercommandline {
    ...
    const char* ip, *master, *logident, *serverpassword, *adminpasswd,
        *demopath, *maprot, *pwdfile, *blfile, *nbfile, *infopath, *motdpath,
        *forbidden, *killmessages, *demofilenameformat, *demotimestampformat;
                ...
                if (cfg.buf) {
                    ...
                    for (char* p = cfg.buf, *l; p < cfg.buf + cfg.filelen;
                        ...
                        if (*l && !this->checkarg(newstring(l)))
                            printf("unknown parameter in file '%s', line %d: "
                                   "'%s'\n",
                                   cfg.filename, line, l);
                    }
                } else
