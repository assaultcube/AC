                   ...
                   bool canreduce, GLenum format)
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                    mipmap
                    ? (trilinear ? (bilinear ? GL_LINEAR_MIPMAP_LINEAR
...
int fixcl(SDL_Surface* s, bool check = true, Uint8 value = 0,
          Uint8 mlimit = 255)
{
    Uint32 pixel = 0;
...
void blitsurface(SDL_Surface* dst, SDL_Surface* src, int x, int y)
{
    ...
    loop(dy, src->h) {
        loop(dx, src->w) {
            uint k1 = (255U - srcp[3]) * dstp[3], k2 = srcp[3] * 255U,
                 kmax = max(dstp[3], srcp[3]), kscale = max(kmax * 255U, 1U);
