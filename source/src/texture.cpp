...
void createtexture(int tnum, int w, int h, void *pixels, int clamp, bool mipmap, bool canreduce, GLenum format)
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                    mipmap ? (trilinear ? (bilinear ? GL_LINEAR_MIPMAP_LINEAR : GL_NEAREST_MIPMAP_LINEAR) : (bilinear ? GL_LINEAR_MIPMAP_NEAREST : GL_NEAREST_MIPMAP_NEAREST)) : (bilinear ? GL_LINEAR : GL_NEAREST));

    int tw = w, th = h;
    if (pixels)
        resizetexture(w, h, mipmap, canreduce, GL_TEXTURE_2D, tw, th);
...
GLuint loadsurface(const char *texname, int &xs, int &ys, int &bpp, int clamp = 0, bool mipmap = true, bool canreduce = false, float scale = 1.0f, bool trydl = false)
{
    ...
    if (uniformtexres && scale > 1.0f)
        scalesurface(s, 1.0f / scale);
...
COMMANDF(
    texturereset, "", () { if(execcontext==IEXC_MAPCFG) slots.setsize(0); });
...
COMMANDF(
    texture, "fs", (float *scale, char *name) {
        intret(slots.length());
        Slot &s = slots.add();
...
bool loadskymap(bool reload)
{
    ...
    if (autodownload && execcontext == IEXC_MAPCFG)
...
COMMANDF(
    getnotexture, "", () { result(mapconfigdata.notexturename); });
...
void scaletmu(int n, int rgbscale, int alphascale)
{
    tmu f = tmus[n];
    if (rgbscale)
        f.rgb.scale = rgbscale;
    if (alphascale)
        f.alpha.scale = alphascale;
    committmu(n, f);
}
...
void gettexturelist(char *_filter, char *_exclude, char *_exts) // create a list of texture paths and filenames
{
    ...
    loopv(files)
    {
        ...
        if (cutprefix)
            cvecprintf(res, " \"%s\"", int(strlen(p)) > filter_n[0] ? p + filter_n[0] : ""); // if there's exactly one filter string, add column with that string omitted
        if (cutext)
        {
            int n = strlen(b);
            if (n > exts_n[0])
...
void textureslotusagelist(char *what)
{
    ...
    vector<char> res;
    if (onlymostvisible)
        cvecprintf(res, "wall %d floor %d ceiling %d \"upper wall\" %d ", mostvisible[0], mostvisible[1], mostvisible[2], mostvisible[3]);
    else
...
void deletetextureslot(int *n, char *opt, char *_replace) // delete texture slot - only if unused or "purge" is specified
{
    ...
    if (purgeall && isdigit(*_replace))
        replace = strtol(_replace, NULL, 0) & 255;
    if (replace > *n)
        replace--;
    sqr *s = world;
    loopirev(cubicsize)
    ...
    loopk(3) // adjust texlists
    {
        ...
        loopi(256)
        {
            if (hdr.texlists[k][i] == *n)
                j = i;
            if (hdr.texlists[k][i] > *n)
                hdr.texlists[k][i]--;
        }
        if (j < 255)
            memmove(&hdr.texlists[k][j], &hdr.texlists[k][j + 1], 255 - j);
        ...
        if (!found)
        {
            if (slots.length() < 255)
            {
                ...
                if (!(hdr.flags & MHF_AUTOMAPCONFIG))
                    automapconfig(); // make sure, the new texture slots will be saved with the map
            }
            else
                conoutf("\f3failed to add texture \"%s\" (scale %.7g): no texture slot available", os.name, os.scale);
        }
    }
    return res;
}
...
void sorttextureslots(char **args, int numargs)
{
    ...
    while (presort.length() < 5)
        presort.add(presort.length());

    EDITMP("sorttextureslots");
    if (unknownarg || slots.length() < 5)
        return;
