...
bool fileexists(const char* path, const char* mode)
{
    ...
    if (GetFileAttributes(path) == INVALID_FILE_ATTRIBUTES)
        exists = false;
#else
    if (access(path, R_OK | (mode[0] == 'w' || mode[0] == 'a' ? W_OK : 0))
        == -1)
        exists = false;
#endif
    return exists;
...
char* getregszvalue(HKEY root, const char* keystr, const char* query)
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, keystr, 0, KEY_READ, &key)
        ...
        if (RegQueryValueEx(key, query, 0, &type, 0, &len) == ERROR_SUCCESS
            ...
            long result
                = RegQueryValueEx(key, query, 0, &type, (uchar*)val, &len);
...
void sethomedir(const char* dir)
{
    ...
    if (!strncmp(dir, substitute, strlen(substitute))) {
        const char* regpath = "Software\\Microsoft\\Windows\\CurrentVersion\\Ex"
                              "plorer\\Shell Folders";
        char* mydocuments
            = getregszvalue(HKEY_CURRENT_USER, regpath, "Personal");
        if (mydocuments) {
        ...
        while ((de = readdir(d)) != NULL) {
            if (!ext)
                ...
                if (namelength > 0 && de->d_name[namelength] == '.'
                    && strncmp(de->d_name + namelength + 1, ext, extsize - 1)
                    == 0)
...
struct filestream : stream {
    ...
    bool open(const char* name, const char* mode) {
        ...
        if (file && strchr(mode, 'w')) {
            ...
            if (fail
                || (int64_t)buf.f_frsize * (int64_t)buf.f_bavail < MINFSSIZE) {
                close();
                return false;
            }
        }
            ...
            if (inflateInit2(&zfile, -MAX_WBITS) != Z_OK)
                reading = false;
                   ...
                   && deflateInit2(&zfile, level, Z_DEFLATED, -MAX_WBITS,
                                   min(MAX_MEM_LEVEL, 8), Z_DEFAULT_STRATEGY)
                   != Z_OK)
            writing = false;
        if (!reading && !writing)
            return false;
