...
void neterr(const char* s)
{
    ...
    conoutf("\f3wrote a network error report to packetlog.txt, please post "
            "this file to the bugtracker now!");
                   ...
                   int revision) // forced map change from the server
{
    ...
    if (m_coop) {
        ...
        /*conoutf("SANITY drop from EDITING");*/ toggleedit(
            true); // fix stuck-in-editmode bug
    }
    ...
    if (download > 0) {
        ...
        if (watchingdemo) {
            if (loaded && !revmatch)
                conoutf(_("%c3demo was recorded on map revision %d, you have "
                          "map revision %d"),
            ...
            if (autogetmap || getnewrev) {
                ...
                if (!loaded || download < 10)
                    conoutf(_("\"getmap\" to download the current map from the "
                              "server"));
                else
                    conoutf(_("\"getmap\" to download a %s version of the "
                              "current map from the server"),
                            revision == 0
                            ? _("different")
    ...
    while (p.remaining())
        ...
        case SV_POSC: {
            ...
            if (type == SV_POSC) {
                ...
                // shoot = ( (g>>5) & 1 ? true : false ); // we are not using
                // this yet
                f = getuint(p);
            }
            int seqcolor = (f >> 6) & 1;
    ...
    while (p.remaining()) {
        ...
        case SV_SERVINFO: { // welcome message from the server
            ...
            if (prot != CUR_PROTOCOL_VERSION
                ...
                conoutf(_("%c3incompatible game protocol (local protocol: %d "
                          ":: server protocol: %d)"),
                        CC, CUR_PROTOCOL_VERSION, prot);
                conoutf("\f3if this occurs a lot, obtain an upgrade from "
                        "\f1http://assault.cubers.net");
                if (watchingdemo)
                    conoutf("breaking loop : \f3this demo is using a different "
                            "protocol\f5 : end it now!"); // SVN-WiP-bug: causes
                // endless retry loop
                // else!
                else
                    disconnect();
            ...
            if (d) {
                if (strcmp(d->name, text))
                    conoutf(_("%s is now known as %s"), colorname(d),
                            colorname(d, text));
                exechook(HOOK_SP, "onNameChange", "%d \"%s\"", d->clientnum,
                         text);
        ...
        case SV_EDITMODE: {
            ...
            if (val)
                ...
                // specators shouldn't be allowed to toggle editmode for
                // themselves. they're ghosts!
        ...
        case SV_SPAWNSTATE: {
            if (map_quality == MAP_IS_BAD) {
                ...
                conoutf(
                    _("map deemed unplayable - fix it before you can spawn"));
                break;
            }
        ...
        case SV_RESUME: {
            loopi(MAXCLIENTS) {
                ...
                if (p.overread() || cn < 0)
                    break;
                int state = getint(p), lifesequence = getint(p),
                    primary = getint(p), gunselect = getint(p),
                    flagscore = getint(p), frags = getint(p),
        ...
        case SV_EDITH: // coop editing messages, should be extended to include
        ...
        case SV_EDITENT: { // coop edit of ent
            ...
            if (ents[i].type == SOUND) {
                ...
                location* loc
                    = audiomgr.locations.find(e.attr1, &entref, mapsounds);
        ...
        case SV_SERVOPINFO: {
            ...
            if (cl >= 0 && r >= 0) {
                ...
                if (pl) {
                    pl->clientrole = r;
                    if (pl->name[0]) {
                        // two messages required to allow for proper german
                        // translation - is there a better way to do it?
                        if (pl == player1)
                            conoutf(_("you claimed %s status"),
                                    r == CR_ADMIN ? "admin" : "master");
                        else
                            conoutf(_("%s claimed %s status"), colorname(pl),
                                    r == CR_ADMIN ? "admin" : "master");
                    }
                }
            }
                    ...
                    if (d->team == fnt) {
                        ...
                        if (you && !watchingdemo) {
                            ...
                            case FTR_PLAYERWISH:
                                ...
                                break;
                            case FTR_AUTOTEAM:
                                hudoutf(_("the server forced you to team %s"),
                                        nts);
                                break;
                            }
                        } else {
                            const char* pls = colorname(d);
                            bool et = team_base(player1->team)
                                      != team_base(fnt);
                            switch (ftr) {
                            case FTR_PLAYERWISH:
                                conoutf(_("player %s switched to team %s"), pls,
                                        nts); // new message
                                break;
                            case FTR_AUTOTEAM:
                                if (watchingdemo || team_isspect(player1->team))
                                    conoutf(
                                        _("the server forced %s to team %s"),
                                        colorname(d), nts);
                                else
                                    hudoutf(
                                        _("the server forced %s to %s team"),
                                        colorname(d),
                                        et ? _("the enemy") : _("your"));
                                break;
                            }
                        }
...
void setDemoFilenameFormat(char* fmt)
{
    ...
    if (fmt && fmt[0] != '\0') {
        ...
        copystring(demofilenameformat, DEFDEMOFILEFMT); // reset to default if
    // passed empty string -
    // or should we output
...
void setDemoTimestampFormat(char* fmt)
{
    ...
    if (fmt && fmt[0] != '\0') {
        ...
        copystring(demotimestampformat, DEFDEMOTIMEFMT); // reset to default if
    // passed empty string
    // - or should we
    // output the current
    // value in this case?
...
void localservertoclient(int chan, uchar* buf, int len,
                         bool demo) // processes any updates from the server
{
    //    pktlogger.queue(enet_packet_create (buf, len, 0));  // log local &
    //    demo packets
