...
bool changemapserv(char *name, int mode, int download, int revision) // forced map change from the server
{
    ...
    if (download > 0)
    {
        ...
        {
            if (loaded && !revmatch)
                conoutf("\f3demo was recorded on map revision %d, you have map revision %d", revision, hdr.maprevision);
        }
        else
        {
            ...
            else
            {
                if (!loaded || download < 10)
                    ...
                    conoutf("\"getmap\" to download a %s version of the current map from the server",
...
void parsemessages(int cn, playerent *d, ucharbuf &p, bool demo = false)
{
    ...
    while (p.remaining())
    {
        ...
        case SV_MAPCHANGE:
        {
            ...
            if (m_arena && joining > 1 && !watchingdemo)
                deathstate(player1);
            break;
        }
        ...
        case SV_CDIS:
        {
            ...
            if (!d)
                break;
            if (d->name[0])
                conoutf("player %s disconnected", colorname(d));
        ...
        case SV_EDITBLOCK:
        {
            ...
            if (m_coop)
                netblockpastexy(pp, bx, by, bxs, bys, light);
            freegzbuf(pp);
            break;
        }
        ...
        case SV_NEWMAP:
        {
            ...
            if (!m_coop)
                break;
            if (size < 0 && sfactor > 9)
                break; // don't enlarge over 10 in MP
                        ...
                        else
                        {
                            ...
                            case FTR_AUTOTEAM:
                                if (watchingdemo || team_isspect(player1->team))
                                    ...
                                    hudoutf("the server forced %s to %s team", colorname(d), et ? "the enemy" : "your");
                                break;
                            }
                        }
