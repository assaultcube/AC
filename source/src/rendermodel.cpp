...
COMMANDF(mapmodelname, "i", (int* idx)
{
    result(mapmodels.inrange(*idx) ? mapmodels[*idx].name : "");
});
...
model*
loadmodel(const char* name, int i,
...
void renderbatchedmodelshadow(model* m, batchedmodel& b)
{
    ...
    if (dynshadowdecay)
        ...
        case ANIM_LYING_DEAD:
            intensity *= max(
                             1.0f - float(lastmillis - b.basetime) / dynshadowdecay, 0.0f);
            break;
        }
...
void rendermodel(const char* mdl, int anim, int tex, float rad, const vec& o,
                 float yaw, float pitch, float speed, int basetime,
...
int findanim(const char* name)
{
    ...
    loopi(sizeof(names)
          / sizeof(names[0])) if (!strcmp(name, names[i])) return i;
    return -1;
}
...
void preload_playermodels()
{
    ...
    loopi(NUMGUNS) {
        if (i == GUN_CPISTOL)
            continue; // RR 18/12/12 - Remove when cpistol is added.
        defformatstring(widn)("modmdlvwep%d", i);
        defformatstring(vwep)("weapons/%s/world", identexists(widn)
                              ? getalias(widn)
                              : guns[i].modelname);
...
void preload_entmodels()
{
    ...
    static const char* bouncemdlnames[]
        = { "misc/gib01", "misc/gib02", "misc/gib03",
...
void preload_mapmodels(bool trydl)
{
    loopv(ents) {
        ...
        if (e.attr4)
            lookuptexture(e.attr4, notexture, trydl);
    }
}
                  ...
                  int tex)
{
    ...
    if (d->state == CS_DEAD) {
        if (d == player1 && d->allowmove())
            return;
        loopv(bounceents) if (bounceents[i]->bouncetype == BT_GIB
                              && bounceents[i]->owner == d) return;
        d->pitch = 0.1f;
        anim = ANIM_DEATH;
        varseed += d->lastpain;
        basetime = d->lastpain;
    ...
    //     else if(!d->onfloor && d->timeinair>50)         { anim =
    //     ANIM_JUMP|ANIM_END; }
...
void renderclient(playerent* d)
{
    ...
    if (cs)
        ...
        if (!m_teammode || !teamdisplaymode)
            formatstring(skin)("%s/%s/%02i.jpg", skinbase, teamname, skinid);
        else
            switch (teamdisplaymode) {
            case 1:
                formatstring(skin)("%s/%s/%02i_%svest.jpg", skinbase, teamname,
    ...
    if (d->weaponsel)
        formatstring(vwep)("weapons/%s/world",
                           identexists(widn) ? getalias(widn)
                           : d->weaponsel->info.modelname);
    else
        vwep[0] = 0;
