...
void mdlattribute(char *attrname, char *val)
{
    ...
    if (i < MMA_NUM)
    {
        ...
        if (*val)
            loadingattributes.n[i] = newstring(val);
    }
}
...
void mapmodel(int *rad, int *h, int *zoff, char *scale, char *name, char *flags)
{
    if (*scale && *name) // ignore "mapmodel" commands with insufficient parameters
    {
        ...
        } // throw errors for unconverted mapmodels (unspecific, because not all get detected)
...
COMMANDF(
    mapmodelslotname, "i", (int *idx) { result(mapmodels.inrange(*idx) ? mmshortname(mapmodels[*idx].name) : ""); }); // returns the model name that is configured in a certain slot
COMMANDF(
    mapmodelslotbyname, "s", (char *name) // returns the slot(s) that a certain mapmodel is configured in
    {
        string res = "";
...
void editmapmodelslot(int *n, char *rad, char *h, char *zoff, char *scale, char *name) // edit slot parameters != ""
{
    ...
    if (mapmodels.inrange(*n))
    {
        ...
        { // change attributes
            if (*rad)
                mmi.rad = strtol(rad, NULL, 0);
            if (*h)
                mmi.h = strtol(h, NULL, 0);
            if (*zoff)
                mmi.zoff = strtol(zoff, NULL, 0);
            float s = atof(scale);
            if (s < 0.25f || s > 4.0f)
                s = 1.0f;
            if (*scale)
...
int tempmmcmp(tempmmslot *a, tempmmslot *b)
{
    ...
    if (n)
        return n;
    if (a->m.rad != b->m.rad)
        return a->m.rad - b->m.rad;
    if (a->m.h != b->m.h)
        return a->m.h - b->m.h;
    if (a->m.zoff != b->m.zoff)
        return a->m.zoff - b->m.zoff;
    if (a->m.scale != b->m.scale)
        return int((a->m.scale - b->m.scale) * 1e6);
    return 0;
}
...
void sortmapmodelslots(char **args, int numargs)
{
    ...
    if (unknownarg || mapmodels.length() < 3)
        return;
...
void setmodelattributes(const char *name, mapmodelattributes &ma)
{
    if (!strchr(name, ' ') && !strncmp(name, mmpath, strlen(mmpath)))
        name += strlen(mmpath); // for now: ignore mapmodels with spaces in the path (next release: ban them)
    else
...
model *loadmodel(const char *name, int i, bool trydl) // load model by name (optional) or from mapmodels[i]
{
    if (!name) // name == NULL -> get index i from mapmodels[]
    {
        ...
        if (mmi.m)
            return mmi.m; // mapmodels[i] was already loaded
        name = mmi.name;
    }
    ...
    while (foundone)
    {
        ...
        loopvk(mapmodels) if (mapmodels[k].flags & MMF_TEMP_USED)
        {
            ...
            if (dep && *dep)
            {
                ...
                {
                    if (!(mapmodels[i].flags & MMF_TEMP_USED))
                        foundone = true;
...
void renderbatchedmodelshadow(model *m, batchedmodel &b)
{
    ...
    if (s->type == FHF)
        center.z -= s->vdelta / 4.0f;
    if (dynshadowquad && center.z - 0.1f > b.o.z)
        return;
    center.z += 0.1f;
    modelattach *a = NULL;
    if (b.attached >= 0)
...
void rendermodel(const char *mdl, int anim, int tex, float rad, const vec &o, float roll, float yaw, float pitch, float speed, int basetime, playerent *d, modelattach *a, float scale)
{
    ...
    if (rad >= 0)
    {
        if (!rad)
        {
            rad = m->radius;
            if (roll != 0 || pitch != 0)
                rad = max(m->radius, m->zradius); // FIXME: this assumes worst-case even for small angles and should be eased up (especially for lamp posts)
            rad *= scale;
        }
    ...
    if (stenciling && d && !raycubelos(camera1->o, o, d->radius))
    {
        ...
        if (!raycubelos(camera1->o, target, d->radius))
            return;
    }
...
void preload_playermodels()
{
    ...
    if (dynshadow && playermdl)
        playermdl->genshadows(8.0f, 4.0f);
    loopi(NUMGUNS)
    {
        if (i == GUN_CPISTOL)
            continue;
        ...
        if (dynshadow && vwepmdl)
            vwepmdl->genshadows(8.0f, 4.0f);
    }
}
...
bool preload_mapmodels(bool trydl)
{
    ...
    {
        entity &e = ents[i];
        if (e.type != MAPMODEL || !mapmodels.inrange(e.attr2))
            continue;
        mapmodels[ents[i].attr2].flags |= MMF_TEMP_USED;
        if (e.attr4 && lookuptexture(e.attr4, notexture, trydl) == notexture && (!_ignoreillegalpaths || gettextureslot(e.attr4)))
            missing++;
    }
...
const char *getclientskin(const char *name, const char *suf)
{
    ...
    loopv(playerskinlist)
    {
        ...
        {
            if (namelen == sl && !strncmp(name, s, namelen))
...
void updateclientname(playerent *d)
{
    ...
    if (!gotlist)
        listfiles(SKINBASE "/custom", "jpg", playerskinlist);
    gotlist = true;
    if (!d || !playerskinlist.length())
        return;
