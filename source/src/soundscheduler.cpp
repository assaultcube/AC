...
// out of sources by floods of low-priority sounds.
// Under load it uses priority and distance information to reassign its
// resources.
...
source* sourcescheduler::newsource(int priority, const vec& o)
{
    ...
    if (!src) {
        ...
        loopv(sources) {
            ...
            float otherdist = otherpos.iszero() ? 0.0f
                              : camera1->o.dist(otherpos);
