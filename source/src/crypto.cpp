...
#ifdef _DEBUG
COMMANDF(
    sha512, "s", (char *msg) // SHA512 debug command
    {
...
COMMANDF(
    tiger, "s", (char *msg) // tiger debug command
    {
        tiger::hashval hash;
...
void entropy_init(uint seed)
{
    ...
    if (buf && len)
        entropy_add_block((uchar *)buf, len);
#ifndef STANDALONE
    else
    {
...
void privkey_from_prepriv(unsigned char *privkey, const unsigned char *prepriv, int preprivlen, unsigned char *privpriv = NULL) // derive private key from even more secret bulk of entropy
{
    ...
    if (privpriv)
        memcpy(privpriv, temp + minescpos, 32);
}
...
void authsetup(char **args, int numargs) // set up private and public keys
{
    ...
    if (!buf)
        entropy_get((buf = new uchar[1536]), 1536);
    uint32_t offs = 7337;
    ...
    if (passdrunning)
    {
        ...
        { // finish password decryption (background mode)
            ...
            return; // ignore further "authsetup" commands while a password decryption is running in the background
    }
    else
    {
        if (numargs < 0)
    ...
    if (numargs > 0)
    {
        if (!strcasecmp(args[0], "PRE"))
        {
            ...
            if (numargs > 1)
                hex2bin(prepriv, args[1], preprivlen);
            if (numargs > 2)
                hex2bin(psalt, args[2], 16);
        ...
        else if (!strcasecmp(args[0], "PRIV"))
        {
            ...
            if (numargs > 1)
                hex2bin(priv, args[1], 32);
            if (numargs > 2)
                hex2bin(salt, args[2], 16);
        ...
        else if (!strcasecmp(args[0], "PPASS"))
        {
            ...
            if (iterations)
                passphrase2key(numargs > 1 ? args[1] : "", psalt, 16, keyhash, preprivlen, &iterations, 0, preprivpwdcfg & 0x3fe);
        ...
        else if (!strcasecmp(args[0], "PASS"))
        {
            ...
            if (iterations)
                passphrase2key(numargs > 1 ? args[1] : "", salt, 16, keyhash, 32, &iterations, 0, privpwdcfg & 0x3ff);
        ...
        else if (!strcasecmp(args[0], "GENPRIV"))
        {
            // authsetup genpriv
            if (!preprivpwdcfg)
                privkey_from_prepriv(priv, prepriv, preprivlen);
            privpwdcfg = 0;
        }
...
bool cert::parse() // parse orgmsg[] and check the signature
{
    ...
    { // frame ok
                ...
                if (line.val && line.val > l)
                {
                    ...
                    *line.val++ = '\0';
                    ...
                    { // parse "signed-by" key directly
                        if (!(strlen(line.val) == 64 && (signedby = (uchar *)line.val) && hex2bin(signedby, orgmsg + (line.val - workmsg), 32) == 32))
                            signedby = NULL;
                    }
                    ...
                    { // parse "pubkey" key directly
                        if (!(strlen(line.val) == 64 && (pubkey = (uchar *)line.val) && hex2bin(pubkey, orgmsg + (line.val - workmsg), 32) == 32))
                            pubkey = NULL;
                    }
            ...
            { // check signature (if signature contained a "signed-by" line)
                if (ed25519_sign_check(signature, orglen - certheaderlenfull + 64, signedby))
                {
                    ...
                    if (certtypeexpiration[type])
                    {
                        ...
                        if (age > certtypeexpiration[type])
                            needsrenewal = true;
                        if (age > 3 * certtypeexpiration[type])
                            expired = true;
                    }
                    isvalid = !expired;
                }
        ...
        if (ischecked && pubkey && !name)
            bin2hex((name = workmsg), pubkey, 32); // if cert has no name, use pubkey as name
    }
    return ischecked;
}
...
COMMANDF(
    newcert, "v", (char **args, int numargs) // create and sign certs from cubescript
    {
                ...
                for (cert *r = treechain; r; r = r->next)
                {
                    if (r->rank && r->isvalid && r->pubkey && !memcmp(r->pubkey, c->signedby, 32))
                    {
                        ...
                        { // include cert into tree
                            ...
                            treechain = c;
                            if (c->pubkey)
                                onemoreround = true; // more public keys in the tree -> need to check the remaining certs again
                        }
                        break;
                    }
                }
