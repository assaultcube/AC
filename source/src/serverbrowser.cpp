                       ...
                       ENetAddress& address)
{
    ...
    if (!connmutex)
        connmutex = SDL_CreateMutex();
    if (!conncond)
        conncond = SDL_CreateCond();
    SDL_LockMutex(connmutex);
    ...
    for (;;) {
        ...
        if (timeout > CONNLIMIT)
            break;
    }

    /* thread will actually timeout eventually if its still trying to connect
     * so just leave it (and let it destroy socket) instead of causing problems
     * on some platforms by killing it
     */
    connthread = NULL;
    SDL_UnlockMutex(connmutex);
...
void checkresolver()
{
    ...
    loopv(servers) {
        ...
        if (si.resolved == serverinfo::RESOLVED)
            continue;
        if (si.address.host == ENET_HOST_ANY) {
...
#define ADDINFOLINE(msg)                                 \
    ...
    sendstring(msg, q)
                case EXTPING_NAMELIST: {
                    ...
                    loopi(si->numplayers) {
                        ...
                        if (text[0] && !p.overread()) {
                            si->playernames.add((const char*)si->namedata
                                                + q.length());
                            sendstring(text, q);
                        } else
                            break;
                ...
                case EXTPING_SERVERINFO: {
                    ...
                    getstring(text, p);
                    if (strlen(text) != 2) {
                        ADDINFOLINE("this server does not provide additional "
                                    "information");
                        break;
                    }
                    strcpy(si->lang, text);
                ...
                case EXTPING_UPLINKSTATS: {
                        ...
                        loopi(si->maxclients - 3) {
                            ...
                            if (si->maxclients < 8 || i > 2) {
                                ...
                                if (ts > maxs)
                                    maxs = ts;
                                if (ts > 0x40 && tc > maxc)
                                    maxc = tc; // spent time = 2 ^ ((ts >> 4) -
                                // 1) * 30 sec, so 0x50 is 8..15
                                // minutes
                            }
                        }
                        if (maxs < 0x90)
                            maxc -= 2; // go easy on fresh started servers
                        if (maxs < 0x50)
                            si->uplinkqual = 3;
                        else if (maxc < 3)
                            si->uplinkqual = 5; // choke_percentage = 1 / 2 ^
                        // (15 - tc), so 0x03 is 0.02%
                        else if (maxc < 6)
                            si->uplinkqual = 4; // 0.2%
                        else if (maxc == 6)
                            si->uplinkqual = 3;
                    ...
                    if (si->getinfo == EXTPING_UPLINKSTATS) {
                        RESETINFOLINES();
                        if (si->maxclients < 4) {
                            ADDINFOLINE("server is too small for uplink "
                                        "quality statistics");
                        } else {
                            ADDINFOLINE("\f1server uplink quality and usage "
                                        "statistics:");
                            ADDINFOLINE("");
                            ...
                            ADDINFOLINE("");
                            ADDINFOLINE("\f4red bar: error rate, green bar: "
                                        "time played");
                        }
                    }
                    break;
...
int sicompare(serverinfo** ap, serverinfo** bp)
{
    ...
    case SBS_MAXPL: // maxplayers
        if (a->maxclients < b->maxclients)
            return dir;
        if (a->maxclients > b->maxclients)
            return -dir;
        break;
    case SBS_MINREM: // minutes remaining
        if (a->minremain < b->minremain)
            return dir;
        if (a->minremain > b->minremain)
            return -dir;
        break;
...
void addfavcategory(const char* refdes)
{
    ...
    const int defk[]
        = { FC_WEIGHT, FC_DESC, FC_TAG, FC_KEYS, FC_IGNORE, FC_ALPHA };
    const char* defv[] = { "0", val, refdes, "", "", "20" };
    loopi(sizeof(defk) / sizeof(defk[0])) {
        if (!alx[defk[i]])
            alias(favcatargname(text, defk[i]), defv[i]);
    }
                    ...
                    const char* key)   // IP:port, #gamemode ,%mapname, desc
{
    ...
    if (isdigit(*key)) { // IP
        ...
        case '$':
            if (key[1]) {
                formatstring(text)("%s \"%s\" %d %d %d %d %d \"%s\" %d %d",
                                   key + 1, si.map, si.mode, si.ping,
                                   si.minremain, si.numplayers, si.maxclients,
    ...
    if (cats.length()) {
        loopv(cats) {
            if (cats[i] == showonlyfavourites) {
                showonlyfavourites = cats[(i + shiftdirection + cats.length())
                                          % cats.length()];
                return;
            }
        }
        ...
        while (k) {
            loopv(servers) {
                ...
                if (si.address.host == ENET_HOST_ANY || si.ping == 9999
                    ...
                    continue;
                if ((!alxn[FC_IGNORE] || showonlythiscat)
                    && favcatcheckkey(si, k)) {
                    ...
                    if (si.favcat == -1 || showonlythiscat) {
                        ...
                        if (alxn[FC_ALPHA]) {
                            if (!si.bgcolor)
                                si.bgcolor = new color;
                            new (si.bgcolor)
                            color(((float)alxn[FC_RED]) / 100,
...
void refreshservers(void* menu, bool init)
{
    ...
    if (!init && menu) { // && servers.inrange(((gmenu *)menu)->menusel))
        ...
        loopv(servers) if (servers[i]->menuline_from == ((gmenu*)menu)->menusel
                           && servers[i]->menuline_to
                           > servers[i]->menuline_from) {
            foundserver = servers[i];
            break;
        }
        if (foundserver) {
            if ((usedselect || ((gmenu*)menu)->menusel > 0))
                oldsel = foundserver;
            lastselectedserver = foundserver;
        }
    }
    if (isscoreboard)
        lastselectedserver = curserver;
    ...
    if (menu) {
        ...
        [NUMSERVSORT] = {
            ...
            "%sping\tplr\tserver (\fs\f0max players\fr)%s%s", // 2:
            // maxplayers
            "%sping\tplr\fs\f0\fr\tserver (\fs\f0minutes "
        ...
        loopv(servers) {
            ...
            bool showthisone = !(banned && showonlygoodservers)
                               && !(showonlyfavourites > 0
                                    && si.favcat != showonlyfavourites - 1);
            ...
            char basecolor = banned ? '4'
                             : (curserver == servers[i] ? '1' : '5');
            char plnumcolor
                = serverfull
                  ? '2'
                  : (needspasswd ? '3'
                     : (mmode != MM_OPEN ? '1' : basecolor));
            ...
            if (si.address.host != ENET_HOST_ANY && si.ping != 9999) {
                if (si.protocol != PROTOCOL_VERSION) {
                    ...
                    if (!hidefavicons && !showweights && showfavtag
                        && si.favcat > -1)
                        favimage = getalias(
                                       favcatargname(favcats[si.favcat], FC_IMAGE));
                    filterrichtext(text, si.favcat > -1 && !favimage
                                   ? favcattags[si.favcat]
                                   : "");
                    if (showweights)
                        concatformatstring(text, "(%d)", si.weight);
                    formatstring(si.full)(
                        showfavtag ? (favimage ? "\t" : "\fs%s\fr\t") : "",
                            text);
                    concatformatstring(
                        si.full, "\fs\f%c%s\t\fs\f%c%d/%d\fr\t\a%c  ",
                    ...
                    if (si.map[0]) {
                        ...
                        if (showmr)
                            concatformatstring(si.full, ", (%d)", si.minremain);
                    } else
                        concatformatstring(si.full, "empty");
                    concatformatstring(si.full, serverbrowserhideip < 2
                                       ? ": \fs%s%s:%d\fr"
                                       : ": ",
                                       serverbrowserhideip == 1 ? "\f4" : "",
                                       si.name, si.port);
            ...
            if (showthisone) {
                cutcolorstring(si.full,
                               76); // cut off too long server descriptions
                ...
                if (sbconnectexists) {
                    ...
                    for (char* p = text; (p = strchr(p, '\"')); *p++ = ' ')
                        ;
                    text[30] = '\0';
                    formatstring(si.cmd)("sbconnect %s %d %d %d %d %d \"%s\"",
                                         si.name, si.port, serverfull ? 1 : 0,
                                         needspasswd ? 1 : 0, mmode, banned,
                                         text);
                } else
                ...
                if (!issearch && servers[i] == oldsel) {
                    ((gmenu*)menu)->menusel = ((gmenu*)menu)->items.length()
                                              - 1;
                    usedselect = true;
                    if (shownamesinbrowser)
                        si.getinfo = EXTPING_NAMELIST;
                }
                if ((shownamesinbrowser && servers[i] == oldsel
                     && si.playernames.length()) || issearch) {
                    int cur = 0;
                    char* t = NULL;
                    loopvj(si.playernames) {
                        ...
                        concatformatstring(
                            t, " \t\fs%s%s\fr",
                            !issearch || matchplayername(si.playernames[j])
                            ? ""
                            : "\f4",
                            si.playernames[j]);
                        cur++;
        ...
        } else if (!((gmenu*)menu)->items.length() && showonlyfavourites
                   && favcats.inrange(showonlyfavourites - 1)) {
            const char* desc = getalias(
                                   favcatargname(favcats[showonlyfavourites - 1], FC_DESC));
            formatstring(notfoundmsg)(
                "no servers in category \f2%s",
...
{
    const int fk[] = { SDLK_1, SDLK_2, SDLK_3, SDLK_4, SDLK_5,
    ...
    loopi(sizeof(fk) / sizeof(fk[0])) if (code == fk[i] && favcats.inrange(i)) {
        ...
        loopvj(servers) if (menu && (servers[j]->menuline_from <= sel
            ...
            if (!desc)
                desc = "";
            if (*ak) { // server was automatically added to this favourite
                // group, don't remove
                conoutf(_("server \"%cs%s%cr\" is in category '%cs%s%cr' "
                          "because of key '%s', please remove manually"),
                        CC, servers[j]->sdesc, CC, CC, desc, CC, ak);
            } else if (rest) { // remove from favourite group
                conoutf(_("removing server \"%cs%s%cr\" from favourites "
                          "category '%cs%s%cr' (rest '%s')"),
                ...
                if (key && *key) {
                    char* newkey
                        = newstring(key, strlen(text) + 1 + strlen(key));
                    strcat(newkey, " ");
                    strcat(newkey, text);
                    ...
                    alias(keyalias, text);
                conoutf(_("adding server \"%cs%s%cr\" to favourites category "
                          "'%cs%s%cr' (new '%s')"),
...
bool serverinfokey(void* menu, int code, bool isdown, int unicode)
{
    ...
    case SDLK_F5:
        if (lastselectedserver)
            lastselectedserver->getinfo = EXTPING_SERVERINFO;
    ...
    case SDLK_F2: // yep: undocumented :)
        if (lastselectedserver)
            lastselectedserver->getinfo = EXTPING_UPLINKSTATS;
        pinglastselected = true;
        break;

    case SDLK_m:
        if (lastselectedserver)
            lastselectedserver->getinfo = EXTPING_MAPROT;
        pinglastselected = true;
        break;
    }
    return false;
...
extern size_t write_callback(void* ptr, size_t size, size_t nmemb,
                             FILE* stream);
...
int progress_callback_retrieveservers(void* clientp, double dltotal,
...
void retrieveservers(vector<char>& data)
{
    if (mastertype == AC_MASTER_HTTP) {
        ...
        curl_easy_setopt(
            ...
            1); // Fixes crashbug for some buggy libcurl versions (Linux)
...
void updatefrommaster(int force)
{
    ...
    if (data.empty()) {
        ...
        if (!strncmp(data.getbuf(), "addserver", 9))
            cllock = false; // the ms could reply other thing... but currently,
        // this is useless
        if (!cllock) {
            execute(data.getbuf());
            if (curserver)
...
COMMANDF(addserver, "sii", (char* n, int* p, int* w)
{
    addserver(n, *p, *w);
});
COMMAND(clearservers, "");
COMMANDF(updatefrommaster, "i", (int* f)
{
    updatefrommaster(*f);
});
...
void writeservercfg()
{
    ...
    loopvrev(servers) {
        ...
        if (servers[i]->msweight)
            f->printf(" %d", servers[i]->msweight);
    }
    f->printf("\n");
    delete f;
}
