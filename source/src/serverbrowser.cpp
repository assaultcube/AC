    ...
    if(menu)
    {
        loopv(servers)
        {
            ...
            if(si.address.host != ENET_HOST_ANY && si.ping != 9999)
            {
                ...
                else if(si.map[0]) s_sprintf(si.full)("%d\t%d/%d\t%s, %s: %s:%d %s", si.ping, si.numplayers, si.maxclients, si.map, modeacronyms ? acronymmodestr(si.mode) : modestr(si.mode), si.name, si.port, si.sdesc);
