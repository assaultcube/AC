                        ...
                        loopi(si->maxclients - 3)
                        {
                            ...
                            if (si->maxclients < 8 || i > 2)
                            {
                                ...
                                if (ts > maxs)
                                    maxs = ts;
                                if (ts > 0x40 && tc > maxc)
                                    maxc = tc; // spent time = 2 ^ ((ts >> 4) - 1) * 30 sec, so 0x50 is 8..15 minutes
                            }
                        }
                        if (maxs < 0x90)
...
void refreshservers(void *menu, bool init)
{
    ...
    if (!init && menu) // && servers.inrange(((gmenu *)menu)->menusel))
    {
        ...
        loopv(servers) if (servers[i]->menuline_from == ((gmenu *)menu)->menusel && servers[i]->menuline_to > servers[i]->menuline_from)
        {
    ...
    if (menu)
    {
        ...
        loopv(servers)
        {
            ...
            if ((!showallservers && si.lastpingmillis <= servermenumillis) || (si.maxclients > MAXCL && searchlan < 2))
            ...
            if (si.address.host != ENET_HOST_ANY && si.ping != 9999)
            {
                if (si.protocol != PROTOCOL_VERSION)
                {
                    if (!showonlygoodservers)
                        formatstring(si.full)("%s:%d [%s]", si.name, si.port, si.protocol < 0 ? "modded version" : (si.protocol < PROTOCOL_VERSION ? "older protocol" : "newer protocol"));
                    else
                        showthisone = false;
                }
                else
                {
                    ...
                    if (serverbrowserhideip < 2)
                        concatformatstring(si.full, "\fs%s%s:%d\fr", serverbrowserhideip == 1 ? "\f4" : "", si.name, si.port);
            ...
            {
                if (!showonlygoodservers)
                    formatstring(si.full)(si.address.host != ENET_HOST_ANY ? "%s:%d [waiting for server response]" : "%s:%d [unknown host]", si.name, si.port);
                else
                    showthisone = false;
            }
        ...
        if (*notfoundmsg)
            menuitemmanual(menu, notfoundmsg, NULL, NULL, NULL);
    }
}
...
void updatefrommaster(int *force)
{
    ...
    if (lastupdate == 0)
        cllock = true;
    if (!*force && lastupdate && totalmillis - lastupdate < masterupdatefrequency * 1000)
        return;
...
COMMANDF(
    addserver, "sii", (char *n, int *p, int *w) { addserver(n, *p, *w); });
