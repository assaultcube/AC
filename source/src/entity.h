...
{ NOTUSED =
      0,  // entity slot not in use in map (usually seen at deleted entities)
...
class playerent : public dynent, public playerstate {
 ...
 public:
  ...
  void damageroll(float damage) {
    damage *= effroll > 0.001f || (effroll > -0.001f && rnd(2))
                  ? 2.0f
                  : -2.0f;  // give player a kick
  ...
  void respawn() {
    ...
    extern int lastmillis;
    weaponchanging =
        lastmillis - weapons[gunselect]->weaponchangetime /
