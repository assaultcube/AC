...
struct persistent_entity // map entity
    ...
    persistent_entity(short x, short y, short z, uchar type, short attr1,
                      uchar attr2, uchar attr3, uchar attr4)
        : x(x)
        , y(y)
        , z(z)
        , attr1(attr1)
...
struct entity : persistent_entity {
    ...
    int lastmillis;
    entity(short x, short y, short z, uchar type, short attr1, uchar attr2,
           uchar attr3, uchar attr4)
...
#define team_opposite(o) \
    (team_isvalid(o) && (o) < TEAM_SPECT ? (o) ^ 1 : TEAM_SPECT)
#define team_base(t) ((t)&1)
#define team_basestring(t) \
    ((t) == 1 ? teamnames[1] : ((t) == 0 ? teamnames[0] : "SPECT"))
...
#define team_tospec(t) \
    ((t) == TEAM_SPECT ? TEAM_SPECT : team_base(t) + TEAM_CLA_SPECT - TEAM_CLA)
...
class playerstate {
    ...
    void resetstats() {
        loopi(NUMGUNS) pstatshots[i] = pstatdamage[i] = 0;
    }
    ...
    virtual void spawnstate(int gamemode) {
        ...
        if (!m_nopistol) {
            ammo[GUN_PISTOL]
                = ammostats[GUN_PISTOL].start
                  - magsize(
                        GUN_PISTOL); // ammostats[GUN_PISTOL].max-magsize(GUN_PISTOL);
    ...
    int dodamage(int damage, int gun) {
        ...
        // Please update ./ac_website/htdocs/docs/introduction.html if this
        // changes.
        int armoursection = 0;
        int ad = damage;
        if (armour > 25)
        ...
        int rd
            = ra
              - (ra * (gi.piercing
                       / 100.0f)); // Who cares about rounding errors anyways?
...
class playerent : public dynent, public playerstate {
    ...
    void respawn() {
        ...
        weaponchanging
            = lastmillis
              - weapons[gunselect]->weaponchangetime
    ...
    bool isspectating() {
        return state == CS_SPECTATE
               || (state == CS_DEAD && spectatemode > SM_NONE);
    }
