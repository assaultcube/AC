...
struct winservice : servercontroller {
                 ...
                 // windows service has stopped
    {
        SERVICE_TABLE_ENTRY dispatchtable[]
            = { {(LPSTR)name, (LPSERVICE_MAIN_FUNCTION)callbacks::main },
                { NULL, NULL } };
        if (StartServiceCtrlDispatcher(dispatchtable))
            exit(EXIT_SUCCESS);
        else
            fatal("an error occurred running the AC server as windows service. "
                  "make sure you start the server from the service control "
                  "manager and not from the command line.");
    }
    ...
    void report(DWORD state, DWORD wait) {
        ...
        status.dwControlsAccepted = SERVICE_START_PENDING == state
                                        || SERVICE_STOP_PENDING == state
                                        ? 0
                                        : SERVICE_ACCEPT_STOP;
    ...
    int WINAPI svcmain() // new server thread's entry point
    {
        ...
        statushandle = RegisterServiceCtrlHandler(
            name, (LPHANDLER_FUNCTION)callbacks::requesthandler);
        if (!statushandle)
            return EXIT_FAILURE;
    ...
    struct callbacks {
        ...
        static int WINAPI main() {
            return svc->svcmain();
        };
        static void WINAPI requesthandler(DWORD request) {
            svc->requesthandler(request);
        };
    };
    ...
    (FACILITY_NULL << 6) & 0x1; // TODO: create event definitions
            LPCTSTR msgs[1] = { msg };
            int r = ReportEvent(eventsrc, error ? EVENTLOG_ERROR_TYPE :
