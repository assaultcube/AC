...
struct weaponmove {
  ...
  weaponmove() : k_rot(0), kick(0), anim(0), basetime(0) {
  ...
  void calcmove(vec aimdir, int lastaction, playerent *p) {
    ...
    if (p->weaponchanging) {
      ...
      float progress = clamp(
          (lastmillis - p->weaponchanging) / (float)weapon::weaponchangetime,
          0.0f, 1.0f);
      ...
      float reloadtime = (float)p->weaponsel->info.reloadtime,
            progress = clamp(
                (lastmillis - p->weaponsel->reloading) / reloadtime, 0.0f,
                clamp(1.0f -
                          (p->lastaction + p->weaponsel->gunwait - lastmillis) /
                              reloadtime,
                      0.5f, 1.0f));
      k_rot = -90 * sinf(progress * PI);
    } else {
      ...
      int timediff = lastmillis - lastaction,
          animtime =
              min(p->weaponsel->gunwait, (int)p->weaponsel->info.attackdelay);
      vec sway = aimdir;
      float progress = 0.0f;
      float k_back = 0.0f;
      ...
      if (nosway)
        ...
        float swayspeed =
            sinf((float)swaymillis / swayspeeddiv) / (swaymovediv / 10.0f);
        float swayupspeed =
            cosf((float)swaymillis / swayupspeeddiv) / (swayupmovediv / 10.0f);

        float plspeed =
            min(1.0f, sqrtf(p->vel.x * p->vel.x + p->vel.y * p->vel.y));
