...
struct weaponmove {
    ...
    void calcmove(vec aimdir, int lastaction, playerent* p) {
        ...
        if (p->weaponchanging) {
            ...
            float progress = clamp((lastmillis - p->weaponchanging)
                                   / (float)weapon::weaponchangetime,
                                   0.0f, 1.0f);
            ...
            float reloadtime = (float)p->weaponsel->info.reloadtime,
                  ...
                  = clamp((lastmillis - p->weaponsel->reloading) / reloadtime,
                          0.0f, clamp(1.0f
                                      - (p->lastaction + p->weaponsel->gunwait
                                         - lastmillis) / reloadtime,
                                      0.5f, 1.0f));
            k_rot = -90 * sinf(progress * M_PI);
        } else {
            anim = ANIM_GUN_IDLE;
            ...
            int timediff = lastmillis - lastaction,
                animtime = min(p->weaponsel->gunwait,
            ...
            if (p->weaponsel->info.mdl_kick_rot
                || p->weaponsel->info.mdl_kick_back) {
            ...
            if (nosway)
                ...
                float swayspeed = sinf((float)swaymillis / swayspeeddiv)
                                  / (swaymovediv / 10.0f);
                float swayupspeed = cosf((float)swaymillis / swayupspeeddiv)
                                    / (swayupmovediv / 10.0f);

                float plspeed = min(
                    1.0f, sqrtf(p->vel.x * p->vel.x + p->vel.y * p->vel.y));
...
void preload_hudguns() {
    loopi(NUMGUNS) {
        ...
        defformatstring(path)("weapons/%s", identexists(widn)
                                                ? getalias(widn)
                                                : guns[i].modelname);
        loadmodel(path);
    }
