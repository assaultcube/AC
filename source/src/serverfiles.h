...
struct servermap // in-memory version of a map file on a server
{
    ...
    void load(void) // load map into memory and extract everything important about it  (assumes struct to be zeroed: can only be called once)
    {
        ...
        if (!staticbuffer)
            staticbuffer = new uchar[FLOORPLANBUFSIZE]; // this buffer gets reused for every map load several times (also: because of this, load() is not thread safe)
        ...
        if (cgzraw)
            tigerhash(cgzhash, cgzraw, cgzlen);
        if (!cgzraw)
        ...
        // extract entity data and header info; compile map statistics; create floorplan
        {
            ...
            else if (f->read(h, sizeof_baseheader) != sizeof_baseheader || (strncmp(h->head, "CUBE", 4) && strncmp(h->head, "ACMP", 4)))
                err = "bad map file";
            if (err)
                goto loadfailed;
...
#define SCALEATTR(x)                                                        \
    ...
    if (ov != e.attr##x)                                                    \
        err = "overflow during conversion of entity attribute";
                    SCALEATTR(1);
                    SCALEATTR(2);
                    SCALEATTR(3);
                    SCALEATTR(4);
#undef SCALEATTR
            ...
            for (int j = 0; j < mapdims.yspan; j++, ss += linegap)
                loopi(mapdims.xspan)
                {
                    if ((ss->type & TAGANYCLIP) > 0 || ss->ceil - ss->floor < 5)
                        ss->type = SOLID; // treat any cube that is not accessible to a player as SOLID
                    if (!SOLID(ss))
                    {
                        if (i < x1)
                            x1 = i;
        ...
        // create compact floorplan
        {
            ...
            loopirev(layoutlen)
            {
                ...
                case CORNER:
                    *layout = zmin;
                    break; // basically: ignore corner areas (otherwise, we'd need a mipmapper to sort it out properly), maybe, this should be fixed at some point...
                default:
                    *layout = ss->floor;
                    break;
                }
    ...
    loadfailed:
        ...
        { // success
            if (readmaplog)
                readmaplog->printf("read map '%s%s': cgz %d bytes, cfg %d bytes (%d gz), version %d, size %d, rev %d, "
...
void updateservermap(int index, bool deletethis)
{
    ...
    while (servermapdropbox)
        readmapsthread_sem->wait(); // wait for the main thread to process the last sent servermap
    servermapdropbox = sm;
}
...
void trymapfiles(const char *fpath, const char *fname, int epoch) //check, if map files were added or altered (detecting alteration requires filesizes to be changed as well, as usual)
{
    ...
    if (updatethis)
        updateservermap(mapfileindex, false);
}
...
void tagmapfile(const char *fpath, const char *fname, int epoch) // tag list entry, if file (+path) is already in it
{
    int mapfileindex = getmapfilenameindex(fname, fpath);
    if (mapfileindex >= 0)
        mapfilenames[mapfileindex].epoch = epoch;
}
...
bool mapisok(mapstats *ms)
{
    ...
    if (Mopen > MAXMAREA)
    {
        logline(ACLOG_INFO, "MAP CHECK FAIL: There is a big open area in this (hint: use more solid walls)");
        return false;
    }
    if (SHhits > MAXHHITS)
    {
