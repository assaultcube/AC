...
struct servermapbuffer // sending of maps between clients
                    ...
                    // distributable), clear buffer otherwise
    {
        ...
        if (cgzdata && (!cfgdata || cfgsize < MAXCFGFILESIZE)) {
            ...
            if (cgzsize + cfgsizegz
                < MAXMAPSENDSIZE) { // map is ok, fill buffer
    ...
    bool sendmap(const char* nmapname, int nmapsize, int ncfgsize,
                 int ncfgsizegz, uchar* ndata) {
        FILE* fp;
        bool written = false;

        if (!nmapname[0] || nmapsize <= 0 || ncfgsizegz < 0
            || nmapsize + ncfgsizegz > MAXMAPSENDSIZE
            || ncfgsize > MAXCFGFILESIZE)
            return false; // malformed: probably modded client
        int cfgsize = ncfgsize;
        if (smode == GMODE_COOPEDIT
            && !strcmp(nmapname, behindpath(smapname))) { // update mapbuffer
                                                          // only in coopedit
                                                          // mode (and on same
                                                          // map)
        ...
        if (fp) {
            ...
            if (fp) {
                ...
                if (uncompress(gzbuf, &rawsize, ndata + nmapsize, ncfgsizegz)
                    == Z_OK && rawsize - ncfgsize == 0)
...
bool mapisok(mapstats* ms) {
    ...
    if (Mopen > MAXMAREA) {
        logline(ACLOG_INFO, "MAP CHECK FAIL: There is a big open area in this "
                            "(hint: use more solid walls)",
                Mheight);
        return false;
    }
    if (SHhits > MAXHHITS) {
        logline(ACLOG_INFO, "MAP CHECK FAIL: Too high height in some parts of "
                            "the map (%d hits)",
                SHhits);
        return false;
    }
    ...
    for (int i = 0; i < ms->hdr.numents; i++) {
        ...
        /*        if (hdensity > 0.0f) { logline(ACLOG_INFO, "ITEM CHECK H %s
           %f", entnames[v], hdensity); }
...
struct servermaprot : serverconfigfile {
    ...
    void read() {
        ...
        while (p < buf + filelen) {
            ...
            if (l) {
                ...
                if (i > 2) {
                    configsets.add(c);
                    logline(ACLOG_VERBOSE, " %s, %s, %d minutes, vote:%d, "
                                           "minplayer:%d, maxplayer:%d, "
                                           "skiplines:%d",
             ...
             bool nochange = false) // load next maprotation set
    {
        ...
        loopi(3 * csl + 1) {
            ...
            if (i >= 3 * csl)
                fatal("maprot unusable"); // not a single map in rotation can be
                                          // found...
        }
        if (!nochange) {
            curcfgset = ccs;
        ...
        loopv(ipranges) {
            ...
            if (ipranges[i].ur <= ipranges[i - 1].ur) {
                if (ipranges[i].lr == ipranges[i - 1].lr
                    ...
                    logline(ACLOG_VERBOSE, " blacklist entry %s got dropped "
...
struct servernickblacklist : serverconfigfile {
    ...
    struct blackline {
        ...
        int line;
        void clear() {
            loopi(MAXNICKFRAGMENTS) frag[i] = -1;
        }
    };
...
struct serverforbiddenlist : serverconfigfile {
    int num;
    char entries[100][2][FORBIDDENSIZE + 1]; // 100 entries and 2 words per
                                             // entry is more than enough
        ...
        loopv(serverinfotexts) {
            ...
            if (si.type == s->type && !strcmp(si.lang, s->lang)) {
                if (servmillis - si.lastcheck
                    > (si.info ? 15 : 45) * 60
                      * 1000) { // re-read existing files after 15 minutes;
            ...
            if (l) {
                ...
                if (s && (fragmsg || gibmsg)) {
                    ...
                    if (strlen(message) > MAXKILLMSGLEN) {
                        logline(ACLOG_INFO, " error in line %i, too long "
                                            "message : string length is %i, "
                                            "max. allowed is %i",
