...
//#define s_sprintfdlv(d,last,fmt) string d; { va_list ap; va_start(ap, last);
//formatstring(d, fmt, ap); va_end(ap); }
...
inline bool
    findpattern(char* s,
                char* d) // returns true if there is more than 80% of similarity
{
    int len, hit = 0;
    if (!d || (len = strlen(d)) < 1)
        return false;
...
struct packetbuf : ucharbuf {
    ...
    packetbuf(ENetPacket* packet)
        : ucharbuf(packet->data, packet->dataLength)
        , packet(packet)
        , growth(0) {
    }
...
inline char* newstringbuf() {
    return newstring(MAXSTRLEN - 1);
}
inline char* newstringbuf(const char* s) {
    return newstring(s, MAXSTRLEN - 1);
}
inline void delstring(const char* s) {
    delete[](char*)s;
}
#define DELSTRING(s)  \
    if (s) {          \
...
extern bool addzip(const char* name, const char* mount = NULL,
                   const char* strip = NULL, bool extract = false,
                   int type = -1);
...
struct sl_semaphore {
    ...
    sl_semaphore(int init, int* errorcount); // init: initial semaphore value;
                                             // errorcount: pointer to error
                                             // counter for semaphore-related
                                             // errors or NULL
    ~sl_semaphore();
