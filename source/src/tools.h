...
public:
    hashtable(int size = 1 << 10)
        : m_size(size), numelems(0), chunks(0), unused(0), table(new chain *[size]())
    {
    }
...
#define enumeratekt(ht, k, e, t, f, b)                                              \
    loopi((ht).capacity()) for (hashtable<k, t>::chain *enumc = (ht).at(i); enumc;) \
    {                                                                               \
...
#define enumeratek(ht, k, e, b)                                                                               \
    loopi((ht).capacity()) for (std::remove_reference<decltype(ht)>::type::chain *enumc = (ht).at(i); enumc;) \
    {                                                                                                         \
        k &e = enumc->key;                                                                                    \
        enumc = enumc->next;                                                                                  \
        b;                                                                                                    \
...
template <>
inline uint64_t endianswap<uint64_t>(uint64_t n) { return endianswap64(n); }
template <class T>
