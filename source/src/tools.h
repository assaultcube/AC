...
#define DEBUG(v)                                                \
  if (DEBUGCOND) {                                              \
    std::cout << behindpath(__FILE__) << ":" << __LINE__ << " " \
              << __FUNCTION__ << "(..) " << v << std::endl;     \
  }
#define DEBUGVAR(v)                                                        \
  if (DEBUGCOND) {                                                         \
    std::cout << behindpath(__FILE__) << ":" << __LINE__ << " "            \
              << __FUNCTION__ << "(..) " << #v << " = " << v << std::endl; \
  }
...
#define rndscale(x) \
  (float((randomMT() & 0xFFFFFF) * double(x) / double(0xFFFFFF)))
...
struct hashtable {
  ...
  void insertchains(chainchunk *emptychunk) {
    loopi(CHUNKSIZE - 1) emptychunk->chains[i].next =
        &emptychunk->chains[i + 1];
...
struct ringbuf {
  ...
  T *remove(
      int *n)  // get n (reduces n, if the data wraps around or isn't available)
  {
...
#define enumeratekt(ht, k, e, t, f, b)                                 \
  loopi((ht).size) for (hashtable<k, t>::chain *enumc = (ht).table[i]; \
                        enumc;) {                                      \
...
struct stopwatch {
  ...
  int elapsed() {
    uint time = SDL_GetTicks() -
                millis;  // subtraction also works in case of timer wraparounds
    return time;
  }
};
#endif
...
#else
#if defined(__GNUC__) && /*!defined(__clang__) && */ !defined(__ICL) && \
    __GNUC__ >= 4 && !defined(WIN32) && !defined(__APPLE__)
...
inline uint64_t endianswap64(uint64_t n) {
  return ((uint64_t)endianswap32((uint)n) << 32) |
         ((uint64_t)endianswap32((uint)(n >> 32)));
}
#endif
#endif
template <class T>
...
template <>
inline uint64_t endianswap<uint64_t>(uint64_t n) {
...
extern const char *timestring(time_t t, bool local = false,
                              const char *fmt = NULL,
                              char *buf = _timestringbuffer);
inline const char *timestring(bool local = false, const char *fmt = NULL,
                              char *buf = _timestringbuffer) {
...
extern stream *opengzfile(const char *filename, const char *mode,
                          stream *file = NULL, int level = Z_BEST_COMPRESSION);
...
extern void filerotate(const char *basename, const char *ext, int keepold,
                       const char *oldformat = NULL);
extern const char *stream_capabilities();
extern void listsubdirs(const char *dir, vector<char *> &subdirs,
...
extern void listfilesrecursive(const char *dir, vector<char *> &files,
                               int level = 0);
extern void listdirsrecursive(const char *dir, vector<char *> &subdirs,
                              int level = 0);
extern void listzipfiles(const char *dir, const char *ext,
                         vector<char *> &files);
...
extern int getlistindex(const char *key, const char *list[],
                        bool acceptnumeric = true, int deflt = -1);
extern void parseupdatelist(hashtable<const char *, int> &ht, char *buf,
