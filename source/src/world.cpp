            ...
            if (!SOLID(r))
            {
                ...
                loopi(4) if (!SOLID(o[i]))
                {
                    ...
                    if (r->type == SEMISOLID)
                    {
                        ...
                        if (o[i]->type == CHF)
                            ch += o[i]->vdelta / 4 + 2; // FIXME: needs to somehow take into account middle vertices on higher mips
                    }
                    if (fh < floor)
            ...
            if (SOLID(r))
            {
                loopi(3)
                {
                    if (o[i]->wtex != o[3]->wtex)
                        goto c; // on an all solid cube, only thing that needs to be equal for a perfect mip is the wall texture
                }
            }
            else
            {
                loopi(3)
                {
                    ...
                    if (*((uint32_t *)q) != *((uint32_t *)o[0])                                                                                 // type, floor, ceil, wtex
                        || iabs(q->b - o[0]->b) > lighterr || q->ftex != o[0]->ftex || q->ctex != o[0]->ctex || iabs(q->r - o[0]->r) > lighterr // perfect mip even if light is not exactly equal
                ...
                {
                    if (o[0]->vdelta - o[1]->vdelta != o[1]->vdelta - SWS(w, x + 2, y, wfactor)->vdelta || o[0]->vdelta - o[2]->vdelta != o[2]->vdelta - SWS(w, x + 2, y + 2, wfactor)->vdelta || o[0]->vdelta - o[3]->vdelta != o[3]->vdelta - SWS(w, x, y + 2, wfactor)->vdelta || o[3]->vdelta - o[2]->vdelta != o[2]->vdelta - SWS(w, x + 2, y + 1, wfactor)->vdelta || o[1]->vdelta - o[2]->vdelta != o[2]->vdelta - SWS(w, x + 1, y + 2, wfactor)->vdelta)
                        goto c;
                }
            }
        mip:
...
COMMANDF(
    toggleclosestentpin, "", () { pinnedclosestent = ents.inrange((pinnedent = pinnedclosestent ? -1 : closestent())); });
...
void pointatent(int *_on)
{
    ...
    if (pointingatents != on)
    {
        ...
        else if (!pinnedclosestent)
        {
            int n = closestent();
            if (n >= 0)
                pinnedent = n, pinnedclosestent = true;
        }
        pointingatents = on;
    }
}
...
COMMANDF(
    getclosestent, "", () { intret(closestent()); });
...
void delent()
{
    ...
    entity &e = ents[n];
    if (t == SOUND)
        deletesoundentity(e); //stop playing sound
    deleted_ents.add(e);
    ents[n].type = NOTUSED;
...
void editentity(char **args, int numargs) // index x y z a1 a2 a3 a4 ...
{
    ...
    if (numargs > 0)
    {
        int n = ATOI(args[0]);
        EDIT("editentity");
        if (!*args[0] || !ents.inrange(n))
            return;
        entity &e = ents[n];
        bool edit = false;
        for (int i = 1; i < numargs; i++)
            if (*args[i])
                edit = true; // only arguments other than empty strings can edit anything - otherwise we're just browsing
        int t = e.type < MAXENTTYPES ? e.type : NOTUSED;
...
COMMANDF(
    addtodoentity, "is", (int *n, const char *desc) { if(ents.inrange(*n)) addtodoentity(*n, desc); });
...
COMMANDF(
    scalelights, "ii", (int *f, int *i) { scalelights(*f, *i); });
...
bool empty_world(int factor, bool force) // main empty world creation routine, if passed factor -1 will enlarge old world by 1, factor -2 will shrink old world by 1
{
    ...
    if (factor < -2)
        return false;
    if (factor == -1 && multiplayer() && sfactor > 9)
        return false;
    if (factor == -2 && !worldbordercheck(ssize / 4 + MINBORD, ssize / 4 + MINBORD, ssize / 4 + MINBORD, ssize / 4 + MINBORD, 0, 0))
    {
    ...
    for (int y = 0; y < ssize; y++)
        for (int x = 0; x < ssize; x++)
        {
            ...
            if (x < 1 || x > ssize - 2 || y < 1 || y > ssize - 2)
                continue; // we need 1 cube safety-space - all outside is invisible anyway
            o[0] = s - ssize;
            o[1] = s + ssize;
            o[2] = s + 1;
            o[3] = s - 1;
            loopi(4) if (!SOLID(o[i]))
            {
                if (SOLID(s) || s->floor > o[i]->floor || (o[i]->type == FHF && (s->type != FHF || s->floor != o[i]->floor)))
                    rs &= ~INVISWTEX; // wall texture visible
                if (!SOLID(s) && (s->ceil < o[i]->ceil || (o[i]->type == CHF && (s->type != CHF || s->ceil != o[i]->ceil))))
...
void mapareacheck(char *what) // "vdelta" | "steepest" | "total" | "pprest" | "pp"(default)
{
    ...
    if (!strcasecmp(what, "vdelta"))
    {
        ...
        cvecprintf(res, "%d.5 %d.5 %d ", ms.steepest % ssize, ms.steepest / ssize, world[ms.steepest % cubicsize].floor);
