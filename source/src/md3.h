...
struct md3 : vertmodel {
    ...
    struct md3part : part {
        bool load(char* path) {
            ...
            if (strncmp(header.id, "IDP3", 4) != 0
                || header.version != 15) // header check
            {
                delete f;
            ...
            loopi(header.nummeshes) {
                ...
                f->read(m.tcverts,
                        2 * sizeof(float) * m.numverts); // read the UV data
    ...
    bool load() {
        ...
        per_idents = false;
        neverpersist = true;
        if (execfile(cfgname) && parts.length()) // configured md3, will call
                                                 // the md3* commands below
        {
            neverpersist = false;
            per_idents = true;
            loadingmd3 = NULL;
...
void md3load(char* model) {
    ...
    if (!mdl.load(path(filename)))
        conoutf("could not load %s", filename); // ignore failure
}
...
void md3link(int* parent, int* child, char* tagname) {
    ...
    if (!loadingmd3->parts.inrange(*parent)
        || !loadingmd3->parts.inrange(*child)) {
