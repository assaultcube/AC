...
static bool findzipdirectory(FILE* f, zipdirectoryheader& hdr)
{
    ...
    if (&buf[len] - src < ZIP_DIRECTORY_SIZE)
        return false;
    ...
    if (hdr.signature != ZIP_DIRECTORY_SIGNATURE
        || hdr.disknumber != hdr.directorydisk
...
static bool readzipdirectory(const char* archname, FILE* f, int entries,
                             int offset, int size, vector<zipfile>& files)
{
    uchar* buf = new uchar[size], *src = buf;
    if (fseek(f, offset, SEEK_SET) < 0 || (int)fread(buf, 1, size, f) != size) {
        delete[] buf;
        return false;
    }
    loopi(entries) {
...
static bool checkprefix(vector<zipfile>& files, const char* prefix,
                        int prefixlen)
{
    loopv(files) {
...
static void mountzip(ziparchive& arch, vector<zipfile>& files,
                     const char* mountdir, const char* stripdir, bool extract)
{
    string packagesdir = "packages/";
    path(packagesdir);
    int striplen = stripdir ? (int)strlen(stripdir) : 0;
    if (arch.type == PCK_MAP)
    ...
    if (!mountdir && !stripdir)
        ...
        if (foundcgz) {
            ...
            while (--cgzdir >= f.name && *cgzdir != PATHDIV)
                ;
            if (cgzdir < f.name
                || checkprefix(files, f.name, cgzdir + 1 - f.name)) {
                if (cgzdir >= f.name) {
                    stripdir = f.name;
    ...
    loopv(files) {
        ...
        formatstring(fname)("%s%s", mdir,
                            striplen && !strncmp(f.name, stripdir, striplen)
                            ? &f.name[striplen]
                            : f.name);
        ...
        if (extract) {
            if (fitspackage(fname, arch.type))
                ...
                conoutf("\f3could not extract \"%s\" : file extension not "
                        "supported",
                        fname);
        }
    }
            ...
            bool extract, int type)
{
    ...
    if (!findzipdirectory(f, h)
        || !readzipdirectory(pname, f, h.entries, h.offset, h.size, files)) {
...
struct zipstream : stream {
    ...
    void stopreading() {
        ...
        if (dbgzip)
            conoutf(info->compressedsize
                    ...
                    info->name, info->compressedsize ? zfile.total_out
                    : reading - info->offset,
                    info->size);
#endif
    ...
    bool seek(long pos, int whence) {
        ...
        if (!info->compressedsize) {
            ...
            arch->owner = NULL;
            if (fseek(arch->data, pos, SEEK_SET) < 0)
                return false;
            arch->owner = this;
            reading = pos;
            ended = false;
...
#ifndef STANDALONE
COMMANDF(addzip, "sss",
         (const char* name, const char* mount, const char* strip)
{
    addzip(name, mount[0] ? mount : NULL, strip[0] ? strip : NULL);
});
COMMANDF(removezip, "s", (const char* name)
{
    removezip(name);
});
#endif
