...
static void rendershadowtiles()
{
    ...
    if (shadowx1 >= shadowx2 || shadowy1 >= shadowy2)
        return;
    ...
    loop(y, SHADOWROWS + 1) {
        ...
        while (mask) {
            ...
            float vx = xstart * tw, vy = y * th, vw = (x - xstart) * tw,
                  vh = (yend - y) * th, vx1 = max(vx, clipx1),
                  vy1 = max(vy, clipy1), vx2 = min(vx + vw, clipx2),
...
void drawstencilshadows()
{
    ...
    if (shadowcasters) {
        ...
        if (dbgtiles)
            memcpy(debugtiles, shadowtiles, sizeof(debugtiles));
