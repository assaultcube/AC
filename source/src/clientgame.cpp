...
//    playerent *h = local ? player1 : updatefollowplayer(0);
    if(/*actor==h && pl!=actor*/ actor == player1) // FIXME
    {
...
void startmap(const char *name, bool reset)   // called just after a map load
{
    ...
    loopv(players) if(players[i]) players[i]->frags = players[i]->flagscore = players[i]->deaths = players[i]->points = players[i]->lifesequence = 0;
    if(editmode) toggleedit(true);
    intermission = false;
    showscores(false);
    minutesremaining = -1;
