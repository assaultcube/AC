...
COMMANDF(
    skin_cla, "i", (int *s) { intret(_setskin(*s, TEAM_CLA)); });
COMMANDF(
    skin_rvsf, "i", (int *s) { intret(_setskin(*s, TEAM_RVSF)); });
COMMANDF(
    skin, "i", (int *s) { intret(_setskin(*s, player1->team)); });
...
COMMAND(benchme, "");
COMMANDF(
    isclient, "i", (int *cn) { intret(getclient(*cn) != NULL ? 1 : 0); });
COMMANDF(
    curmastermode, "", (void) { intret(servstate.mastermode); });
COMMANDF(
    curautoteam, "", (void) { intret(servstate.autoteam); });
COMMAND(curmodeattr, "s");
COMMANDF(
    curmap, "", (void) { result(behindpath(getclientmap())); });
COMMANDF(
    curplayers, "", (void) { intret(players.length() + 1); });
...
void playerinfo(int *cn, const char *attr)
{
    ...
    if (p == player1 || (team_base(p->team) == team_base(player1->team) && m_teammode) || player1->team == TEAM_SPECT || m_coop)
    {
    ...
    if (addr[3] != 0 || player1->clientrole == CR_ADMIN)
        ...
        formatstring(addrstr)("%d.%d.%d.x", addr[0], addr[1], addr[2]); // censored IP
    ATTR_STR(ip, addrstr);
...
COMMANDF(
    curserver, "i", (int *i) { result(currentserver(*i)); });

COMMANDF(
    getmode, "i", (int *acr) { result(modestr(gamemode, *acr != 0)); });
...
COMMANDF(
    resetsleeps, "", (void) { resetsleep(true); });
...
COMMANDF(
    fragmessage, "ss", (const char *gun, const char *message) { setkillmessage(gun, false, message); });
COMMANDF(
    gibmessage, "ss", (const char *gun, const char *message) { setkillmessage(gun, true, message); });
...
COMMANDF(
    burstshots, "si", (char *gun, int *shots) {
        int guni = getlistindex(gun, gunnames, true, -1);
...
void pstat_weap(int *cn)
{
    ...
    if (pl)
        loopi(NUMGUNS) concatformatstring(weapstring, "%s%d %d", strlen(weapstring) ? " " : "", pl->pstatshots[i], pl->pstatdamage[i]);
    result(weapstring);
}
...
void startmap(const char *name, bool reset, bool norespawn) // called just after a map load
{
    ...
    if (good_map() == MAP_IS_BAD)
        conoutf("You cannot play in this map due to quality requisites. Please, report this incident.");
    if (mapstats_hud)
        showmapstats();
    ...
    if (*clientmap)
        conoutf("game mode is \"%s\"%s", modestr(gamemode, modeacronyms > 0), noflags ? " - \f2but there are no flag bases on this map" : "");
...
void flagmsg(int flag, int message, int actor, int flagtime)
{
    ...
    case FM_SCORE:
        ...
        if (firstperson)
        {
            ...
            hudoutf("\f2%s scored for %s", colorname(act), neutral ? teamnames[act->team] : teammate ? "your team" : "the enemy team");
        break;
    case FM_KTFSCORE:
    {
...
char *votestring(int type, const char *arg1, const char *arg2, const char *arg3)
{
    ...
    case SA_GIVEADMIN:
    {
        ...
        if (type == SA_KICK || type == SA_BAN)
        {
            string reason = "";
            if (m_teammode)
                formatstring(reason)("%s (%d tk%s, ping %d)", arg2, p->tks, p->tks == 1 ? "" : "s", p->ping);
            else
...
{
    if (calledvote || !multiplayer(NULL))
        return;
    packetbuf p(MAXTRANS, ENET_PACKET_FLAG_RELIABLE);
    putint(p, SV_CALLVOTE);
    putint(p, SA_MAP);
...
void whois(int *cn)
{
    loopv(players) if (players[i] && players[i]->type == ENT_PLAYER && (*cn == -1 || players[i]->clientnum == *cn))
    {
        ...
        if (ip[3] != 0 || player1->clientrole == CR_ADMIN)
            ...
            conoutf("WHOIS client %d:\tname %s , IP %d.%d.%d.x , %d teamkill%s", p->clientnum, colorname(p), ip[0], ip[1], ip[2], p->tks, p->tks == 1 ? "" : "s"); // censored IP
    }
}
COMMAND(whois, "i");
...
playerent *updatefollowplayer(int shiftdirection)
{
    ...
    loopv(players) if (players[i])
    {
        if (team_group(players[i]->team) == omit_team)
            continue; // if not team SPECT and not watchingdemo and teammode: don't spectate enemy team
        if ((players[i]->state == CS_DEAD || players[i]->isspectating()) && (!stayondeadplayers || players[i] != f))
...
COMMANDF(
    setfollowplayer, "i", (int *cn) { setfollowplayer(*cn); });

// set new spect mode
void spectatemode(int mode)
{
    if ((player1->state != CS_DEAD && player1->state != CS_SPECTATE && !team_isspect(player1->team)) || (!m_teammode && !team_isspect(player1->team) && servstate.mastermode == MM_MATCH))
...
COMMANDF(
    spectatemode, "i", (int *mode) { spectatemode(*mode); });
...
void togglespect() // cycle through all spectating modes
{
    ...
    else
    {
        ...
        if (player1->spectatemode == SM_NONE)
            ...
            mode = SM_FOLLOW1ST + ((player1->spectatemode - SM_FOLLOW1ST + 1) % (SM_OVERVIEW - SM_FOLLOW1ST)); // replace SM_OVERVIEW by SM_NUM to enable overview mode
        spectatemode(mode);
    }
}
...
COMMANDF(
    changefollowplayer, "i", (int *dir) { changefollowplayer(*dir); });
