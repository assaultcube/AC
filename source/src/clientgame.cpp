...
void mode(int* n)
{
    ...
    if (m_mp(*n) || !multiplayer())
        addmsg(SV_GAMEMODE, "ri", *n);
}
COMMAND(mode, "i");
...
const char* highlight(const char* text)
{
    ...
    if (!marker || !strstr(text, player1->name))
        return text;
...
void newname(const char* name)
{
    if (name[0]) {
        ...
        exechook(HOOK_SP, "onNameChange", "%d \"%s\"", player1->clientnum,
                 tmpname);
        copystring(player1->name, tmpname); // 12345678901234//
        if (!player1->name[0])
            copystring(player1->name, "unarmed");
...
void newteam(char* name)
{
    if (*name) {
        ...
        if (nt == player1->team)
            return; // same team
        if (!team_isvalid(nt)) {
            conoutf(_("%c3\"%s\" is not a valid team name (try CLA, RVSF or "
                      "SPECTATOR)"),
                    CC, name);
            return;
        }
...
COMMANDF(skin_cla, "i", (int* s)
{
    intret(_setskin(*s, TEAM_CLA));
});
COMMANDF(skin_rvsf, "i", (int* s)
{
    intret(_setskin(*s, TEAM_RVSF));
});
COMMANDF(skin, "i", (int* s)
{
    intret(_setskin(*s, player1->team));
});
...
COMMAND(benchme, "");
COMMANDF(isclient, "i", (int* cn)
{
    intret(getclient(*cn) != NULL ? 1 : 0);
});
COMMANDF(curmastermode, "", (void)
{
    intret(servstate.mastermode);
});
COMMANDF(curautoteam, "", (void)
{
    intret(servstate.autoteam);
});
COMMAND(curmodeattr, "s");
COMMANDF(curmap, "i", (int* cleaned)
{
    result(*cleaned ? behindpath(getclientmap()) : getclientmap());
});
COMMANDF(curplayers, "", (void)
{
    intret(players.length() + 1);
});
...
void playerinfo(int* cn, const char* attr)
{
    ...
    if (!p) {
        if (!m_botmode && multiplayer(false)) // bot clientnums are still
            // glitchy, causing this message
            // to sometimes appear in
            // offline/singleplayer when it
...
void
spawnstate(playerent* d) // reset player state not persistent accross spawns
{
    d->respawn();
...
playerent* newplayerent() // create a new blank player
{
    ...
    setskin(d, rnd(6));
    weapon::equipplayer(d); // flowtron : avoid overwriting
    // d->spawnstate(gamemode) stuff from the following
    // line (this used to be called afterwards)
    spawnstate(d);
    return d;
}
...
botent* newbotent() // create a new blank player
{
    ...
    while (lukin) {
        ...
        loopv(players) if (!used && players[i]) if (players[i]->clientnum
                    == nextcn) used = true;
        if (!used)
            lukin = false;
        else
    ...
    if (players.length() == getclientnum())
        players.add(NULL);
    d->clientnum = nextcn;
    players.add(d);
    return d;
...
{
    if (intermission || spawnpermission > SP_OK_NUM)
        return;
    if (m_arena) {
        ...
        int secs = 5;
        showhudtimer(secs, player1->respawnoffset, "READY!",
                     lastspawnattempt >= arenaintermission
                     && lastmillis < lastspawnattempt + 100);
    }
}
...
float nearestenemy(vec place, int team)
{
    ...
    loopv(players) {
        ...
        if (!other || isteam(team, other->team))
            continue;
...
void findplayerstart(playerent* d, bool mapcenter, int arenaspawn)
{
    ...
    if (!mapcenter) {
        ...
        if (m_arena && arenaspawn >= 0) {
            ...
            loopi(r) {
                // 2013jun28:lucas: SKB suggested to use FFA spawns only in FFA
                // modes, which seems reasonable.
                spawncycle = /*m_ktf && */ numspawn[2] > 4
                                           ? findentity(PLAYERSTART, spawncycle + 1, 100)
                                           : findentity(PLAYERSTART, spawncycle + 1);
                if (spawncycle < 0)
                    continue;
                float dist
                    = nearestenemy(vec(ents[spawncycle].x, ents[spawncycle].y,
                                       ents[spawncycle].z),
                                   d->team);
...
inline const char* spawn_message()
{
    if (spawnpermission == SP_WRONGMAP) {
        // Don't use "/n" within these messages. If you do, the words won't
        ...
        return "3Type /getmap or send a map and vote for it to start co-op "
               "edit.";
    else if (multiplayer(false))
        return "4Awaiting permission to spawn. \f2DON'T PANIC!";
    else
        return ""; // theres no waiting for permission in sp
    // Despite its many glaring (and occasionally fatal) inaccuracies,
    // AssaultCube itself has outsold the
    // Encyclopedia Galactica because it is slightly cheaper, and because it has
...
bool tryrespawn()
{
    if (m_mp(gamemode) && multiplayer(false) && bad_map()) {
        hudoutf("This map is not supported in multiplayer. Read the docs about "
...
void setkillmessage(const char* gun, bool gib, const char* message)
{
    ...
    if (guni < 0) {
        ...
        if (!message || !*message) {
            ...
            copystring(killmessages[gib ? 1 : 0][guni], message,
                       sizeof(killmessages[gib ? 1 : 0][guni]));
        }
    }
}
...
void dodamage(int damage, playerent* pl, playerent* actor, int gun, bool gib,
              bool local)
{
    if (pl->state != CS_ALIVE || intermission)
        return;
    pl->respawnoffset = pl->lastpain = lastmillis;
    // could the author of the FIXME below please elaborate what's to fix?!
    // (ft:2011mar28)
    // I suppose someone wanted to play the hitsound for player1 or spectated
    // player (lucas:2011may22)
    ...
    if (!h)
        h = player1;
    exechook(HOOK_SP, "onHit", "%d %d %d %d %d", actor->clientnum,
             pl->clientnum, damage, gun, gib ? 1 : 0);

    if (actor == h && pl != actor) {
        if ((hitsound == 1 || (hitsound && h != player1))
            && lasthit != lastmillis)
            audiomgr.playsound(S_HITSOUND, SP_HIGH);
        lasthit = lastmillis;
    }
...
void dokill(playerent* pl, playerent* act, bool gib, int gun)
{
    ...
    exechook(HOOK_SP, "onKill", "%d %d %d %d", act->clientnum, pl->clientnum,
             gun, gib ? 1 : 0);
...
void pstat_weap(int* cn)
{
    ...
    if (pl)
        loopi(NUMGUNS) concatformatstring(
            weapstring, "%s%d %d", strlen(weapstring) ? " " : "",
...
void timeupdate(int milliscur, int millismax)
{
    ...
    if (lastdisplay >= 0 && lastdisplay <= 1000)
        return; // avoid double-output
    ...
    if (!minutesremaining) {
        ...
        extern int clockdisplay; // only output to console if no hud-clock is
        // being shown
        int sec = 60
                  - ((gametimecurrent + (lastmillis - lastgametimeupdate))
                     / 1000) % 60;
...
void showmapstats()
{
    ...
    if (Hhits)
        conoutf("  Height check is: %d", Hhits);
    if (MA)
        conoutf("  The max area is: %d (of %d)", MA, Ma);
    if (m_flags && F2F < 1000)
        conoutf("  Flag-to-flag distance is: %d", (int)fSqrt(F2F));
    if (item_fail)
        conoutf("  There are one or more items too close to each other in this "
                "map");
}
              ...
              bool norespawn) // called just after a map load
{
    ...
    if (good_map() == MAP_IS_BAD)
        conoutf(_("You cannot play in this map due to quality requisites. "
    ...
    intermission = false;
    showscores(false);
    closemenu("Download demo"); // close and reset demo list, because it may be
    // no longer accurate, since a new game has
    // started
    ...
    if (showmodedescriptions && (multiplayer(false) || m_botmode)) {
        loopv(gmdescs) if (gmdescs[i].mode == gamemode) {
            // conoutf(_("%c1%s"), CC, gmdescs[i].desc); // 3rd useless call to
            // translation - these should be translated inside the
            // cube-script-definition
    ...
    if (mapstartonce && mapstartonce[0]) {
        addsleep(0, mapstartonce); // do this as a sleep to make sure map
        // changes don't recurse inside a welcome
        // packet
        // BTW: in v1.0.4 sleep 1 was required to make it work on initial
        // mapload [flowtron:2010jun25]
        alias("mapstartonce", "");
    }
...
void flagmsg(int flag, int message, int actor, int flagtime)
{
    ...
    if (actor != getclientnum() && !act && message != FM_RESET)
        return;
    ...
    const char* teamstr = m_ktf ? "the" : neutral ? ownerstr
                          : own ? "your" : "the enemy";
    const char* flagteam = (m_ktf && !neutral)
    ...
    case FM_PICKUP:
        ...
        if (firstperson) {
            ...
            audiomgr.musicsuggest(M_FLAGGRAB, m_ctf ? 90 * 1000 : 900 * 1000,
                                  true);
            musicplaying = flag;
        } else
    ...
    case FM_KTFSCORE: {
        ...
        if (m)
            ...
            hudoutf("\f2%s%s%s kept the flag for %d seconds now", tc, ta, tb,
                    flagtime);
        break;
    }
                 ...
                 const char* arg3)
{
    ...
    case SA_GIVEADMIN: {
        ...
        if (type == SA_KICK || type == SA_BAN) {
            string reason = "";
            if (m_teammode)
                formatstring(reason)("%s (%d tks, ping %d)", arg2, p->tks,
                                     p->ping);
            else
    ...
    case SA_MAP: {
        ...
        if (arg3 && arg3[0]) {
            ...
            if (time > 0 && time != defaultgamelimit(n))
                formatstring(timestr)(" for %d minute%s", time,
                                      time == 1 ? "" : "s");
        }

        if (n >= GMODE_NUM) {
            formatstring(out)(msg, arg1,
                              modestr(n - GMODE_NUM, modeacronyms > 0),
        ...
        case SA_BAN: {
            if (!arg1 || !isdigit(arg1[0]) || !arg2 || strlen(arg2) <= 3
                ...
                if (!multiplayer(false))
                    conoutf(_("%c3%s is not available in singleplayer."), CC,
                            t == SA_BAN ? "Ban" : "Kick");
...
void callvotesuc()
{
    ...
    calledvote = NULL;
    vote(VOTE_YES); // not automatically done by callvote to keep a clear
    // sequence
}
...
{
    if (calledvote || !multiplayer(false))
        return;
    packetbuf p(MAXTRANS, ENET_PACKET_FLAG_RELIABLE);
    putint(p, SV_CALLVOTE);
    putint(p, SA_MAP);
...
{
    if (calledvote && calledvote->owner == p)
        calledvote->owner = NULL;
    if (curvote && curvote->owner == p)
        curvote->owner = NULL;
}
    ...
    loopv(players) if (players[i] && players[i]->type == ENT_PLAYER
        ...
        if (ip[3] != 0 || player1->clientrole == CR_ADMIN)
            conoutf("WHOIS client %d:\n\f5name\t%s\n\f5IP\t%d.%d.%d.%d",
                    p->clientnum, colorname(p), ip[0], ip[1], ip[2],
                    ip[3]); // full IP
        else
            conoutf("WHOIS client %d:\n\f5name\t%s\n\f5IP\t%d.%d.%d.x",
                    p->clientnum, colorname(p), ip[0], ip[1],
                    ip[2]); // censored IP
    }
}
COMMAND(whois, "i");
...
void refreshsopmenu(void* menu, bool init)
{
    ...
    loopv(players) if (players[i]) {
        ...
        formatstring(m.cmd)(
            "%s %d%s",
            menu == kickmenu ? "kick"
            : (menu == banmenu ? "ban" : (menu == forceteammenu
                                          ? "forceteam"
                                          : "giveadmin")),
            i, (menu == kickmenu || menu == banmenu)
...
playerent* updatefollowplayer(int shiftdirection)
{
    ...
    loopv(players) if (players[i]) {
        if (player1->team != TEAM_SPECT && !watchingdemo && m_teammode
            && team_base(players[i]->team) != team_base(player1->team))
            continue;
        ...
        if (player1->spectatemode == SM_NONE)
            ...
            mode = SM_FOLLOW1ST
                   + ((player1->spectatemode - SM_FOLLOW1ST + 1)
                      % (SM_OVERVIEW - SM_FOLLOW1ST)); // replace SM_OVERVIEW by
        // SM_NUM to enable
        // overview mode
        spectatemode(mode);
    }
...
COMMAND(spectate, "");
COMMANDF(spectatemode, "i", (int* mode)
{
    spectatemode(*mode);
});
COMMAND(togglespect, "");
COMMANDF(changefollowplayer, "i", (int* dir)
{
    changefollowplayer(*dir);
});
COMMANDF(setfollowplayer, "i", (int* cn)
{
    setfollowplayer(*cn);
});
