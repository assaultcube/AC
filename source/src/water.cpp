...
FVARF(waterlevel, -10000, -10000, 127, {
  ...
  waterlevel = float(hdr.waterlevel) /
               WATERLEVELSCALING;  // waterlevel and hdr.waterlevel need to be
                                   // kept in sync
});
...
void getwatercolour() {
  defformatstring(res)("%d %d %d %d", hdr.watercolor[0], hdr.watercolor[1],
                       hdr.watercolor[2], hdr.watercolor[3]);
  result(res);
}
...
VERTWC(vertwc,
       ...
       {
         varray::attrib<uchar>(
             hdr.watercolor[0], hdr.watercolor[1], hdr.watercolor[2],
...
int renderwater(float hf, GLuint reflecttex, GLuint refracttex) {
  ...
  effective_waveheight =
      mapoverride_limitwaveheight && !ignoreoverride_limitwaveheight
          ? 0.1f
          : waveheight;
...
void calcwaterscissor() {
  ...
  loopi(4) {
    vec4 &p = v[i];
    mvpmatrix.transform(
        vec(i & 1 ? wx2 : wx1, i & 2 ? wy2 : wy1, waterlevel - 0.3f), p);
    if (p.z >= 0) {
