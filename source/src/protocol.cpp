...
ucharbuf *getgzbuf(ucharbuf &p) // fetch a gzipped buffer; needs to be freed properly later
{
    ...
    if (sizegz == 0)
        sizegz = size; // uncompressed was smaller
        ...
        if (trans)
        {
            ...
            if (filename)
            {
                ...
                if (insidepointybrackets || c == '>')
                    continue; // filter anything between '<' and '>' as this may contain commands for texture loading
                const char *org = "[]{}\\",
                           *asc = "()()/",
                           *a = strchr(org, c);
                if (a)
