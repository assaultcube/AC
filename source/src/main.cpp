...
void writepngchunk(stream *f, const char *type, const void *data = NULL, uint len = 0)
{
    ...
    if (data)
        loopi(len) enet_crc32_inc(&crc, ((const uchar *)data)[i]);
    f->putbig<uint>(~crc);
}
...
#define FLUSHZ                                 \
    ...
    writepngchunk(f, "tEXt", "\123o\165rc\145\0\101ss\141"
...
void mapscreenshot(const char *imagepath, bool mapshot, int fileformat, float scale, int height, int quality)
{
    ...
    delete[] tmpdst;
    if (!mapshot && imagepath)
        audiomgr.playsound(S_CAMERA);
...
void screenshot(const char *filename)
{
    ...
    if (filename && filename[0])
        ...
        formatstring(buf)("screenshots/%s_%s_%s%s", timestring(), behindpath(getclientmap()), modestr(gamemode, true), getscrext());
    else
...
VIRTW = scr_w * VIRTH / scr_h;
    ...
    COMMANDF(
        screenres, "ii", (int *w, int *h) { screenres(*w, *h); });
        ...
        if (modes && modes != (SDL_Rect **)-1)
        {
            for (int i = 0; modes[i]; i++)
            {
                if (lw != modes[i]->w && lh != modes[i]->h)
                    concatformatstring(res, "%s%d %d", *res ? " " : "", (lw = modes[i]->w), (lh = modes[i]->h));
            }
        }
        result(res);
    }
...
#define AUTOSTARTPATH "config" PATHDIVS "autostart" PATHDIVS
    ...
    void connectprotocol(char *protocolstring) // assaultcube://example.org[:28763][/][?[port=28763][&password=secret]]
    {
        ...
        if (*u.path)
            clientlogf(" ignoring path part of url (%s)", u.path);
...
#define initlog(s) clientlogf("init: " s)
            ...
            if (*stream_capabilities())
                clientlogf("info: %s", stream_capabilities());
...
#define SDLVERSIONSTRING STRINGIFY(SDL_MAJOR_VERSION) \
