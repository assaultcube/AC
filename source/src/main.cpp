...
void bmp_screenshot(const char* imagepath, bool mapshot = false)
{
    ...
    if (mapshot) {
        ...
        if (screenshotscale != 1.0f) {
            ...
            glReadPixels(0, 0, iw, ih, GL_RGB, GL_UNSIGNED_BYTE,
                         buf); // screen->w//screen->h
...
void writepngchunk(stream* f, const char* type, uchar* data = NULL,
                   uint len = 0)
{
    f->putbig<uint>(len);
    f->write(type, 4);
...
void png_screenshot(const char* imagepath, bool mapshot = false)
{
    ...
    if (save_png(filename, image) < 0)
        conoutf("\f3Error saving png file");
...
void screenres(int w, int h)
{
    ...
    COMMANDF(screenres, "ii", (int* w, int* h)
    {
        screenres(*w, *h);
    });
    ...
    void setupscreen(int& usedcolorbits, int& useddepthbits, int& usedfsaa)
    {
        ...
        if (colorbits && modes)
            hasbpp
                = SDL_VideoModeOK(modes != (SDL_Rect**)-1 ? modes[0]->w : scr_w,
        ...
        loopi(sizeof(configs) / sizeof(configs[0])) {
            ...
            if (!depthbits && config & 1)
                continue;
            if (!stencilbits && config & 2)
                continue;
            if (fsaa <= 0 && config & 4)
                continue;
            if (depthbits)
                SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE,
                                    config & 1 ? depthbits : 16);
            if (stencilbits) {
        ...
        if (!screen)
            ...
            if (!hasbpp)
                conoutf("%d bit color buffer not supported - disabling",
                        colorbits);
...
#endif
            if (event.motion.x == screen->w / 2
                && event.motion.y == screen->h / 2)
        ...
        while (events.length() || SDL_PollEvent(&event)) {
            ...
            case SDL_KEYUP:
                ...
                if (event.key.keysym.sym <= SDLK_5
                    && event.key.keysym.sym >= SDLK_1 && senst) {
...
#define AUTOSTARTPATH "config" PATHDIVS "autostart" PATHDIVS
        ...
        if (!files) { // first run: fetch file names and sort them
            ...
            for (int i = files->length() - 1; i > 0; i--) {
                if (!strcmp((*files)[i], (*files)[i - 1]))
                    delstring(files->remove(i)); // delete doubles
            }
        }
        loopv(*files) {
            if (*prefix && strncmp((*files)[i], prefix, strlen(prefix)))
                continue;
    ...
    void createconfigtemplates(const char* templatezip) // create customisable
    // config files in
    // homedir - only if
    // missing
    {
        if (addzip(templatezip)) {
            vector<char*> files;
            listzipfiles("", "cfg", files); // only look for config files in the
            // root dir of the zip file
            loopv(files) {
                defformatstring(fname)(
                    "config%c%s.cfg", PATHDIV,
                    behindpath(files[i])); // (the "behindpath()" should not be
                // necessary, but we want to be
                // careful: the zip may be weird)
                stream* f = openrawfile(fname, "rb");
                if (f)
                    delete f; // config already exists
                else {
                    int flen;
                    char* buf = loadfile(behindpath(fname),
                                         &flen); // fetch file content from zip
                    if (buf) {
                        f = openfile(fname, "wb");
                        if (f) {
                            f->write(buf, flen);
                            delete f;
                            conoutf("created %s from template %s", fname,
                                    templatezip);
                        }
                        delete[] buf;
                    }
    ...
    void connectprotocol(char* protocolstring, string& servername,
                         int& serverport, string& password,
                         bool& direct_connect)
    {
        ...
        do {
            ...
            if (!strncmp(c, "port=", 5)) {
                ...
                if (len)
                    copystring(password, p, min(len + 1, MAXSTRLEN));
            } else
                break;
        } while (*c && *c == '&' && *c != '/');
    }
    ...
    static char* parsecommandline(const char* src, vector<char*>& args)
    {
        ...
        for (;;) {
            ...
            if (!*src)
                break;
            args.add(dst);
            for (bool quoted = false; *src && (quoted || !isspace(*src));
                 src++) {
                if (*src != '"')
                    *dst++ = *src;
    ...
    VARP(compatibilitymode, 0, 1,
         1); // FIXME : find a better place to put this ?
...
// atexit((void (__cdecl *)(void))_CrtDumpMemoryLeaks);
        ...
        __try {
            ...
            char* initdemo = NULL;
            bool direct_connect
                = false; // to connect via assaultcube:// browser switch
...
#define initlog(s) clientlogf("init: " s)
            ...
            for (int i = 1; i < argc; i++) {
                clientlogf("parsing commandline argument %d: \"%s\"", i,
                           argv[i]);
                ...
                if (!scl.checkarg(argv[i])) {
                    ...
                    if (argv[i][0] == '-')
                        ...
                        connectprotocol(argv[i], servername, serverport,
                                        password, direct_connect);
                    } else
                        conoutf("\f3unknown commandline argument: %c",
                                argv[i][0]);
                }
            }
            initclientlog();
...
#define SDLVERSIONSTRING                        \
    STRINGIFY(SDL_MAJOR_VERSION) "." STRINGIFY( \
        SDL_MINOR_VERSION) "." STRINGIFY(SDL_PATCHLEVEL)
            ...
            if (!dedicated)
                initclient();
            // FIXME the server executed in this way does not catch the SIGTERM
            // or ^C
            ...
            notexture = noworldtexture
                        = textureload("packages/misc/notexture.jpg");
            if (!notexture)
                fatal("could not find core textures (hint: run AssaultCube "
            ...
            scoremenu = addmenu("score", "columns", false, renderscores, NULL,
                                false, true);
            servmenu
                = addmenu("server", NULL, true, refreshservers, serverskey);
            searchmenu
                = addmenu("search", NULL, true, refreshservers, serverskey);
            ...
            applymenu = addmenu("apply", "apply changes now?", true,
                                refreshapplymenu);
            ...
            if (xmn)
                conoutf("loaded %d xmaps", xmn);
            per_idents = true;

            static char resdata[]
                = { 112, 97,  99,  107, 97,  103, 101, 115, 47,  116, 101, 120,
                    116, 117, 114, 101, 115, 47,  107, 117, 114, 116, 47,  107,
                    108, 105, 116, 101, 50,  46,  106, 112, 103, 0
                  };
            ...
            if (compatibilitymode) {
                per_idents = false;
                exec("config/compatibility.cfg"); // exec after saved.cfg to get
                // "compatibilitymode", but
                // before user scripts..
                per_idents = true;
            }
            ...
            execute("addallfavcatmenus"); // exec here, to add all categories
            ...
            if (autodownload && (gamestarts % 100 == 1))
                sortpckservers();
            ...
            int lastflush = 0;
#endif
            if (scl.logident[0]) { // "-Nxxx" sets the number of client log
                // lines to xxx (after init)
            ...
            for (;;) {
                ...
                if (lastmillis)
                    updateworld(curtime, lastmillis);
                ...
                if (needsautoscreenshot) {
                    showscores(true);
                    // draw again after swapping buffers, to make sure menu is
                    // captured
                    // in the screenshot regardless of which frame buffer is
                    // current
                    if (!minimized) {
                        gl_drawframe(
                            screen->w, screen->h,
