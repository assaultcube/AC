...
void processevent(client* c, explodeevent& e) {
    ...
    for (int i = 1; i < c->events.length() && c->events[i].type == GE_HIT;
        ...
        if (target->type == ST_EMPTY || target->state.state != CS_ALIVE
            || h.lifesequence != target->state.lifesequence || h.dist < 0
            || h.dist > EXPDAMRAD)
            continue;
...
void processevent(client* c, shotevent& e) {
    ...
    if (!gs.isalive(gamemillis) || e.gun < GUN_KNIFE || e.gun >= NUMGUNS
        || wait < gs.gunwait[e.gun] || gs.mag[e.gun] <= 0)
        return;
    if (e.gun != GUN_KNIFE)
        gs.mag[e.gun]--;
    loopi(NUMGUNS) if (gs.gunwait[i]) gs.gunwait[i]
        = max(gs.gunwait[i] - (e.millis - gs.lastshot), 0);
    ...
    gs.shotdamage
        += guns[e.gun].damage
           * (e.gun == GUN_SHOTGUN ? SGMAXDMGLOC
                                   : 1); // 2011jan17:ft: so accuracy stays
                                         // correct, since SNIPER:headshot also
                                         // "exceeds expectations" we use
                                         // SGMAXDMGLOC instead of SGMAXDMGABS!
    switch (e.gun) {
    case GUN_GRENADE:
        ...
        for (int i = 1; i < c->events.length() && c->events[i].type == GE_HIT;
            ...
            if (e.gun == GUN_SHOTGUN) {
                ...
                int numhits_c = (h.info & 0x0000FF00) >> 8,
                    numhits_m = (h.info & 0x00FF0000) >> 16,
                ...
                if (rays < 1 || tothits_c > SGRAYS || tothits_m > SGRAYS
                    || tothits_o > SGRAYS || bonusdist > SGDMGBONUS)
                    continue;

                gib = rays == maxrays;
                float fdamage = (SGDMGTOTAL / (21 * 100.0f))
                                * (numhits_o * SGCOdmg / 10.0f
                                   + numhits_m * SGCMdmg / 10.0f
                                   + numhits_c * SGCCdmg / 10.0f);
                fdamage += (float)bonusdist;
                damage = (int)ceil(fdamage);
#ifdef ACAC
                if (!sg_engine(target, c, numhits_c, numhits_m, numhits_o,
                               bonusdist))
                    continue;
#endif
            } else {
...
void processevent(client* c, reloadevent& e) {
    clientstate& gs = c->state;
    if (!gs.isalive(gamemillis) || e.gun < GUN_KNIFE || e.gun >= NUMGUNS
        || !reloadable_gun(e.gun) || gs.ammo[e.gun] <= 0)
        return;

    bool akimbo = e.gun == GUN_PISTOL && gs.akimbomillis > e.millis;
    int mag = (akimbo ? 2 : 1) * magsize(e.gun),
        numbullets = min(gs.ammo[e.gun], mag - gs.mag[e.gun]);
    if (numbullets <= 0)
        return;
