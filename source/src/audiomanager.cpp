...
void audiomanager::musicfadeout(int id)
{
    if (nosound || !gamemusic)
        return;
    if (!gamemusic->playing() || !musics.inrange(id))
        return;
...
{
    if (gamemusic)
        gamemusic->setvolume(musicvol > 0 ? musicvol / 255.0f : 0);
}
...
void audiomanager::setlistenervol(int vol)
{
    ...
    if (!nosound)
    {
        ...
        if (vol && !oldvol && !mapsounds.length())
...
{
    if (e.type != SOUND || !mapsounds.inrange(e.attr1))
        return false;
...
{
    if (!nosound)
        sourcescheduler::instance().init(num);
};
    ...
    if (!footsteps || (local && !localfootsteps) || !inrange || p->state != CS_ALIVE || lastmillis - p->lastpain < 300 || (!p->onfloor && p->timeinair > 50) || (!p->move && !p->strafe) || p->inwater)
    {
        ...
        loopi(sizeof(locs) / sizeof(locs[0]))
        {
            ...
            if (l->playmillis + minplaytime > totalmillis)
                continue; // tolerate short interruptions by enforcing a minimal playtime
            l->drop();
        }
    }
    else
...
{
    if (p)
        _playsound(n, physentreference(p), priority);
}
void audiomanager::playsound(int n, entity *e, int priority)
{
    if (e)
        _playsound(n, entityreference(e), priority);
}
void audiomanager::playsound(int n, const vec *v, int priority)
{
    if (v)
        _playsound(n, staticreference(*v), priority);
}
...
void audiomanager::playsoundname(char *s, const vec *loc, int vol)
{
    ...
    if (id < 0)
        id = addsound(s, vol, 0, false, gamesounds, true, 0);
...
void audiomanager::updateaudio()
{
    ...
    if (pitchfx && currentpitch != lowpitch)
    {
        ...
        if (tinnitusloc)
            tinnitusloc->pitch(1.9f); // super high pitched tinnitus
    }
...
COMMANDF(
    registersound, "siii", (char *name, int *vol, int *loop, int *audibleradius) {
...
COMMANDF(
    mapsound, "si", (char *name, int *maxuses) {
        ...
        else if (!strncmp(stripped, mapsoundbasepath, mapsoundbasepath_n))
        {
            ...
            if (!strncmp(name, mapsoundfinalpath, mapsoundfinalpath_n))
                name += mapsoundfinalpath_n; // base path for mapsoundlines is "packages/audio/ambience"
...
COMMANDF(
    registermusic, "s", (char *name) {
        audiomgr.registermusic(name);
    });

COMMANDF(
    musicpreload, "i", (int *id) {
        audiomgr.musicpreload(*id);
    });

COMMANDF(
    music, "sis", (char *name, int *millis, char *cmd) {
...
void editmapsoundslot(int *n, char *name, char *maxuses) // edit slot parameters != ""
{
    ...
    if (mapconfigdata.mapsoundlines.inrange(*n))
    {
        ...
        { // change attributes
            if (*maxuses)
                msl.maxuses = strtol(maxuses, NULL, 0);
            if (*name)
                copystring(msl.name, strncmp(name, mapsoundfinalpath, mapsoundfinalpath_n) ? name : name + mapsoundfinalpath_n);
            reloadmapsoundconfig();
            mapsoundchanged = 1;
            unsavededits++;
            hdr.flags |= MHF_AUTOMAPCONFIG; // requires automapcfg
...
void sortmapsoundslots(char **args, int numargs)
{
    ...
    if (unknownarg || mapconfigdata.mapsoundlines.length() < 3)
        return;
