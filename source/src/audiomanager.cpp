...
void audiomanager::musicsuggest(
    int id, int millis, bool rndofs) // play bg music if nothing else is playing
{
    ...
    if (gamemusic->open(name)) {
        ...
        if (rndofs)
            gamemusic->seek(millis ? (double)rnd(millis) / 2.0f
                            : (double)lastmillis);
...
{
    loopv(sounds) {
        if (!strcmp(sounds[i].buf->name, name)
            && (!vol || sounds[i].vol == vol))
            return i;
    }
    return -1;
}

int audiomanager::addsound(char* name, int vol, int maxuses, bool loop,
...
void audiomanager::updateplayerfootsteps(playerent* p)
{
    ...
    if (!footsteps || (local && !localfootsteps) || !inrange
        ...
        int grounddist;
        if (((int)p->o.x) >= 0 && ((int)p->o.y) >= 0 && ((int)p->o.x) < ssize
            && ((int)p->o.y) < ssize) { // sam's fix to the sound crash
            grounddist = hdr.waterlevel - S((int)p->o.x, (int)p->o.y)->floor;
        } else {
            grounddist = 0;
        }
        //        int grounddist = hdr.waterlevel-S((int)p->o.x,
        //        (int)p->o.y)->floor;
        ...
        int stepsound;
        if (p->crouching)
            stepsound = water ? S_WATERFOOTSTEPSCROUCH
                        : S_FOOTSTEPSCROUCH; // crouch
        else
...
void voicecom(char* sound, char* text)
{
    ...
    if (!last || lastmillis - last > 2000) {
        ...
        if (s < 0 || s < S_AFFIRMATIVE || s >= S_NULL)
            return;
        if (voicecomsounds > 0)
...
{
    intret(audiomgr.addsound(name, *vol, -1, *loop != 0, gamesounds, true,
                             *audibleradius));
});
...
COMMANDF(music, "sis", (char* name, int* millis,
                        char* cmd)
{
