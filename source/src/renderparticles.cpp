...
void setupexplosion()
{
    ...
    if (mtexplosion && maxtmus >= 2) {
        ...
        if (!expmodtex[0])
            expmodtex[0] = createexpmodtex(64, 0);
        if (!expmodtex[1])
            expmodtex[1] = createexpmodtex(64, 0.25f);
        lastexpmodtex = 0;
    }
}
...
static struct parttype {
    ...
    {
        PT_PART, 0.0f, 0.0f, 1.0f, 20, 0,
        0.08f
    }, // blue:   edit mode closest ent
    ...
    { PT_FLASH, 1.0f, 1.0f, 1.0f, 0, 6, 0.7f }, // muzzle flash
    {
        ...
        0.08f
    }, // white: edit mode ent type : light
    {
        PT_PART, 0.0f, 1.0f, 0.0f, 20, 0,
        0.08f
    }, // green: edit mode ent type : spawn
    {
        PT_PART, 1.0f, 0.0f, 0.0f, 20, 0,
        0.08f
    }, // red: edit mode ent type : ammo
    {
        PT_PART, 1.0f, 1.0f, 0.0f, 20, 0,
        0.08f
    }, // yellow: edit mode ent type : pickup
    {
        PT_PART, 1.0f, 0.0f, 1.0f, 20, 0,
        0.08f
    }, // magenta: edit mode ent type : model, sound
    {
        PT_PART, 1.0f, 0.5f, 0.2f, 20, 0,
        0.08f
    }, // orange: edit mode ent type : "carrot"
    {
        PT_PART, 0.5f, 0.5f, 0.5f, 20, 0,
        0.08f
    }, // grey: edit mode ent type : ladder, (pl)clip
    {
        PT_PART, 0.0f, 1.0f, 1.0f, 20, 0,
        0.08f
    }, // turquoise: edit mode ent type : CTF-flag
    ...
    for (int i = MAXPARTYPES - 1; i >= 0; i--)
        if (typemask & (1 << parttypes[i].type) && parlist[i]) {
            ...
            for (particle* p, **pp = &parlist[i]; (p = *pp);) {
                ...
                case PT_FIREBALL: {
                    sz = 1.0f
                         + (pt.sz - 1.0f) * min(p->fade, lastmillis - p->millis)
                         / p->fade;
                    glPushMatrix();
                    ...
                    glRotatef(inside ? camera1->yaw - 180 : atan2(oc.y, oc.x)
                              / RAD - 90,
                              0, 0, 1);
                    glRotatef((inside ? camera1->pitch
                               : asin(oc.z / oc.magnitude()) / RAD) - 90,
                              1, 0, 0);
                ...
                case PT_DECAL: {
                    ...
                    glColor4f(s->r / 127.5f, s->g / 127.5f, s->b / 127.5,
                              max(0.0f, min((p->millis + p->fade - lastmillis)
                                            / 1000.0f,
                                            0.7f)));
                ...
                case PT_BULLETHOLE: {
                    float blend = max(
                                      0.0f, min((p->millis + p->fade - lastmillis) / 1000.0f,
                                                1.0f));
                ...
                case PT_BLOOD: {
                    ...
                    ;
                    float tx = 0.5f * (n & 1), ty = 0.5f * ((n >> 1) & 1),
                          tsz = 0.5f;
                    static const int tc[4][2]
                    = { { 0, 1 }, { 1, 1 }, { 1, 0 }, { 0, 0 } };
...
{
    if (parttypes[type].type == PT_BLOOD && !blood)
        return;
    loopi(num) {
                   ...
                   bool noisy, int type)
{
    ...
    if (noisy && bulletbouncesound && bulletbouncesoundrad && d != player1
        ...
        audiomgr.playsound(
            o.z < hdr.waterlevel ? S_BULLETWATERHIT : S_BULLETHIT, &o, SP_LOW);
    }
    return true;
}
...
bool addscorchmark(vec& o, float radius)
{
    ...
    if (s->type != SPACE || o.z - s->floor > radius)
        return false;
...
{
    if (pl == player1 || !shotlinettl || !shotline)
        return;
    bool fx = shotlinettl > 0
              && (player1->isspectating()
                  || !multiplayer(
                      false)); // allow fx only in spect mode and locally
    if (rnd(3) && !fx)
        return; // show all shotlines when fx enabled
