    ...
    for (int i = MAXPARTYPES - 1; i >= 0; i--)
        if (typemask & (1 << parttypes[i].type) && parlist[i])
        {
            ...
            for (particle *p, **pp = &parlist[i]; (p = *pp);)
            {
                ...
                case PT_PART:
                    glTexCoord2i(0, 1);
                    glVertex3f(p->o.x + (-camright.x + camup.x) * sz, p->o.y + (-camright.y + camup.y) * sz, p->o.z + (-camright.z + camup.z) * sz);
                    glTexCoord2i(1, 1);
                    glVertex3f(p->o.x + (camright.x + camup.x) * sz, p->o.y + (camright.y + camup.y) * sz, p->o.z + (camright.z + camup.z) * sz);
                    glTexCoord2i(1, 0);
                    glVertex3f(p->o.x + (camright.x - camup.x) * sz, p->o.y + (camright.y - camup.y) * sz, p->o.z + (camright.z - camup.z) * sz);
                ...
                case PT_DECAL:
                {
                    ...
                    glVertex3f(p->o.x + (-dx.x + dy.x) * pt.sz, p->o.y + (-dx.y + dy.y) * pt.sz, p->o.z + (-dx.z + dy.z) * pt.sz);
                ...
                {
                    if (pt.gr)
                        p->o.z -= ((lastmillis - p->millis) / 3.0f) * time / (pt.gr * 10000);
