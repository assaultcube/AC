              ...
              bool flicker = false) // done in realtime, needs to be fast
{
    ...
    int stepl = (int)(l / (float)steps); // incorrect: light will fade quicker
    ...
    if (maxtmus) {
            ...
            // because most lights are white
        {
            ...
            if (flicker) {
                int dimness
                    = rnd((((255 << PRECBITS) - (light.attr2 * fadescale))
                           >> (PRECBITS + 4)) + 1);
                x += stepx * dimness;
                y += stepy * dimness;
            }
...
void postlightarea(const block& a) // median filter, smooths out random noise in
// light and makes it more mipable
{
...
void calclight()
{
    ...
    if (!r && !g) {
        ...
        r = g = b = max(max(r, g), b); // the old (white) light code, here for
    // the few people with old video cards
    // that don't support overbright
    sqr* s = S(0, 0);
    loopirev(cubicsize) {
        s->r = r;
        s->g = g;
    ...
    block bb = { mapdims.x1 - 1, mapdims.y1 - 1, mapdims.xspan + 2,
                 mapdims.yspan + 2
               };
    postlightarea(bb);
    setvar("fullbright", 0);
...
void dodynlights()
{
    ...
    loopv(dlights) {
        dlight& d = dlights[i];
        persistent_entity l((int)d.o.x, (int)d.o.y, (int)d.o.z, LIGHT, d.reach,
                            d.r, d.g, d.b);
...
void blockpaste(const block& b, int bx, int by,
                bool light) // slow version, editmode only
{
