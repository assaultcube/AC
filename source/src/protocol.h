...
#define PROTOCOL_VERSION \
    1201 // bump when protocol changes (use negative numbers for mods!)
#define DEMO_VERSION 2 // bump when demo format changes
#define DEMO_MAGIC "ASSAULTCUBE_DEMO"
#define DEMO_MINTIME \
    10000 // don't keep demo recordings with less than 10 seconds
#define MAXMAPSENDSIZE 65536
#define MAXCFGFILESIZE 65536

extern bool modprotocol;
#define CUR_PROTOCOL_VERSION \
    (modprotocol ? -PROTOCOL_VERSION : PROTOCOL_VERSION)
...
enum { MM_OPEN = 0, MM_PRIVATE, MM_MATCH, MM_NUM };
enum { MM_MASK = 0x03 };
...
#define m_autospawn                                                    \
    (gamemode == 0 || gamemode == 2 || gamemode == 6 || gamemode == 13 \
     || gamemode == 16)
#define m_teammode                                                           \
    (gamemode == 0 || gamemode == 4 || gamemode == 5 || gamemode == 7        \
     || gamemode == 11 || gamemode == 13 || gamemode == 14 || gamemode == 16 \
     || gamemode == 17 || gamemode == 20 || gamemode == 21)
#define m_tarena (m_arena && m_teammode)
#define m_botmode                                     \
    (gamemode == 7 || gamemode == 8 || gamemode == 12 \
     || (gamemode >= 18 && gamemode <= 21))
#define m_valid(mode) (((mode) >= 0 && (mode) < GMODE_NUM) || (mode) == -1)
#define m_mp(mode)                                              \
    (m_valid(mode) && (mode) >= 0 && (mode) != 7 && (mode) != 8 \
     && (mode) != 12 && ((mode) < 18 || (mode) > 21))
...
#define FTXT__CHAT \
    (FTXT_NOWHITE | FTXT_ALLOWBLANKS | FTXT_NOCOLOR | FTXT_TABTOBLANK)
