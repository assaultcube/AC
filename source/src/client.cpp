...
bool multiplayer(const char *op)
{
    ...
    if (curpeer && op)
        conoutf("%s%s%s not available in multiplayer", *op ? "\"" : "", *op ? op : "operation", *op ? "\"" : "");
    return curpeer != NULL;
}
...
{
    if (*minutes)
        shiftgametime(gametimemaximum - ATOI(minutes) * 60000);
}
...
void rewinddemo(char *seconds)
{
    int gamemillis = gametimecurrent + (lastmillis - lastgametimeupdate);
    if (*seconds)
        shiftgametime(gamemillis - ATOI(seconds) * 1000);
}
...
COMMANDF(
    watchingdemo, "", () { intret(watchingdemo); });

COMMANDF(
    systime, "", () { result(numtime()); });
COMMANDF(
    timestamp, "", () { result(timestring(true, "%Y %m %d %H %M %S")); });
COMMANDF(
    datestring, "", () { result(timestring(true, "%c")); });
