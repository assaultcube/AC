...
void c2sinfo(playerent* d) // send update to the server
{
    ...
    if (d->state == CS_ALIVE || d->state == CS_EDITING) {
        packetbuf q(100);
        int cn = d->clientnum,
            x = (int)(d->o.x * DMF), // quantize coordinates to 1/16th of a
            // cube, between 1 and 3 bytes
            ...
            f = (d->strafe & 3) | ((d->move & 3) << 2)
                | (((int)d->onfloor) << 4) | (((int)d->onladder) << 5)
                | ((d->lifesequence & 1) << 6) | (((int)d->crouching) << 7),
    ...
    if (sendmapidenttoserver || messages.length()
        ...
        int i = 0;
        while (i < messages.length()) // send messages collected during the
            // previous frames
        {
        ...
        case ENET_EVENT_TYPE_DISCONNECT: {
            ...
            if (event.peer == connpeer) {
                ...
                if (!discmillis || event.data)
                    conoutf(
                        _("%c3server network error, disconnecting (%s) ..."),
...
authkey* findauthkey(const char* desc)
{
    loopv(authkeys) if (!strcmp(authkeys[i]->desc, desc)
                        && !strcmp(authkeys[i]->name,
                                   player1->name)) return authkeys[i];
...
bool _hasauthkey(const char* name, const char* desc)
{
    ...
    loopvrev(authkeys) if (!strcmp(authkeys[i]->desc, desc)
                           && !strcmp(authkeys[i]->name, name)) return true;
    return false;
}
...
void saveauthkeys()
{
    if (authkeys.length()) {
        ...
        conoutf("you need to use 'addauthkey USER KEY DESC' first; one DESC "
                "'public', one DESC empty(=private)");
}
...
COMMANDF(hasauthkey, "ss",
         (char* name, char* desc)
{
    intret(_hasauthkey(name, desc) ? 1 : 0);
});
COMMAND(genauthkey, "s");
COMMAND(saveauthkeys, "");
COMMANDF(auth, "s", (char* desc)
{
    intret(tryauth(desc));
});
...
bool securemapcheck(const char* map, bool msg)
{
    ...
    loopv(securemaps) if (!strcmp(securemaps[i], map)) {
        if (msg) {
            conoutf(_("%c3Map %c4%s%c3 is secured. This means you CAN'T send, "
                      "receive or overwrite it."),
                    CC, CC, map, CC);
            if (connected) {
                conoutf(_("%c3If you get this error often, you (or the server) "
                          "may be running an outdated game."),
                        CC);
                conoutf(_("%c3You can check for updates at "
                          "%c1http://assault.cubers.net/download.html"),
                        CC, CC);
            }
        }
        return true;
...
void resetpckservers()
{
    ...
    conoutf("%c3Restart AssaultCube to take effect after resetting the list of "
...
int pingpckservers(void* data)
{
    ...
    loopv(serverstoping) {
        ...
        curl_easy_setopt(cu, CURLOPT_NOBODY, 1); // don't download response body
        // (as its size may vary a lot
        // from one server to another)
        curl_easy_setopt(cu, CURLOPT_CONNECTTIMEOUT,
                         10); // the timeout should be large here
        ...
        ping -= namelookup; // ignore DNS lookup time as it should be performed
        // only once
    ...
    SDL_mutexP(pingpcksrvlock);
    loopv(serverstoping)
    loopvj(pckservers) if (!strcmp(serverstoping[i].addr,
                                   pckservers[j]->addr))* pckservers[j]
            = serverstoping[i];
...
int progress_callback_dlpackage(void* clientp, double dltotal, double dlnow,
                                double ultotal, double ulnow)
{
    package* pck = (package*)clientp;
    loadingscreen(_("downloading package %d out of %d...\n%s %.0f/%.0f KB "
                    "(%.1f%%)\n(ESC to cancel)"),
                  pck->number + 1, pck->number + pendingpackages.numelems,
                  pck->name, dlnow / double(1024.0), dltotal / double(1024.0),
...
int processdownload(package* pck)
{
    ...
    if (!pck->pending) {
        ...
        case PCK_AUDIO: {
            ...
            // with textures/sounds, the image/audio file itself is sent. Just
            // need to copy it from the temporary file
...
double dlpackage(package* pck)
{
    ...
    string req, pckname = "";
    sprintf(req, "%s/%s%s", pck->source->addr,
    ...
    curl_easy_setopt(pck->curl, CURLOPT_CONNECTTIMEOUT,
                     10); // generous timeout for Bukz ;)
    ...
    if (result == CURLE_OPERATION_TIMEDOUT
        ...
        // update all packages
        enumerate(pendingpackages, package*, pack, { pack->source = source; });
...
void writepcksourcecfg()
{
    if (pckservers.length()) {
        ...
        if (!f)
            return;
        f->printf("// list of package source servers (only add servers you "
                  "trust!)\n\n");
