...
COMMANDF(editmeta, "d", (bool on)
{
    editmetakeydown = on ? 1 : 0;
});
...
void toggleedit(bool force)
{
    ...
    player1->state = player1->state == CS_SPECTATE
                     ? CS_SPECTATE
                     : (editing ? CS_EDITING : CS_ALIVE);
    if (editing && player1->onladder)
...
char* editinfo()
{
    ...
    if (e >= 0) {
        entity& c = ents[e];
        formatstring(info)("closest entity = %s (%d, %d, %d, %d), ",
    ...
    sqr* s;
    if (!OUTBORD(cx, cy) && (s = S(cx, cy)) && !editfocusdetails(s) && !SOLID(s)
        && s->tag)
...
#define EDITSEL                                  \
    if (noteditmode("EDITSEL") || noselection()) \
    return
#define EDITSELMP                                                   \
    if (noteditmode("EDITSELMP") || noselection() || multiplayer()) \
    return
#define EDITMP                                  \
    if (noteditmode("EDITMP") || multiplayer()) \
    return
...
void makesel(bool isnew)
{
    ...
    if (selset())
        rtex = *S((sels.last()).x, (sels.last()).y);
}
...
#define SEL_ATTR(attr)                                    \
    {                                                     \
        string buf = "";                                  \
        loopv(sels) {                                     \
            concatformatstring(buf, "%d ", sels[i].attr); \
        }                                                 \
        result(buf);                                      \
    }
COMMANDF(selx, "", (void)
...
COMMANDF(showtagclipfocus, "d", (bool on)
{
    showtagclipfocus = on;
});
...
void cursorupdate() // called every frame from hud
{
    ...
    editaxis = editmode ? (fabs(camera1->pitch) > 65
                           ? 13
                           : (cyaw < 45 || cyaw > 135 ? 12 : 11))
               : 0;
    ...
    if (sparkletime)
        lastsparkle = lastmillis
                      - (lastmillis % 20); // clip adding sparklies at 50 fps
...
void pruneundos(int maxremain) // bound memory
{
    ...
    loopvrev(undos) {
        ...
        if (u > maxremain)
            freeblockp(undos.remove(i));
    }
    loopvrev(redos) {
        r += redos[i]->xs * redos[i]->ys;
        if (r > maxremain)
            freeblockp(redos.remove(i));
    }
}
...
void restoreeditundo(ucharbuf& q)
{
    ...
    while (!q.overread() && (type = getuint(q))) {
        int bx = getuint(q), by = getuint(q), bxs = getuint(q),
            bys = getuint(q);
        short pp[5];
        ...
        if (worldiodebug)
            ...
            case 10:
            case 20:
                clientlogf("  got %sdo x %d, y %d, xs %d, ys %d, remaining %d, "
                           "overread %d",
                           type == 10 ? "un" : "re", b->x, b->y, b->xs, b->ys,
                           q.remaining(), int(q.overread()));
                break;
            }
#endif
    }
...
#ifdef _DEBUG
    if (worldiodebug)
        clientlogf("    compressing redo/undo x %d, y %d, xs %d, ys %d, "
                   "compressed length %d, cubes %d",
    ...
    loopvrev(undos) {
        ...
        if (worldiodebug)
            clientlogf(
                ...
                undos[i]->x, undos[i]->y, undos[i]->xs, undos[i]->ys,
                tmp.length());
#endif
    }
    loopvrev(redos) {
        ...
        if (worldiodebug)
            clientlogf(
                ...
                redos[i]->x, redos[i]->y, redos[i]->xs, redos[i]->ys,
                tmp.length());
#endif
    }
    putuint(buf, 0);
...
void countwalls(int* type)
{
    ...
    loopselsxy(if (s->type == *type) counter++) intret(counter);
}
    ...
    if (!isdown)
        for (int i = sels.length() - 2; i >= 0; i--) {
            ...
            if (a.x == b.x && a.y == b.y && a.xs == b.xs
                && a.ys == b.ys) { // making a selection twice will deselect
                // both of it
                sels.drop();
                sels.remove(i);
                break;
...
void editheight(int* flr, int* amount)
{
    ...
    loopv(sels) {
        ...
        addmsg(SV_EDITH, "ri6", sels[i].x, sels[i].y, sels[i].xs, sels[i].ys,
               isfloor, *amount);
    }
}
...
void edittex(int type, int dir)
{
    ...
    loopv(sels) {
        ...
        addmsg(SV_EDITT, "ri6", sels[i].x, sels[i].y, sels[i].xs, sels[i].ys,
               type, t);
    }
    unsavededits++;
}
...
void edittype(int type)
{
    ...
    loopv(sels) {
        block& sel = sels[i];
        if (type == CORNER
            && (sel.xs != sel.ys || (sel.xs != 1 && sel.xs != 2 && sel.xs != 4
                                     && sel.xs != 8 && sel.xs != 16)
...
void setvdelta(int delta)
{
    ...
    loopv(sels) {
        ...
        addmsg(SV_EDITD, "ri5", sels[i].x, sels[i].y, sels[i].xs, sels[i].ys,
               delta);
    }
}
...
{
    EDITSEL;
    loopv(sels) newentity(-1, sels[i].x, sels[i].y, (int)camera1->o.z, what,
                          *a1, *a2, *a3, *a4);
}
...
void movemap(int xo, int yo, int zo) // move whole map
{
    ...
    if (!worldbordercheck(MINBORD + max(-xo, 0), MINBORD + max(xo, 0),
                          MINBORD + max(-yo, 0), MINBORD + max(yo, 0),
                          max(zo, 0), max(-zo, 0))) {
    ...
    if (xo || yo) {
        block b
        = { max(-xo, 0), max(-yo, 0), ssize - abs(xo), ssize - abs(yo) },
        *cp = blockcopy(b);
        cp->x = max(xo, 0);
        cp->y = max(yo, 0);
        blockpaste(*cp);
...
void selfliprotate(block& sel, int dir)
{
    ...
    int x1 = sel.x, x2 = sel.x + sel.xs - 1, y1 = sel.y,
        y2 = sel.y + sel.ys - 1, x, y;
    switch (dir) {
...
COMMAND(arch, "i");
COMMANDF(slope, "ii", (int* xd, int* yd)
{
    slope(*xd, *yd);
});
COMMANDF(vdelta, "i", (int* d)
{
    setvdelta(*d);
});
...
COMMAND(copy, "");
COMMAND(paste, "");
COMMANDF(edittex, "ii", (int* type, int* dir)
{
    edittex(*type, *dir);
});
COMMAND(newent, "siiii");
COMMANDF(perlin, "iii", (int* sc, int* se, int* ps)
{
    perlin(*sc, *se, *ps);
});
COMMANDF(movemap, "iii", (int* x, int* y, int* z)
{
    movemap(*x, *y, *z);
});
COMMANDF(selectionrotate, "i", (int* d)
{
    selectionrotate(*d);
});
COMMAND(selectionflip, "s");
COMMAND(countwalls, "i");
COMMANDF(settex, "ii", (int* texture, int* type)
{
    settex(*texture, *type);
});

void transformclipentities() // transforms all clip entities to tag clips, if
// they are big enough (so, that no player could be
// above or below them)
{
    EDITMP;
    int total = 0, thisrun;
    do {
        ...
        loopv(ents) {
            entity& e = ents[i];
            if ((e.type == CLIP || e.type == PLCLIP) && e.attr2 && e.attr3
                && e.attr4) {
                int allowedspace = e.type == CLIP ? 1 : 4;
                int clipmask = e.type == CLIP ? TAGCLIP : TAGPLCLIP;
                int x1 = e.x - e.attr2, x2 = e.x + e.attr2 - 1,
                    y1 = e.y - e.attr3, y2 = e.y + e.attr3 - 1;
                ...
                for (int xx = x1; xx <= x2; xx++)
                    for (int yy = y1; yy <= y2; yy++) {
                        ...
                        int vdeltamax = 0;
                        loopj(4) if (s[j]->vdelta > vdeltamax) vdeltamax
                                = s[j]->vdelta;
                        int floor
                            = s[0]->floor
                              - (s[0]->type == FHF ? (vdeltamax + 3) / 4 : 0),
                              ceil
                              = s[0]->ceil
                                - (s[0]->type == CHF ? (vdeltamax + 3) / 4 : 0);
                        if ((z1 - floor > allowedspace
                             || ceil - z2 > allowedspace)
                            && !(s[0]->tag & (TAGCLIP | clipmask)))
                            bigenough = false;
                    }
                if (bigenough) {
                    for (int xx = x1; xx <= x2; xx++)
