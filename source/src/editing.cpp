...
COMMANDF(
    editmeta, "d", (bool on) { editmetakeydown = on ? 1 : 0; });
...
COMMANDF(
    select, "iiii", (int *x, int *y, int *xs, int *ys) { resetselections(); addselection(*x, *y, *xs, *ys, -999); });
COMMANDF(
    addselection, "iiii", (int *x, int *y, int *xs, int *ys) { addselection(*x, *y, *xs, *ys, -999); });
...
void makesel(bool isnew)
{
    if (isnew || sels.length() == 0)
        addselection(min(lastx, cx), min(lasty, cy), iabs(lastx - cx) + 1, iabs(lasty - cy) + 1, max(lasth, ch));
    else
    {
...
COMMANDF(
    selx, "", (void) { SEL_ATTR(x); });
COMMANDF(
    sely, "", (void) { SEL_ATTR(y); });
COMMANDF(
    selxs, "", (void) { SEL_ATTR(xs); });
COMMANDF(
    selys, "", (void) { SEL_ATTR(ys); });
#undef SEL_ATTR
...
COMMANDF(
    showtagclipfocus, "d", (bool on) { showtagclipfocus = on; });
...
void gotoposition(char *x, char *y, char *z, char *yaw, char *pitch)
{
    ...
    if (*z)
        player1->o.z = atof(z);
    if (*yaw)
        player1->yaw = (atoi(yaw) % 360 + 360) % 360;
    if (*pitch)
        player1->pitch = atoi(pitch) % 90;
    player1->resetinterp();
...
{
    if (bxs < 0 || bys < 0 || bxs > ssize || bys > ssize || bxs * bys > MAXNETBLOCKSQR || OUTBORD(bx, by) || OUTBORD(bx + bxs - 1, by + bys - 1))
        return;
...
void gotoundolevel(char *lev)
{
    if (*lev && isdigit(*lev))
    {
        EDITMP("undolevel");
        int n = ATOI(lev);
        while (undolevel > n && undos.length() > 0)
            editundo();
    }
    intret(undolevel);
}
...
#ifdef _DEBUG
    if (worldiodebug)
        clientlogf("    compressing redo/undo x %d, y %d, xs %d, ys %d, compressed length %d, cubes %d", s->x, s->y, s->xs, s->ys, tmp.length(), s->xs * s->ys);
#endif
    return t.length();
}
...
#ifdef _DEBUG
        if (worldiodebug)
            clientlogf("  written undo x %d, y %d, xs %d, ys %d, compressed length %d", undos[i]->x, undos[i]->y, undos[i]->xs, undos[i]->ys, tmp.length());
#endif
    }
    loopvrev(redos)
    {
...
#ifdef _DEBUG
        if (worldiodebug)
            clientlogf("  written redo x %d, y %d, xs %d, ys %d, compressed length %d", redos[i]->x, redos[i]->y, redos[i]->xs, redos[i]->ys, tmp.length());
#endif
    }
    putuint(buf, 0);
    return numundo;
...
void paste()
{
    ...
    loopv(sels)
    {
        ...
        loopvj(copybuffers)
        {
            ...
            if (mp)
                netblockpaste(*copyblock, sel.x, sel.y, true);
        }
    }
}
COMMAND(paste, "");
...
void archvertex(int *span, int *vert, char *delta)
{
    ...
    if (*delta)
        archverts[*span][*vert] = ATOI(delta);
}
...
void selectionwalk(char *action, char *beginsel, char *endsel)
{
    ...
    for (int i = 0; *localvars[i]; i++)
        push(localvars[i], "");
    int maxsels = sels.length();
    loopv(sels)
    {
        if (i >= maxsels)
            break;           // don't allow the script to add selections
...
#undef GETA
            if (memcmp(s, &so, sizeof(sqr)))
            {
                ...
                {
                    if (!didwarn)
                        conoutf("\f3selectionwalk: selected area %dx%d too big for multiplayer editing, changes ignored!", sel.xs, sel.ys);
                    didwarn = true;
                }
                else
                {
        ...
        loopv(ents)
        {
            ...
            if ((e.type == CLIP || e.type == PLCLIP) && e.attr2 / ENTSCALE5 && e.attr3 / ENTSCALE5 && e.attr4 / ENTSCALE5)
            {
                ...
                for (int xx = x1o; xx <= x2o; xx++)
                    for (int yy = y1o; yy <= y2o; yy++) // loop over outer rectangle to check, if a clip has the required height
                    {
                        ...
                        if (!inner && !alreadytagged)
                            nodelete = true; // fractional part of the clip would not be covered: entity can not be deleted
                    }
                if (bigenough)
                {
