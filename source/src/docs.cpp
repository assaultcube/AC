...
void docwriteref(int allidents, const char *ref, const char *schemalocation, const char *transformation)
{
    ...
    static const int bases[] = { "i", "s", "i", "i"F, ARG_1EST };
    string name;
    loopv(inames)
    {
        ...
        f->printf("\t\t\t\t\t%s\n", desc);
        if(id->narg != "" && id->narg != "d" && id->narg != ARG_IVAL && id->narg != ARG_FVAL && id->narg != ARG_SVAL)
        {
            f->printf("\t\t\t\t\t<arguments>\n");
            if(id->narg == "c" || id->narg == "c"W || id->narg == ARG_VARI) f->printf("\t\t\t\t\t\t<variableArgument token=\"...\" description=\"TODO\"/>\n");
            else
            {
                int base = -1;
