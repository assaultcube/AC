...
docidents; // manage globally instead of a section tree to ensure uniqueness
...
void adddocref(char* name, char* ident, char* url,
               char* article) // FIXME... someone, please :P
{
    if (!lastident || !name)
        return;
...
void adddocexample(char* code, char* explanation)
{
    ...
    e.explanation = explanation && strlen(explanation) ? newstring(explanation)
                    : NULL;
}
...
void docfind(char* search)
{
    enumerate(docidents, docident, i, {
        ...
        if ((r = cvecstr(srch, search, &rline))) {
            ...
            string match;
            copystring(match,
            r - srch[rline] > matchchars / 2 ? r - matchchars / 2
            : srch[rline],
            matchchars / 2);
                 ...
                 const char* transformation)
{
    ...
    loopv(inames) {
        ...
        switch (id->type) {
        case ID_VAR:
            f->printf("\t\t\t\t\t<value %s description=\"TODO\" "
                      "minValue=\"%i\" maxValue=\"%i\" defaultValue=\"%i\" "
                      "%s/>\n",
        ...
        case ID_FVAR:
            f->printf("\t\t\t\t\t<value %s description=\"TODO\" "
                      "minValue=\"%s\" maxValue=\"%s\" defaultValue=\"%s\" "
                      "%s/>\n",
...
void renderdoc(int x, int y, int doch)
{
    ...
    for (size_t i = 0; i < clen; i++) // search first matching cmd doc by
        // stripping arguments of exp from right
        // to left
    {
        char* end = c + clen - i;
        if (!*end || *end == ' ') {
            ...
            if (ident) {
                ...
                if (ident->arguments.length() > 0) { // args
                    ...
                    if (args) {
                        ...
                        if (arg >= 0) // multipart idents need a fixed argument
                            // offset
                        {
                            char* c = cmd;
                            if (!nc)
                        ...
                        if (arg >= ident->arguments.length()
                            && ident->arguments.last().vararg)
                    ...
                    loopvj(ident->arguments) {
                        ...
                        formatstring(doclines.add(newstringbuf()))(
                            "~\f%d%-8s%s %s%s%s", j == arg ? 4 : 5, a->token,
                            a->desc, a->values ? "(" : "",
                            a->values ? a->values : "", a->values ? ")" : "");
                    }
                ...
                if (ident->examples.length()) { // examples
                    doclines.add(ident->examples.length() == 1 ? "Example:"
                                 : "Examples:");
                ...
                if (ident->keys.length()) { // default keys
                    doclines.add(ident->keys.length() == 1 ? "Default key:"
                                 : "Default keys:");
                    loopvj(ident->keys) {
                ...
                if (ident->references.length()) { // references
                    struct category {
                        string label;
                        string refs;
                    } categories[] = { { "related identifiers", "" },
                        { "web resources", "" },
                        { "wiki articles", "" },
                        { "other", "" }
                    };
                    loopvj(ident->references) {
                        ...
                        char* ref
                            = r.ident
                              ? categories[0].refs
                              : (r.url ? categories[1].refs
                                 : (r.article ? categories[2].refs
                                    : categories[3].refs));
