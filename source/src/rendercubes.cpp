...
#define RENDERSTRIPS(strips, type)                                            \
    if (strips.first.length()) {                                              \
        if (hasMDA && !ati_mda_bug)                                           \
            glMultiDrawArrays_(type, strips.first.getbuf(),                   \
                               strips.count.getbuf(), strips.first.length()); \
        else                                                                  \
            loopv(strips.first)                                               \
...
// the
// leaves of all these functions, and are part of the cpu bottleneck on really
// slow
...
#define TEXTURESCALE     \
    (float(texturescale) \
     * ((uniformtexres && t->scale > 1.0f) ? 1.0f : t->scale))
                   ...
                   sqr* l1, sqr* l2, bool flip, int dir) // wall quads
{
    ...
    if (first) {
        ...
        int lighterr = lighterror * 2;
        if ((!hf && !ohf)
            && ((abs(ol1r - l2->r) < lighterr && abs(ol1g - l2->g) < lighterr
                 && abs(ol1b - l2->b) < lighterr)
...
void rendershadow_tris(int x, int y, bool topleft, sqr* h1, sqr* h2)
{
    if (topleft) {
        if (h1)
            rendershadow_tri(h1, x + 1, y + 1, x, y + 1, x, y);
        if (h2)
            rendershadow_tri(h2, x, y, x + 1, y, x + 1, y + 1);
    } else {
        if (h1)
            rendershadow_tri(h1, x, y, x + 1, y, x, y + 1);
        if (h2)
            rendershadow_tri(h2, x + 1, y, x + 1, y + 1, x, y + 1);
    }
}
...
#define df(x) s->floor - (x->vdelta / 4.0f)
    ...
    for (shadowvertex* v = shadowverts.getbuf(),
         *end = &v[shadowverts.length()];
         v < end; v++) {
        float vx = v->x, vy = v->y;
