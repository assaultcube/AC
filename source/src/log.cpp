...
#include "cube.h"
    ...
    bool initlogging(const char *identity, int facility_, int consolethres, int filethres, int syslogthres, bool logtimestamp)
    {
        ...
        if (filethres >= 0)
            filethreshold = min(filethres, (int)ACLOG_NUM);
        if (syslogthres >= 0)
            syslogthreshold = min(syslogthres, (int)ACLOG_NUM);
        facility &= 7;
...
#else
            if ((logsock = enet_socket_create(ENET_SOCKET_TYPE_DATAGRAM)) == ENET_SOCKET_NULL || enet_address_set_host(&logdest, "localhost") < 0)
                syslogthreshold = ACLOG_NUM;
#endif
        }
        ...
        if (fp)
            concatformatstring(msg, ", \"%s\"", filepath);
