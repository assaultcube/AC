...
VARP(akimboendaction, 0, 3, 3); // 0: switch to knife, 1: stay with pistol (if
// has ammo), 2: switch to grenade (if
// possible), 3: switch to primary (if has ammo)
// - all fallback to previous one w/o ammo for
// target
...
{
    // 2011jan17:ft: once this basic approach has been agreed upon this function
    // should be optimized
    // 2011may27:ft: indeed
    ...
    loopk(3) {
        ...
        loopi(SGRAYS) {
            int j = k * SGRAYS;
            sgr[j + i].ds
                = k; // 2011jun18 - damage-independent-section-indicator .. not:
            // k==0 ? SGSEGDMG_O : (k==1? SGSEGDMG_M: SGSEGDMG_C);
            vec p(spoke);
            float base = 0.0f;
            int wrange = 50;
            switch (sgr[j + i].ds) {
...
static inline bool intersectbox(const vec& o, const vec& rad, const vec& from,
                                const vec& to,
...
playerent* intersectclosest(const vec& from, const vec& to, playerent* at,
                            float& bestdist, int& hitzone, bool aiming = true)
{
    playerent* best = NULL;
    bestdist = 1e16f;
    int zone;
    ...
    loopv(players) {
        ...
        if (!o || o == at
            || (o->state != CS_ALIVE && (aiming || (o->state != CS_EDITING
                                         && o->state != CS_LAGGED))))
            continue;
...
{
    if (owner == player1)
        exechook(HOOK_SP, "onAttack", "%d", weapon);
}
            ...
            if (hits[h]) {
                ...
                if (SGDMGBONUS) {
                    ...
                    if (d2o <= (SGDMGDISTB / 10.0f) && numhits) {
                        /*
                         float soc = ( SGCCdmg/10.0f * SGDMGTOTAL/100.0f ) + (
                         SGCMdmg/10.0f * SGDMGTOTAL/100.0f ) + ( SGCOdmg/10.0f *
                         SGDMGTOTAL/100.0f );
                         float d2t = SGDMGTOTAL - soc;
                         */
                        dmg4r += SGDMGBONUS; // d2t; //was: "d2t" == if
                        // percentage-points are dangling
                        // and we have hits, we have a
                        // base-damage value
                        withBONUS = true;
                ...
                dmg4r += /*(int)*/ ((SGCOdmg / 10.0f * SGDMGTOTAL / 100.0f)
                                    * numhits_o / 21.0f);
                dmg4r += /*(int)*/ ((SGCMdmg / 10.0f * SGDMGTOTAL / 100.0f)
                                    * numhits_m / 21.0f);
                dmg4r += /*(int)*/ ((SGCCdmg / 10.0f * SGDMGTOTAL / 100.0f)
                                    * numhits_c / 21.0f);
                //              int dmgreal = (SGSEGDMG_O * numhits_o +
                //              SGSEGDMG_M * numhits_m + SGSEGDMG_C *
                //              numhits_c)/3;
                /*
                ...
                //                conoutf("%d [%.3f%s] DMG with %d hits",
                //                lastsgs_dmgt, dmg4r, withBONUS ? "\fs\f3+\fr"
                //                : "", lastsgs_hits);
                ...
                //              if(numhits) hitpush( dmgreal, o, d, from, to,
                //              d->weaponsel->type, dmgreal == SGMAXDMGABS,
                //              dmgreal);
...
void r_accuracy(int h)
{
    ...
    loopi(NUMGUNS) if (i != GUN_CPISTOL && accuracym[i].shots) {
        ...
        if (i == GUN_GRENADE || i == GUN_SHOTGUN) {
            formatstring(line)("\f5%5.1f%s (%.1f/%d) :\f0%s", acc, "%",
                               accuracym[i].hits, (int)accuracym[i].shots,
                               weapstr(i));
        } else {
            formatstring(line)("\f5%5.1f%s (%d/%d) :\f0%s", acc, "%",
                               (int)accuracym[i].hits, (int)accuracym[i].shots,
                               weapstr(i));
        }
...
void weapon::renderhudmodel(int lastaction, int index)
{
    ...
    rendermodel(path, wm.anim | ANIM_DYNALLOC
                | (righthanded == index ? ANIM_MIRROR : 0)
                | (emit ? ANIM_PARTICLE : 0),
                0, -1, wm.pos, p->yaw + 90, p->pitch + wm.k_rot, 40.0f,
...
{
    if (!editmode)
        drawcrosshair(owner, teamwarning ? CROSSHAIR_TEAMMATE
                      : owner->weaponsel->type);
    else
...
bool grenades::attack(vec& targ)
{
    ...
    case GST_THROWING:
        if (attackmillis >= throwwait) { // throw done
            reset();
            if (!mag
                && this == owner->weaponsel) { // switch to primary immediately
...
{
    return shots > 2 ? 70
           : (info.spread + (shots > 0 ? (shots == 1 ? 5 : 10)
                             : 0)); // CHANGED: 2010nov19
    // was: min(info.spread
    // + 10 * shots, 80)
}
...
int sniperrifle::dynspread()
{
    if (scoped) {
        ...
        if (scopetime > SCOPESETTLETIME)
            ...
            return max((info.spread * (SCOPESETTLETIME - scopetime))
                       / SCOPESETTLETIME,
                       1);
    }
    return info.spread;
...
void sniperrifle::renderaimhelp(bool teamwarning)
{
    ...
    if (!editmode) {
        if (scoped || teamwarning)
            drawcrosshair(owner,
                          teamwarning ? CROSSHAIR_TEAMMATE : CROSSHAIR_SCOPE,
                          NULL, 24.0f);
    } else
        drawcrosshair(owner, CROSSHAIR_EDIT);
}
...
void akimbo::onammopicked()
{
    ...
    if (owner == player1) {
        // if(owner->weaponsel->type!=GUN_SNIPER &&
        // owner->weaponsel->type!=GUN_GRENADE) owner->weaponswitch(this);
        if (akimboautoswitch || owner->weaponsel->type == GUN_PISTOL)
            owner->weaponswitch(this); // Give the client full control over
        // akimbo auto-switching // Bukz
        // 2011apr23
