...
void selectweapon(weapon *w)
{
    ...
    if (w->selectable())
    {
        if (player1->attacking && player1->state == CS_ALIVE)
            attack(false);
        int i = w->type;
        // substitute akimbo
...
void quicknadethrow(bool on)
{
    ...
    if (on)
    {
        if (player1->weapons[GUN_GRENADE]->mag > 0)
        {
            ...
            {
                if (!player1->weapons[GUN_GRENADE]->busy())
                    attack(true);
            }
        }
    }
...
COMMANDF(
    currentprimary, "", () { intret(player1->primweap->type); });
COMMANDF(
    prevweapon, "", () { intret(player1->prevweaponsel->type); });
COMMANDF(
    curweapon, "", () { intret(player1->weaponsel->type); });
...
inline bool intersecttriangle(const vec &from, const vec &dir, const vec &v0, const vec &v1, const vec &v2, vec *end, float *_t) // precise but rather expensive, based on Moeller–Trumbore intersection algorithm
{
    ...
    if (t < EPSILON)
        return false;
    if (_t)
        *_t = t; // 0..1 if intersection is between from and to
    if (end)
...
void intersectgeometry(const vec &from, vec &to) // check line for contact with map geometry, shorten if necessary
{
    ...
    if (OUTBORD(x, y) || from.z < -127.0f || from.z > 127.0f)
        return;
                ...
                loopk(2)
                {
                    ...
                    { // cube s is a probable hit, examine further
                        ...
                        if (s->type == CORNER)
                        {
                            ...
                            vec newto;
                            float newdist;
                            if (intersectcorner(from, d, bx, by, bs, !(q & 1), &newto, &newdist) && newdist < distmin && (!h || newto.z < ns->floor || newto.z > ns->ceil))
...
void hit(int damage, playerent *d, playerent *at, const vec &vel, int gun, bool gib, int info)
{
    ...
    else
    {
        ...
        if (d == player1)
        {
            ...
            d->damageroll(damage);
            if (d != at)
                updatedmgindicator(player1, at->o);
...
void shorten(const vec &from, vec &target, float distsquared)
{
    ...
    if (m < 0.07f)
        target.add(vec(0.1f, 0.1f, 0.1f)); // if "from == target" just fake a target to avoid zero-length fx vectors
    else
        ...
        if (attackmillis >= throwwait) // throw done
        {
            if (this == owner->weaponsel)
            {
                if (quicknade || !mag)
                {
                    ...
                    if (quicknade)
                        owner->nextweaponsel = owner->weaponsel = owner->prevweaponsel; // switch to previous weapon immediately
                    else
...
int grenades::modelanim()
{
    ...
    {
        if (lastmillis - owner->lastaction >= throwwait)
            state = GST_NONE;
...
void shoot(playerent *p, vec &targ)
{
    ...
    if (bweap->busy())
        ...
        bweap = NULL;
    if (weap && !p->weaponchanging && weap != bweap)
        weap->attack(targ);
}
