...
struct model {
    ...
    // translucency(0.25f), scale(1), radius(0), shadowdist(0), translate(0, 0,
    // 0), cachelimit(8), batch(-1) {}
    model()
        : cullface(true)
        , vertexlight(false)
        , alphablend(false)
    ...
    virtual void render(int anim, int varseed, float speed, int basetime,
                        const vec& o, float yaw, float pitch, dynent* d,
                        modelattach* a = NULL, float scale = 1.0f) = 0;
    virtual void setskin(int tex = 0) = 0;

    virtual void genshadows(float height, float rad) {
    }
    virtual void rendershadow(int anim, int varseed, float speed, int basetime,
                              const vec& o, float yaw, modelattach* a = NULL) {
    }
