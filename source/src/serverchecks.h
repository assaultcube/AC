inline bool is_lagging(client *cl) {
  return (cl->spj > 50 || cl->ping > 500 ||
          cl->ldt > 80);  // do not change this except if you really know what
                          // are you doing
}
...
inline void checkclientpos(client *cl) {
  ...
  if (outside_border(po) ||
      maplayout[((int)po.x) + (((int)po.y) << maplayout_factor)] > po.z + 3) {
    if (gamemillis > 10000 && (servmillis - cl->connectmillis) > 10000)
      cl->mapcollisions++;
    if (cl->mapcollisions && !(cl->mapcollisions % 25)) {
      logline(ACLOG_INFO, "[%s] %s is colliding with the map", cl->hostname,
              cl->name);
    }
  }
}
...
const char *medal_messages[] = {"defended the flag", "covered the flag stealer",
...
#define CTFDROPPT \
  -cnumber  // player dropped the flag to other player (probably)
...
bool validlink(client *actor, int cn) {
  return actor->md.linked >= 0 && actor->md.linked == cn &&
         gamemillis < actor->md.linkmillis && valid_client(actor->md.linked);
}
