inline bool is_lagging(client* cl) {
    return (cl->spj > 50 || cl->ping > 500
            || cl->ldt > 80); // do not change this except if you really know
                              // what are you doing
}
...
inline void checkclientpos(client* cl) {
    vec& po = cl->state.o;
    if (outside_border(po)
        || maplayout[((int)po.x) + (((int)po.y) << maplayout_factor)] > po.z
                                                                        + 3) {
...
int getmaxarea(int inversed_x, int inversed_y, int transposed, int ml_factor,
               char* ml) {
    int ls = (1 << ml_factor);
    ...
    for (int y = (inversed_y ? ls - 1 : 0); (inversed_y ? y >= 0 : y < ls);
        ...
        for (int x = (inversed_x ? ls - 1 : 0); (inversed_x ? x >= 0 : x < ls);
             (inversed_x ? x-- : x++)) {
            if (ml[(x << fx) + (y << fy)] != 127) { // if it is not solid
                ...
                if (xf - xi > MINELINE)
                    break; // if the empty line is greater than a minimum, get
                           // out
        ...
        if (xf - xi
            > MINELINE) { // if the line has the minimun threshold of emptiness
...
const char* medal_messages[]
    = { "defended the flag", "covered the flag stealer",
        "defended the dropped flag", "covered the flag keeper",
        "covered teammate" };
...
#define BONUSPT \
    target->state.points / 400 // bonus (for killing high level enemies ::
                               // beware with exponential behavior!)
...
void flagpoints(client* c, int message) {
    ...
    case FM_LOST:
        if (m_htf)
            ...
            if (distance > 200)
                distance
                    = 200; // ~200 is the distance between the flags in ac_depot
            addpt(c, CTFLOSTPT);
        }
        break;
    case FM_RETURN:
    ...
    if (diffhittime < 750) {
        if (gun == actor->md.lastgun) {
            if (diffhittime * 2 < guns[gun].attackdelay * 3) {
                ...
                if (actor->md.combohits > mh2c
                    && actor->md.combohits % mh2c == 1) {
                    if (actor->md.combo < 5)
                        actor->md.combo++;
                    actor->md.ncombos++;
                }
...
int checkteamrequests(int sender) {
    ...
    loopv(clients) {
        ...
        if (i != sender && prot->type != ST_EMPTY && prot->team == ant->team
            ...
            if (dist < REPLYDIST
                && (dtime = prot->md.askmillis - gamemillis) > besttime) {
                bestid = i;
                besttime = dtime;
            }
        }
...
float a2c = 0, c2t = 0,
      a2t
      = 0; // distances: actor to covered, covered to target and actor to target
...
void checkcover(client* target, client* actor) {
    ...
    if (!(covered && actor->md.linked == coverid)
        && validlink(actor, actor->md.linked)) {
    ...
    loopv(clients) {
        ...
        if ((c.state.state == CS_DEAD && !m_arena
            ...
            || (c.state.state == CS_ALIVE && c.upspawnp) /*||
             ...
             // only kick spectator if server is full - 2011oct16:flowtron: mmh,
             that seems reasonable enough .. still, kicking spectators for
             inactivity seems harsh! disabled ATM, kick them manually if you
             must.
             */
            ) {
...
/** This function counts how much non-killing-damage the player does to any
   teammates
    The damage limit is 100 hp per minute, which is about 2 tks per minute in a
   normal game
...
inline int check_pdist(client* c, float& dist) // pick up distance
{
    ...
    float delay = 9.0f + (float)c->ping * 0.02f
                  + (float)c->spj * 0.025f; // at pj/ping 40/100, delay = 12
    if (dist > delay) {
        if (dist < 1.5f * delay)
            return 1;
#ifdef ACAC
...
implemented.
However this file is the sketch to a very special kind of cheat detection tools
in server side.
...
data, and verify if
this data is expected or possible. Also, there is no need to check all clients
all time, and
one coding this kind of check must pay a special attention to the lag effect and
how it can
affect the data observed. This is not a trivial task, and probably it is the
main reason why
...
bool validdamage(client*& target, client*& actor, int& damage, int& gun,
                 bool& gib) {
#ifdef ACAC
