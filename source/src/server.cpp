...
void cleanworldstate(ENetPacket* packet)
{
    loopv(worldstates) {
        ...
        if (ws->positions.inbuf(packet->data)
            || ws->messages.inbuf(packet->data))
            ws->uses--;
        else
            continue;
        if (!ws->uses) {
                ...
                bool demopacket)
{
    if (n < 0) {
        ...
        loopv(clients) if (i != exclude && (clients[i]->type != ST_TCPIP
                                            || clients[i]->isauthed))
    ...
    loopv(clients) {
        ...
        if (psize && (pkt[i].posoff < 0 || psize - pkt[i].poslen > 0)) {
            packet = enet_packet_create(
                         &ws.positions[pkt[i].posoff < 0 ? 0 : pkt[i].posoff
                                       + pkt[i].poslen],
        ...
        if (msize && (pkt[i].msgoff < 0 || msize - pkt[i].msglen > 0)) {
            packet = enet_packet_create(
                         &ws.messages[pkt[i].msgoff < 0 ? 0 : pkt[i].msgoff
                                      + pkt[i].msglen],
...
savedscore* findscore(client& c, bool insert)
{
    ...
    enet_uint32 mask = ENET_HOST_TO_NET_32(
                           mastermode == MM_MATCH ? 0xFFFF0000 : 0xFFFFFFFF); // in match mode,
    // reconnecting from
    // /16 subnet is
    // allowed
    if (!insert) {
...
const char* currentserver(int i)
{
    ...
    switch (i) {
    case 1: {
        copystring(r, scl.ip[0] ? scl.ip : "local"); // IP
        break;
    }
    case 2: {
        copystring(r, scl.logident[0] ? scl.logident : "local"); // HOST
        break;
    }
    case 3: {
        formatstring(r)("%d", scl.serverport); // PORT
        break;
    }
    ...
    while (cc < mc) {
        ...
        case '%': {
            if (cc < (mc - 1)) {
                ...
                default:
                    logline(ACLOG_INFO, "bad formatstring: demonameformat @ %d",
                            cc);
                    cc -= 1;
...
void enddemorecord()
{
    ...
    int mr = gamemillis >= gamelimit ? 0 : (gamelimit - gamemillis + 60000 - 1)
             / 60000;
    ...
    // len/1024.0f, len > 1024*1024 ? "MB" : "kB"); // the datetime bit is
    // pretty useless in the servmesg, no?!
    formatstring(d.info)("%s: %s, %s, %.2f%s", asctime(), modestr(gamemode),
                         smapname, len > 1024 * 1024 ? len / (1024 * 1024.f)
                         : len / 1024.0f,
                         len > 1024 * 1024 ? "MB" : "kB");
    if (mr) {
    ...
    formatstring(d.file)("%d:%d:%d:%s:%s", gamemode, mPLAY, mDROP, mTIME,
                         iMAPN);
    ...
    if (scl.demopath[0]) {
        ...
        if (demo) {
            ...
            delete demo;
            logline(ACLOG_INFO, "demo written to file \"%s\" (%d bytes)", msg,
                    wlen);
        } else {
...
void senddemo(int cn, int num)
{
    ...
    if (scl.demo_interm && (!interm || totalclients > 2) && !is_admin) {
        sendservmsg("\f3sorry, but this server only sends demos at "
...
void setupdemoplayback()
{
    ...
    if (!demoplayback)
        ...
        if (hdr.version != DEMO_VERSION)
            formatstring(msg)(
                "demo \"%s\" requires an %s version of AssaultCube", file,
            ...
            sendservmsg("WARNING: using experimental compatibility mode for "
                        "older demo protocol, expect breakage");
...
void flagaction(int flag, int action, int actor)
{
    ...
    if (m_ctf || m_htf) {
        ...
        case FA_SCORE: // f = carried by actor flag
            if (actor != -1 || f.state != CTFF_STOLEN) {
                abort = 21; // no client msg allowed here
                break;
            }
    ...
    if (abort) {
        logline(ACLOG_DEBUG, "aborting flagaction(flag %d, action %d, actor "
                "%d), reason %d, resending flag states",
                flag, action, actor, abort); // FIXME: remove this logline after
        // some time - it will only show a
        // few bad ping effects
        sendflaginfo();
        return;
    }
    if (score) {
    ...
    if (valid_client(actor)) {
        ...
        case FM_SCORE:
            if (m_htf)
                logline(
                    ACLOG_INFO, "[%s] %s hunted the flag for %s, new score %d",
...
int clienthasflag(int cn)
{
    if (m_flags && valid_client(cn)) {
        loopi(2) {
            if (sflaginfos[i].state == CTFF_STOLEN
                && sflaginfos[i].actor_cn == cn)
                return i;
        }
    }
    return -1;
...
int cmptwoint(const struct twoint* a, const struct twoint* b)
{
...
{
    client* c = clients[n];
    int waitspawn = min(c->ping, 200) + c->state.spawn; // flowtron to Brahma:
...
void arenacheck()
{
    ...
    if (m_botmode && clients[0]->type == ST_LOCAL) {
        ...
        loopv(players) if (players[i] && (!m_teammode
                                          || players[i]->team
                                          == team_opposite(player1->team))) {
            enemies++;
            ...
            sendf(-1, 1, "ri2", SV_ARENAWIN,
                  ...
                  ? (alive ? alive->clientnum : -1)
                      : (alive && alive->type == ENT_BOT
                         ? -2
                         : player1->state == CS_ALIVE ? player1->clientnum
                         : -1));
...
// chat
// XX     -->   RVSF players                >-----/ | | | | | | | |      c:
// normal chat in all mastermodes except 'match'
// XX X   -->   RVSF spect players          >-------/ | | | | | | |      t: all
// chat in mastermode 'match', otherwise only team chat
// X X    -->   CLA players                 >---------/ | | | | | |
...
void sendteamtext(char* text, int sender, int msgtype)
{
    ...
    loopv(clients) if (i != sender) {
        ...
        if ((rt == TEAM_SPECT && clients[i]->role == CR_ADMIN)
            || // spect-admin reads all
...
// first client that gets it
{
    ...
    if (!e.spawned) {
        if (!e.legalpickup && hn && !m_demo)
            logline(ACLOG_INFO, "[%s] tried to pick up entity #%d (%s) - can't "
                    "be picked up in this gamemode or at all",
    ...
    if (ts.health <= 0) {
        ...
        if (isdedicated && actor->type == ST_TCPIP && tk) {
            if (actor->state.frags < scl.banthreshold ||
                /** teamkilling more than 6 (defaults), more than 2 per minute
                   and less than 4 frags per tk */
                ...
                addban(actor, DISC_AUTOBAN);
                       ...
                       /** teamkilling more than 5 (defaults), more than 1 tk
                          per minute and less than 4 frags per tk */
...
int canspawn(client* c) // beware: canspawn() doesn't check m_arena!
{
    if (!c || c->type == ST_EMPTY || !c->isauthed || !team_isvalid(c->team)
        ...
        || (c->type == ST_TCPIP
            && (servmillis - c->connectmillis
                < 1000 + c->state.reconnections * 2000 && gamemillis > 10000
                && totalclients > 3 && !team_isspect(c->team))))
...
void autospawncheck()
{
    if (mastermode != MM_MATCH || !m_autospawn || interm)
        return;
    ...
    if (newteam == TEAM_ANYACTIVE) { // when spawning from spect
        if (mastermode == MM_MATCH && cl.team < TEAM_SPECT) {
            ...
            if (autoteam && teamsizes[TEAM_CLA] != teamsizes[TEAM_RVSF])
                newteam = teamsizes[TEAM_CLA] < teamsizes[TEAM_RVSF]
                          ? TEAM_CLA
                          : TEAM_RVSF;
                ...
                loopv(clients) if (clients[i]->type != ST_EMPTY && i != cln
                    ...
                    teamscore[team_base(clients[i]->team)]
                    += clients[i]->at3_score;
                }
                newteam = sum > 200
    ...
    if (ftr == FTR_PLAYERWISH) {
        if (mastermode == MM_MATCH && matchteamsize && m_teammode) {
            if (newteam != TEAM_SPECT
                ...
                return false; // no switching sides in match mode when teamsize
            // is set
        }
        if (team_isactive(newteam)) {
            ...
            if (mastermode == MM_MATCH) {
                if (m_teammode && matchteamsize
                    && teamsizes[newteam] >= matchteamsize)
            ...
            newteam = TEAM_SPECT; // only match mode (team) has more than one
        // spect team
    }
    if (cl.team == newteam && ftr != FTR_AUTOTEAM)
        return true; // no change
...
int calcscores() // skill eval
{
    ...
    loopv(clients) if (clients[i]->type != ST_EMPTY) {
        ...
        sum += clients[i]->at3_score = cs.points > 0
...
void shuffleteams(int ftr = FTR_AUTOTEAM)
{
    ...
    if (gamemillis < 2 * 60 * 1000) { // random
        ...
        loopv(clients) if (clients[i]->type != ST_EMPTY
                           ...
                           && !team_isspect(
                               clients[i]
                               ->team)) { // only shuffle active players
            sums += rnd(1000);
            team = sums & 1;
...
bool balanceteams(int ftr) // pro vs noobs never more
{
    ...
    loopv(clients) if (clients[i]->type != ST_EMPTY) {
        ...
        if (c->isauthed && team_isactive(c->team)) {
            ...
            tsize[c->team]++;
            // effective score per minute, thanks to wtfthisgame for the nice
            // idea
    ...
    if (tsize[h] - tsize[l]
        ...
        loopv(clients) if (clients[i]->type != ST_EMPTY) {
            ...
            // client from the h team, not forced in this game, and without the
            // flag
            if (c->isauthed && c->team == h && !c->state.forced
                && clienthasflag(i) < 0) {
                // do not exchange in the way that weaker team becomes the
                // stronger or the change is less than 20% effective
                if (2 * c->eff_score <= diffscore
                    && 10 * c->eff_score >= diffscore && c->eff_score > besth) {
                    besth = c->eff_score;
                    hid = i;
                }
            }
        ...
        if (bestpair[h] >= 0 && bestpair[l] >= 0) {
            ...
            clients[bestpair[h]]->state.forced
                = clients[bestpair[l]]->state.forced = true;
            return true;
        }
    }
    return false;
...
bool refillteams(bool now, int ftr) // force only minimal amounts of players
{
    ...
    if (diffnum > 1) {
        if (now || gamemillis - lasttime_eventeams > 8000 + allplayers * 1000
            ...
            loopv(clients) if (clients[i]->type != ST_EMPTY
                               && clients[i]->team != bigteam)
                clients[i]->at3_dontmove = true; // dont move small team players
            while (diffnum > 1 && moveable[bigteam] > 0) {
                ...
                loopv(clients) if (clients[i]->type != ST_EMPTY
                                   && !clients[i]->at3_dontmove) // try all
                    // still
                    // movable
                    // players
                {
                    ...
                    int forcedelay
                        = clients[i]->at3_lastforce
                          ? (1000
                             - (gamemillis - clients[i]->at3_lastforce)
                             / (5 * 60))
                          : 0;
                    if (forcedelay > 0)
                        fit += (fit * forcedelay)
                               / 600; // avoid lately forced players
                    if (fit < bestfit
                        + fit * rnd(100)
                        / 400) { // search 'almost' best fit
                        bestfit = fit;
                        pick = i;
                    }
                }
                ...
                if (updateclientteam(pick, !bigteam, ftr)) {
                    ...
                    clients[pick]->at3_lastforce
                        = gamemillis; // try not to force this player again for
                    // the next 5 minutes
                    switched = true;
                }
            }
    ...
    if (newmode == GMODE_DEMO) {
        ...
        if (ms) {
            ...
            loopi(2) {
                ...
                if (smapstats.flags[i] == 1) // don't check flag positions, if
            ...
            logline(ACLOG_INFO, "Map height density information for %s: H = "
                    "%.2f V = %d, A = %d and MA = %d",
                    smapname, Mheight, Mvolume, Marea, Mopen);
            items_blocked = false;
        } else if (isdedicated)
            sendservmsg("\f3server error: map not found - please start another "
                        "map or send this map to the server");
        if (notify) {
            // change map
        ...
        if (ms)
            concatformatstring(
                gsmsg, "(map rev %d/%d, %s, 'getmap' %sprepared)",
                smapstats.hdr.maprevision, smapstats.cgzsize, maplocstr[maploc],
        ...
        if (notify) {
            ...
            // prepare spawns; players will spawn, once they've loaded the
            // correct map
            loopv(clients) if (clients[i]->type != ST_EMPTY) {
                client* c = clients[i];
                c->mapchange();
...
struct voteinfo {
    ...
    void evaluate(bool forceend = false) {
        ...
        loopv(
            clients) if (clients[i]->type
                         != ST_EMPTY /*&& clients[i]->connectmillis < callmillis*/) { // new connected people will vote now
...
#define yes_condition                                              \
    ((min_time && stats[VOTE_YES] - stats[VOTE_NO] > 0.34f * total \
      && totalclients > 4) || stats[VOTE_YES] > requiredcount * total)
#define no_condition                                            \
    (forceend || !valid_client(owner)                           \
     || stats[VOTE_NO] >= stats[VOTE_YES] + stats[VOTE_NEUTRAL] \
     || adminvote == VOTE_NO)
#define boot_condition                    \
    (!boot || (boot && valid_client(num1) \
               && clients[num1]->peer->address.host == host))
               ...
               ENetPacket* msg) // true if a regular vote was called
{
    ...
    if (!v || !v->isvalid() || (v->boot && (!b || cn2boot == v->owner)))
        ...
        error = VOTEE_DISABLED;
              ...
              && c->role != CR_ADMIN && numclients() > 1) || c->nvotes > 3)
        error = VOTEE_MAX;
    else if (((v->boot == 1 && c->role < roleconf('w'))
              || (v->boot == 2 && c->role < roleconf('X'))) && !is_lagging(b)
             && !b->mute && b->spamcount < 2) {
        /** not same team, with low ratio, not lagging, and not spamming... so,
         * why to kick? */
    ...
    if (error >= 0) {
        ...
        if (v->type == SA_MAP && v->num1 >= GMODE_NUM)
            map_queued = true;
        if (!v->gonext)
            sendpacket(-1, 1, msg, v->owner); // FIXME in fact, all votes should
        // go to the server, registered,
        // and then go back to the clients
        else
            callvotepacket(-1, v); // also, no vote should exclude the caller...
        // these would provide many code
...
void callvotepacket(int cn,
                    voteinfo* v
                    = curvote)   // FIXME, it would be far smart if the msg
{
    // buffer from SV_CALLVOTE was simply saved
    int n_yes = 0, n_no = 0;
    loopv(clients) if (clients[i]->type != ST_EMPTY) {
    ...
    if (force || role == CR_DEFAULT
        || (role == CR_ADMIN && pwd && pwd[0]
            && passwords.check(clients[client]->name, pwd,
                               clients[client]->salt, &pd) && !pd.denyadmin)) {
        ...
        logline(ACLOG_INFO, "[%s] set role of player %s to %s",
                ...
                role == CR_ADMIN ? "admin"
                : "normal player"); // flowtron : connecting
        // players haven't got a
        // name yet (connectadmin)
        if (role > CR_DEFAULT)
            sendiplist(client);
    } else if (pwd && pwd[0])
...
const char* disc_reason(int reason)
{
    static const char* disc_reasons[]
        = { "normal", "error - end of packet", "error - client num",
            "vote-kicked from the server", "vote-banned from the server",
            "error - tag type",
            "connection refused - you have been banned from this server",
            "incorrect password", "unsuccessful administrator login",
            "the server is FULL - try again later",
            "servers mastermode is \"private\" - wait until the servers "
            "mastermode is \"open\"",
    ...
    return reason >= 0
           && (size_t)reason < sizeof(disc_reasons)
           / sizeof(disc_reasons[0])
           ? disc_reasons[reason]
           : "unknown";
}
...
void welcomepacket(packetbuf& p, int n)
{
    ...
    if (smapname[0] && !m_demo) {
        ...
        if (smode > 1 || (smode == 0 && numnonlocalclients() > 0)) {
            putint(p, SV_TIMEUP);
            putint(p, (gamemillis >= gamelimit || forceintermission)
                   ? gamelimit
                   : gamemillis);
            putint(p, gamelimit);
    ...
    if (c) {
        if (c->type == ST_TCPIP && serveroperator() != -1)
            sendserveropinfo(n);
        c->team = mastermode == MM_MATCH && sc ? team_tospec(sc->team)
                  : TEAM_SPECT;
        putint(p, SV_SETTEAM);
        putint(p, n);
    ...
    const char* motd = scl.motd[0] ? scl.motd
                       : infofiles.getmotd(c ? c->lang : "");
    if (motd) {
        putint(p, SV_TEXT);
        sendstring(motd, p);
    }
...
int checktype(int type, client* cl)
{
    ...
    if (cl) {
        loopi(sizeof(servtypes) / sizeof(int)) if (type
                == servtypes[i]) return -1;
        loopi(sizeof(edittypes) / sizeof(int)) if (type
                == edittypes[i]) return smode
                                            == GMODE_COOPEDIT
                                            ? type
                                            : -1;
        if (++cl->overflow >= 200)
...
void process(ENetPacket* packet, int sender, int chan)
{
    ...
    if (cl && !cl->isauthed) {
        ...
        if (chan == 0)
            ...
            if (matchreconnect && !banned) { // former player reconnecting to a
                // server in match mode
                cl->isauthed = true;
                logline(ACLOG_INFO, "[%s] %s logged in (reconnect to match)%s",
                        cl->hostname, cl->name, tags);
            } else if (wl == NWL_IPFAIL
                       || wl == NWL_PWDFAIL) { // nickname matches whitelist,
                // but IP is not in the required
                // range or PWD doesn't match
                logline(ACLOG_INFO,
                ...
                if (!pd.denyadmin && wantrole == CR_ADMIN)
                    clientrole = CR_ADMIN;
                if (bantype == BAN_VOTE) {
                    loopv(bans) if (bans[i].address.host
                                    == cl->peer->address.host) {
                        bans.remove(i); // remove admin bans
                ...
                if (srvfull) {
                    loopv(clients) if (i != sender
                                       && clients[i]->type == ST_TCPIP) {
                        disconnect_client(i, DISC_MAXCLIENTS); // disconnect
                        // someone else
                        // to fit
                        // maxclients
                        // again
        ...
        loopv(clients) {
            if (clients[i] && clients[i]->clientnum != cl->clientnum
                && (clients[i]->role == CR_ADMIN
                    || clients[i]->type == ST_LOCAL))
    ...
    if (!cl) {
        logline(ACLOG_ERROR,
                "<NULL> client in process()"); // should never happen anyway
        return;
    }
        ...
        case SV_TEAMTEXT:
            ...
            if (*text) {
                ...
                if (!spamdetect(cl, text) && canspeech) { // team chat
                    ...
                    if (canspeech) {
                        sendservmsg("\f3Please do not spam; your message was "
                                    "not delivered.",
                                    sender);
                        if (cl->spamcount > SPAMMAXREPEAT + 2)
                            disconnect_client(cl->clientnum, DISC_ABUSE);
                    } else {
                        sendservmsg("\f3Watch your language! Your message was "
                                    "not delivered.",
                                    sender);
                        kick_abuser(cl->clientnum, cl->badmillis, cl->badspeech,
                                    3);
                    }
                }
            }
            ...
            if (*text) {
                ...
                if (!spamdetect(cl, text) && canspeech) {
                    if (mastermode != MM_MATCH || !matchteamsize
                        ...
                        sendteamtext(text, sender, type == SV_TEXTME
                                     ? SV_TEAMTEXTME
                                     : SV_TEAMTEXT);
                    }
                } else {
                    logline(ACLOG_INFO, "[%s] %s%s says: '%s', %s",
                            cl->hostname, type == SV_TEXTME ? "(me) " : "",
                            cl->name, text,
                            canspeech ? "SPAM detected" : "Forbidden speech");
                    if (canspeech) {
        ...
        case SV_VOICECOMTEAM: {
            ...
            if (servmillis > cl->mute) { // client is not muted
                if (s < S_AFFIRMATIVE || s >= S_NULL)
                    ...
                    if (cl->spam > 0)
                        cl->spam -= (servmillis - cl->lastvc)
                                    / 4000; // no vc in the last 4 seconds
                } else
                ...
                if (cl->spam > 4) {
                    cl->mute = servmillis + 10000; // 5 vcs in less than 20
                    // seconds... shut up please
                    break;
                }
                if (m_teammode)
                    checkteamplay(
                        s, sender); // finally here we check the teamplay comm
                if (type == SV_VOICECOM) {
                    QUEUE_MSG;
                } else
        ...
        case SV_MAPIDENT: {
            ...
            if (!isdedicated
                || (smapstats.cgzsize == gzs
                    ...
                    == rev)) { // here any game really starts for a client:
                // spawn, if it's a new game - don't spawn if
                ...
                if (team_isspect(cl->team)) {
                    if (numclients() < 2 && !m_demo
                        ...
                        spawn = updateclientteam(cl->clientnum, TEAM_ANYACTIVE,
                                                 FTR_INFO);
                    }
                } else {
...
{                                                                  \
    event.id = getint(p);                                          \
    if (!cl->timesync || (cl->events.length() == 1                 \
                          && cl->state.waitexpired(gamemillis))) { \
        cl->timesync = true;                                       \
        ...
        case SV_AUTHT: {
            /*                int n = getint(p);
                            loopi(n) getint(p);*/
            //                 if (cl) disconnect_client(cl->clientnum,
            //                 DISC_TAGT); // remove this in the future, when
            //                 auth is complete
            break;
        }
        // :for AUTH
        ...
        case SV_SENDMAP: {
            ...
            if (readonlymap(mp)) {
                reject = "map is ro";
                defformatstring(msg)(
                    "\f3map upload rejected: map %s is readonly", sentmap);
                sendservmsg(msg, sender);
            } else if (scl.incoming_limit
                       && (scl.incoming_limit << 20) < incoming_size + mapsize
                       + cfgsizegz) {
                ...
                sendservmsg(
                    ...
                    sender);
                       ...
                       && cl->role
                       < CR_ADMIN) { // default: only admins can update maps
                reject = "no permission to update";
                sendservmsg("\f3map update rejected: you need to be admin",
                            sender);
            } else if (mp == MAP_TEMP && revision < mapbuffer.revision
                       && !strchr(scl.mapperm, 'r')
                       && cl->role < CR_ADMIN) // default: only admins can
                // revert maps to older revisions
            {
                reject = "no permission to revert revision";
                sendservmsg("\f3map revert to older revision rejected: you "
                            "need to be admin to upload an older map",
                            sender);
            } else {
                if (mapbuffer.sendmap(sentmap, mapsize, cfgsize, cfgsizegz,
                                      &p.buf[p.len])) {
                    incoming_size += mapsize + cfgsizegz;
                    logline(ACLOG_INFO, "[%s] %s sent map %s, rev %d, %d + "
                            "%d(%d) bytes written",
                            clients[sender]->hostname, clients[sender]->name,
                            sentmap, revision, mapsize, cfgsize, cfgsizegz);
                    defformatstring(msg)(
                        "%s (%d) up%sed map %s, rev %d%s",
                        clients[sender]->name, sender,
                        mp == MAP_NOTFOUND ? "load" : "dat", sentmap, revision,
        ...
        case SV_REMOVEMAP: {
            ...
            if (cl->role < reqrole)
                ...
                sendservmsg(msg, sender);
                logline(ACLOG_INFO, "[%s] deleted map %s",
                        clients[sender]->hostname, rmmap);
            }
            if (reject) {
        ...
        case SV_CALLVOTE: {
            ...
            case SA_MAP: {
                ...
                if (mode == GMODE_DEMO)
                    ...
                    char* vmap
                        = newstring(vi->text ? behindpath(vi->text) : "");
                    vi->action = new mapaction(vmap, qmode, time, sender,
                                               qmode != mode);
                }
                break;
            }
        ...
        case SV_VOTE: {
            ...
            // queued to prevent double-freeing by
            // packetbuf
            svote(sender, n, msg);
            --msg->referenceCount;
            break;
        ...
        case SV_EXTENSION: {
            ...
            //
            // rules:
            // 1. extensions MUST NOT modify gameplay or the behavior of the
            // game in any way
            // 2. extensions may ONLY be used to extend or automate server
            // administration tasks
            // 3. extensions may ONLY operate on the server and must not send
            // any additional data to the connected clients
            // 4. extensions not adhering to these rules may cause the hosting
            // server being banned from the masterserver
            //
            // also note that there is no guarantee that custom extensions will
            // work in future AC versions
            ...
            if (!strcmp(ext, "driAn::writelog")) {
                ...
                // usage:       /serverextension driAn::writelog "your log
                // message here.."
                // note:        There is a 49 character limit. The server will
...
void loggamestatus(const char* reason)
{
    ...
    if (mastermode == MM_MATCH) {
        loopv(savedscores) {
            savedscore& sc = savedscores[i];
            if (sc.valid) {
                formatstring(text)(m_teammode ? "%-4s " : "",
                                   team_string(sc.team, true));
                if (m_flags)
                    concatformatstring(text, "%4d ", sc.flagscore);
                logline(ACLOG_INFO, "   %-16s %s%4d %5d%s    - disconnected",
                        sc.name, text, sc.frags, sc.deaths,
                        m_teammode ? "  -" : "");
...
{
    static unsigned int chokes[MAXCLIENTS + 1] = { 0 },
            spent[MAXCLIENTS + 1] = { 0 },
...
void serverslice(uint timeout) // main server update, called from cube main loop
// in sp, or dedicated server loop
{
    ...
    if (forceintermission
        || ((smode > 1 || (gamemode == 0 && nonlocalclients))
            && gamemillis - diff > 0
    ...
    if (serverhost->bandwidthThrottleEpoch != lastThrottleEpoch) {
        if (lastThrottleEpoch)
            linequalitystats(serverhost->bandwidthThrottleEpoch
                             - lastThrottleEpoch);
    ...
    while (!serviced) {
        ...
        case ENET_EVENT_TYPE_CONNECT: {
            ...
            char hn[1024];
            copystring(c.hostname, (enet_address_get_host_ip(
...
void extping_uplinkstats(ucharbuf& po)
{
    if (scl.maxclients > 3)
        po.put(chokelog + 4,
               scl.maxclients - 3); // send logs for 4..n used slots
}
                      ...
                      ENetAddress& addr, ENetBuffer& buf, int len, int* csend)
{
    loopv(clients) {
        ...
        // "team_string(clients[i]->team)" sometimes return NULL according to
        // RK, causing the server to crash. WTF ?
        ...
        putint(p,
               clients[i]
               ->state.state); // State (Alive,Dead,Spawning,Lagged,Editing)
        uint ip = clients[i]->peer->address.host; // only 3 byte of the ip
...
void quitproc(int param)
{
    ...
    exit(param == 2 ? EXIT_SUCCESS : EXIT_FAILURE); // 3 is the only reply on
    // Win32 apparently, SIGINT
    // == 2 == Ctrl-C
}
...
void initserver(bool dedicated, int argc, char** argv)
{
    ...
    if (isdedicated) { // do not return, this becomes main loop
        ...
        // (or VS-2008?) for caveat (seems not to pertain to AC - 2011feb05:ft)
...
// SIGKILL!");
        ...
        logline(ACLOG_INFO, "Ctrl-C to exit"); // this will now actually call
        // the atexit-hooks below -
        // thanks to SIGINT hooked above
        // - noticed and signal-code-docs
        // found by SKB:2011feb05:ft:
