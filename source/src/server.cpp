...
void serverdamage(client *target, client *actor, int damage, int gun, bool gib, const vec &hitpush = vec(0, 0, 0))
{
    ...
    if (ts.health <= 0)
    {
        ...
        if (!suic)
            logline(ACLOG_INFO, "[%s] %s %s%s %s", actor->hostname, actor->name, valid_weapon(gun) ? killmessages[gib ? 1 : 0][gun] : "smurfed", tk ? " their teammate" : "", target->name);
...
void autospawncheck()
{
    ...
    {
        client *cl = clients[i];
        if ((cl->state.state == CS_DEAD || cl->state.state == CS_SPECTATE) && canspawn(cl) == SP_OK && !cl->autospawn)
        {
            sendspawn(cl);
            cl->autospawn = true;
        }
...
#define MSG_PACKET(packet)                                              \
    ...
    while ((curmsg = p.length()) < p.maxlen)
    {
        ...
        case SV_WEAPCHANGE:
        {
            ...
            if (!valid_weapon(gunselect) || gunselect == GUN_CPISTOL)
                break;
            if (!m_demo && !m_coop)
        ...
        case SV_SPAWN:
        {
            ...
            if ((cl->state.state != CS_ALIVE && cl->state.state != CS_DEAD && cl->state.state != CS_SPECTATE) ||
                ls != cl->state.lifesequence || cl->state.lastspawn < 0 || !valid_weapon(gunselect) || gunselect == GUN_CPISTOL)
                break;
