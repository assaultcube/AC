...
struct md2 : vertmodel {
    ...
    struct md2part : part {
        void gentcverts(int* glcommands, vector<tcvert>& tcverts,
                        vector<ushort>& vindexes, vector<tri>& tris) {
        ...
        bool load(char* path) {
            ...
            int numglcommands
                = (int)file->read(glcommands,
                                  sizeof(int) * header.numglcommands)
                  / sizeof(int);
            ...
            loopi(header.numframes) {
                ...
                loopj(m.numverts) {
                    ...
                    *curvert++ = vec(
                        v.vertex[0] * frame.scale[0] + frame.translate[0],
        ...
        void getdefaultanim(animstate& as, int anim, int varseed) {
            ...
            //                      21  23  24
            //                      I   R   A   P   P   P   J   L   F   S   T
            //                      W   P   CI  CW  CA  CP  CD  D   D   D   LD
            //                      LD  LD   F
            static int frame[] = { 0,   40,  46,  54,  58,  62,  66,  69,  72,
                                   84,  95,  112, 123, 135, 154, 160, 169, 173,
                                   178, 184, 190, 183, 189, 197, 0 };
            static int range[] = { 40, 6, 8, 4, 4, 4, 3, 3, 12, 11, 17, 11, 12,
                                   19, 6, 9, 4, 5, 6, 6, 8, 1,  1,  1,  7 };
            static int animfr[] = { 0,  1,  2,  3,  6,  7,  8,  9,  10, 11,
    ...
    void render(int anim, int varseed, float speed, int basetime, const vec& o,
                float yaw, float pitch, dynent* d, modelattach* a,
                float scale) {
        if (!loaded)
            return;
    ...
    void rendershadow(int anim, int varseed, float speed, int basetime,
                      const vec& o, float yaw, modelattach* a) {
    ...
    bool load() {
        ...
        per_idents = true;
        loadingmd2 = 0;
        loopv(parts) parts[i]->scaleverts(
            scale / 16.0f, vec(translate.x, -translate.y, translate.z));
        radius = calcradius();
        if (shadowdist)
            calcneighbors();
        calcbbs();
