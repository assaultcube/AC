...
struct md2 : vertmodel {
  ...
  struct md2part : part {
    ...
    bool load(char *path) {
      ...
      int numglcommands =
          (int)file->read(glcommands, sizeof(int) * header.numglcommands) /
          sizeof(int);
  ...
  void render(int anim, int varseed, float speed, int basetime, const vec &o,
              float roll, float yaw, float pitch, dynent *d, modelattach *a,
              float scale) {
    if (!loaded) return;
    ...
    if (a)
      for (int i = 0; a[i].tag; i++) {
        ...
        m->setskin();
        m->render(anim, varseed, speed, basetime, o, roll, yaw, pitch, d, NULL,
                  scale);
      }
