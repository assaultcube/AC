...
COMMANDF(
    curmenu, "", () { result(curmenu ? curmenu->name : ""); });
        ...
        if (isselection())
        {
            ...
            { // load map description and preview picture
                ...
                {                                                                   // checking map messages is slow and may be disabled
                    ...
                    if (!mapmessage)
                        mapmessage = getfiledesc((cgzpath = "packages" PATHDIVS "maps" PATHDIVS "official"), behindpath(filename), "cgz"); // check for official map
                }
                defformatstring(p2p)("%s/preview/%s.jpg", cgzpath, filename);
                if (!hidebigmenuimages)
            ...
            if (mapmessage && *mapmessage)
                draw_text(mapmessage, xp - FONTH, ym, 0xFF, 0xFF, 0xFF, 0xFF, -1, 2 * VIRTW - (xp - FONTH) - FONTH / 2);
        }
    }
};
...
struct mitemtextinput : mitemtext
{
    ...
    virtual void render(int x, int y, int w)
    {
        ...
        while (iboff > 0)
        {
            ...
            if (text_width(showinput) > menurighttabwidth)
                break;
            iboff--;
            sc++;
        }
...
struct mitemslider : mitem
{
                                                                                                                                                    ...
                                                                                                                                                    min_(_min), max_(_max), value(_min), maxvaluewidth(0), text(text), valueexp(value), action(action), wrap(wrap), isradio(isradio)
    {
        ...
        if (!*display || *enddigits)
        {
            ...
            {
                if (max_ != -1)
                    clientlogf("menuitemslider: display string length (%d) doesn't match max-min (%d) \"%s\" [%s]", opts.length(), max_ - min_ + 1, text, display);
...
COMMANDF(
    menuheader, "ss", (char *header, char *footer) { if(lastmenu) menuheader(lastmenu, *header ? newstring(header) : NULL, *footer ? newstring(footer) : NULL, true); });
...
{
    if (!lastmenu || !lastmenu->dirlist || !*action)
        return;
...
void chmenutexture(char *menu, char *texname, char *title)
{
    ...
    if (*texname)
        m->previewtexture = newstring(texname);
    DELETEA(m->previewtexturetitle);
    if (*title)
        m->previewtexturetitle = newstring(title);
}
...
{
    if (title)
        conoutf("[::  %s  ::]", title); //if(items.length()) conoutf( " %03d items", items.length());
...
void gmenu::init()
{
    ...
    if (dirlist && dirlist->dir && dirlist->ext)
    {
        ...
        {
            vector<char *> subdirs;
            if (dirlist->subdirdots)
                subdirs.add(newstring(".."));
...
void gmenu::render()
{
    ...
    if (header)
        w = max(w, text_width(header) + menupageswidth);
    ...
    if (footer || hasdesc)
    {
        ...
        if (!hasdesc)
            text_bounds(footer, footwidth, maxfootheight, w);
        if (maxfootheight)
            footlen = min(MAXMENU / 4, (maxfootheight + step / 2) / step);
        if (usefoot)
            draw_text(usefoot, x, y, 0xFF, 0xFF, 0xFF, 0xFF, -1, w);
    }
...
void gmenu::renderbg(int x1, int y1, int x2, int y2, bool border)
{
    ...
    if (!tex)
        tex = textureload("packages/misc/menu.jpg");
