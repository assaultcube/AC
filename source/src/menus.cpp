...
char* getfiledesc(const char* dir, const char* name, const char* ext)
{
    ...
    if (!strcmp(ext, "dmo")) {
        ...
        formatstring(text)(
            "%s%s", (hdr.version > MAPVERSION) ? "(incompatible file) " : "",
            hdr.maptitle);
        text[DHDR_DESCCHARS - 1] = '\0';
        return newstring(text);
    }
...
struct mitemtextvar : mitemmanual {
    string dtext;
    const char* textexp;
    mitemtextvar(gmenu* parent, const char* evalp, char* action,
                 char* hoveraction)
...
struct mitemimagemanual : mitemmanual {
    ...
    mitemimagemanual(gmenu* parent, const char* filename,
                     const char* altfontname, char* text, char* action,
    ...
    virtual void render(int x, int y, int w) {
        ...
        if (image || altfont) {
            ...
            if (image && isselection() && !hidebigmenuimages
                && image->ys > FONTH) {
                w += FONTH;
...
struct mitemimage : mitemimagemanual {
    mitemimage(gmenu* parent, const char* filename, char* text, char* action,
               char* hoveraction, color* bgcolor, const char* desc = NULL)
        : mitemimagemanual(parent, filename, NULL, text, action, hoveraction,
                           bgcolor, desc) {
    }
    virtual ~mitemimage() {
        DELETEA(filename);
...
struct mitemmaploadmanual : mitemmanual {
    ...
    mitemmaploadmanual(gmenu* parent, const char* filename,
                       const char* altfontname, char* text, char* action,
                       char* hoveraction, color* bgcolor,
        ...
        if (filename) {
            ...
            if (d) {
                ...
                if (d) {
                    ...
                    formatstring(maptitle)("-n/a-:%s", mapname);
            }
            ...
            formatstring(maptitle)("-n/a-:%s", filename);
            image = textureload("packages/misc/nopreview.png", 3);
        }
        copystring(mapstats, "");
    }
        ...
        if (image) {
            ...
            if (image && isselection() && !hidebigmenuimages
                ...
                glBindTexture(GL_TEXTURE_2D,
                              image->id); // I just copy&pasted this...
                glDisable(GL_BLEND);
                glColor3f(1, 1, 1);
                glBegin(GL_TRIANGLE_STRIP);
                glTexCoord2f(0, 0);
                ...
                if (maptitle[0]) {
                    ...
                    draw_text(maptitle, FONTH / 2, VIRTH - FONTH / 2, 0xFF,
                              0xFF, 0xFF, 0xFF, -1,
                              maploaditemlength * FONTH / 2);
                }
...
struct mitemmapload : mitemmaploadmanual {
    mitemmapload(gmenu* parent, const char* filename, char* text, char* action,
                 char* hoveraction, color* bgcolor, const char* desc = NULL)
        : mitemmaploadmanual(parent, filename, NULL, text, action, hoveraction,
                             bgcolor, desc) {
    }
    //  mitemimage  (gmenu *parent, const char *filename, char *text, char
    //  *action, char *hoveraction, color *bgcolor, const char *desc = NULL) :
    //  mitemimagemanual  (parent, filename, NULL, text, action, hoveraction,
    //  bgcolor, desc) {}
    //  mitemmapload(gmenu *parent, const char *filename, char *text, char
    //  *action, char *hoveraction, color *bgcolor, const char *desc = NULL) :
    //  mitemmaploadmanual(parent, filename, NULL, text, action, hoveraction,
    //  bgcolor, desc) {}
    //  mitemmanual (gmenu *parent, char *text, char *action, char *hoveraction,
    //  color *bgcolor, const char *desc) : mitem(parent, bgcolor), text(text),
...
struct mitemtextinput : mitemtext {
    ...
    mitemtextinput(gmenu* parent, char* text, char* value, char* action,
                   char* hoveraction, color* bgcolor, int maxchars,
                   int maskinput)
    ...
    virtual int width() {
        ...
        int maxw
            = min(input.max, 15)
              * text_width("w"); // w is broadest, not a - but limit to 15*w
        return labelw + maxw;
    }
    ...
    virtual void render(int x, int y, int w) {
        ...
        while (iboff > 0) {
            ...
            if (text_width(showinput) > 15 * text_width("w"))
                break;
            iboff--;
            sc++;
        }
        ...
        if (hideinput) // "mask" user input with asterisks, use for
        ...
        draw_text(
            showinput, x + w - tw, y, 255, 255, 255, 255,
            selection
    ...
    virtual void key(int code, bool isdown, int unicode) {
        if (input.key(code, isdown, unicode))
            modified = true;
    }
...
struct mitemslider : mitem {
    ...
    mitemslider(gmenu* parent, char* text, int min_, int max_, int step,
                char* value, char* display, char* action, color* bgcolor)
        ...
        , action(action) {
        if (sliderwidth == 0)
            sliderwidth = max(VIRTW / 4, 15 * text_width("w")); // match slider
        // width with
        // width of text
        // input fields
    }
...
struct mitemkeyinput : mitem {
    ...
    virtual void key(int code, bool isdown, int unicode) {
        if (!capture || code < -5 || code > SDLK_MENU)
            return;
        keym* km;
        while ((km = findbinda(bindcmd))) {
            bindkey(km, ""); // clear existing binds to this cmd
        }
...
struct mitemcheckbox : mitem {
    ...
    mitemcheckbox(gmenu* parent, char* text, char* value, char* action,
                  color* bgcolor)
...
void menumanual(void* menu, char* text, char* action, color* bgcolor,
                const char* desc)
{
    gmenu& m = *(gmenu*)menu;
...
void menuitemimage(char* name, char* text, char* action, char* hoveraction)
{
    ...
    if (fileexists(name, "r") || findfile(name, "r") != name)
        lastmenu->items.add(new mitemimage(
                                lastmenu, newstring(name), newstring(text),
...
void menuitemmapload(char* name, char* text)
{
    ...
    if (!text || text[0] == '\0')
        ...
        formatstring(caction)("%s", text);
    lastmenu->items.add(new mitemmapload(lastmenu, newstring(name),
                                         newstring(name), newstring(caction),
                                         NULL, NULL, NULL));
}
...
void menuitemslider(char* text, int* min_, int* max_, char* value, int* step,
                    char* display, char* action)
{
    if (!lastmenu)
        return;
    lastmenu->items.add(new mitemslider(
                            lastmenu, newstring(text), *min_, *max_, *step, newstring(value),
...
void menuitemkeyinput(char* text, char* bindcmd)
{
    if (!lastmenu)
        return;
    lastmenu->items.add(
        new mitemkeyinput(lastmenu, newstring(text), newstring(bindcmd), NULL));
}
...
{
    if (!r[0] || !col)
        return false; // four possible parameter schemes:
    if (!g[0])
        g = b = r; // grey
    if (!b[0]) {
        a = g; // grey alpha
...
bool menukey(int code, bool isdown, int unicode, SDLMod mod)
{
    ...
    if (isdown) {
        bool hasdesc = false;
        loopv(curmenu->items) if (curmenu->items[i]->getdesc()) {
            hasdesc = true;
            break;
        }
        // int pagesize = MAXMENU - (curmenu->header ? 2 : 0) - (curmenu->footer
        // || hasdesc ? 2 : 0); // FIXME: footer-length
        int pagesize = MAXMENU - (curmenu->header ? 2 : 0)
                       - (curmenu->footer
        ...
        if (curmenu->items.inrange(menusel)) {
            ...
            if (m->mitemtype == mitem::TYPE_KEYINPUT
                && ((mitemkeyinput*)m)->capture && code != SDLK_ESCAPE) {
        ...
        case SDLK_PAGEUP:
            ...
            break;
        case SDLK_PAGEDOWN:
            if (menusel + pagesize >= n
                && menusel / pagesize != (n - 1) / pagesize)
                menusel = n - 1;
            else
                menusel += pagesize;
            break;
        ...
        if (code == SDLK_RETURN || code == SDLK_SPACE
            ...
            if (m.getaction() != NULL && !strcmp(m.getaction(), "-1"))
                return true; // don't playsound S_MENUENTER if menuitem action
            // == -1 (null/blank/text only item) - Bukz
            // 2013feb13
...
void rendermenumdl()
{
    ...
    bool isplayermodel
        = !strncmp(m.mdl, "playermodels", strlen("playermodels"));
    ...
    float yaw = 1.0f;
    if (m.rotspeed)
        yaw += lastmillis / 5.0f / 100.0f * m.rotspeed;
...
void gmenu::refresh()
{
    ...
    if (menusel >= items.length())
        menusel = max(items.length() - 1, 0);
}
...
{
    if (items.inrange(menusel))
        items[menusel]->focus(false);
}

void gmenu::conprintmenu()
{
    if (title)
        conoutf("[::  %s  ::]", title); // if(items.length()) conoutf( " %03d
    // items", items.length());
    loopv(items) {
...
const char* menufilesortorders[]
    = { "normal", "reverse", "ignorecase", "ignorecase_reverse", "" };
int (*menufilesortcmp[])(const char**, const char**)
= { stringsort, stringsortrev, stringsortignorecase,
    stringsortignorecaserev
  };
    ...
    if (dirlist && dirlist->dir && dirlist->ext) {
        ...
        loopv(files) {
            ...
            if (dirlist->image) {
                ...
                // items.add(new mitemimage(this, newstring(jpgname), f,
                ...
                if (dirlist->dir[0]) {
                    ...
                    if (strlen(dirlist->dir) > ddsl) {
                        ...
                        if (!strcmp(prefix, "packages/"))
                            diroffset = ddsl;
                    }
                }
                defformatstring(fullname)(
                    "%s%s%s", dirlist->dir[0] ? dirlist->dir + diroffset : "",
                    dirlist->dir[0] ? "/" : "", f);
                defformatstring(title)("%s", f);
                items.add(new mitemmapload(
                              this, newstring(fullname), newstring(title),
                              newstring(dirlist->action), NULL, NULL, NULL));
            } else
                items.add(new mitemtext(this, f, newstring(dirlist->action),
                                        NULL, NULL, d));
        }
    }
...
void gmenu::render()
{
    ...
    // int hitems = (header ? 2 : 0) + (footer || hasdesc ? 2 : 0), // FIXME:
    // footer-length
    int hitems = (header ? 2 : 0)
                 + (footer ? (footlen ? (footlen + 1) : 2) : (hasdesc ? 2 : 0)),
                     pagesize = MAXMENU - hitems, offset = menusel - (menusel % pagesize),
...
void gmenu::renderbg(int x1, int y1, int x2, int y2, bool border)
{
    ...
    if (!tex)
        tex = textureload("packages/textures/makke/menu.jpg");
...
void refreshapplymenu(void* menu, bool init)
{
    ...
    m->items.add(new mitemtext(
                     m, newstring("No"),
                     newstring("echo [..restart AssaultCube to apply the new settings]"),
                     NULL, NULL));
    if (init)
