...
void addpckserver(char *host, char *priority) // add/adjust a package server
{
    ...
    if (*host)
    {
        ...
        if (!s)
            s = pckservers.add(new pckserver);
        copystring(s->host, host);
        if (*priority)
            s->priority = atoi(priority);
    }
    sem_pckservers.post();
}
...
void getpckserver() // return a table of all package servers with four columns
{
    ...
    if (sem_pckservers.timedwait(1000))
        cvecprintf(res, "\"serverlist is busy, try again later\" "); // single-word return value signals an error
    else
    {
            ...
            if (h.response == 200)
            {
                ...
                else
                {
                    ...
                    if (DEBUGCOND)
                    {
                        ...
                        cvecprintf(pckping_log, "lheader: %s", escapestring(h.header ? h.header : h.rawrec.getbuf(), true, true));
                        pckping_log.add('\n');
                    }
#endif
                    SDL_mutexV(pckpinglog_lock);
...
bool requirepackage(int type, const char *name, const char *host)
{
    ...
    if (forceext && strlen(pck->name) > strlen(forceext) && !strcmp(pck->name + strlen(pck->name) - strlen(forceext), forceext))
    ...
    pck->type = type;
    if (host)
        copystring(pck->host, host); // assign fixed server
        ...
        if (mz)
        {
            ...
            loopv(files)
            {
                ...
                if (isok && !strcmp(cleanzname, zname))
                {
                    ...
                    {
                        DEBUGCODE(int got =)
                        zipmanualread(mz, i, zf, MAXMEDIADOWNLOADFILESIZE); // extract file from zip and write to @filename
    ...
    if (*u.domain && h.set_host(u.domain))
    {
        ...
        if (got < 0 || !h.response)
        {
            if (!canceldownloads && s)
                s->ping = 0; // received a hard error from the server connection, better not try this one again
        ...
        else
        {
            if (h.response == 200)
            {
                ...
                clientlogf("download %s:%d%s failed, server response %d%s%s", u.domain, h.ip.port, url, h.response, h.err ? ", err: " : "", h.err ? h.err : "");
        }
        DELETEP(h.outstream);
    }
    else
...
int pckserversort(pckserver **a, pckserver **b)
{
    ...
    if ((*a)->priority != (*b)->priority)
        return (*b)->priority - (*a)->priority; // manual priority wins over ping (high priority to the top)
    if ((*a)->ping && (*b)->ping)
...
int downloadpackages(bool loadscr) // get all pending packages
{
    ...
    loopvj(pckservers)
    {
        ...
        loopv(pendingpackages)
        {
            ...
            if (!*pck->host)
            {
                if (dlpackage(h, pck, s))
                    delete pendingpackages.remove(i--);
            }
            if (!s->resolved || !s->ping)
                break;
        }
    }
    if (canceldownloads)
        pendingpackages.deletecontents();
