...
struct tristrip {
    ...
    template <class T> void addtriangles(const T* tris, int numtris) {
        if (dbgts)
            conoutf("addtriangles: tris = %d, inds = %d", numtris, numtris * 3);
        loopi(numtris) {
    ...
    void findconnectivity() {
        ...
        if (dbgts)
            conoutf("no connections: %d", connectivity[0].length());
    }
    ...
    ushort nexttriangle(triangle& tri, bool& nextswap, ushort v1 = UNUSED,
                        ushort v2 = UNUSED) {
        ushort next = UNUSED;
        int nextscore = 777;
        nextswap = false;
    ...
    void buildstrips(vector<ushort>& strips, vector<drawcall>& draws,
                     bool prims = true, bool degen = false) {
