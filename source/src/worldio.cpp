...
const char* setnames(const char* name)
{
    ...
    formatstring(cbakname)("packages/%s/%s.cfg_%s.BAK", pakname, mapname,
                           numtime());
...
// messy mapping by
// setting certain properties (vdeltas and textures) to neighbouring values
// wherever there is no
// visible difference. This allows the mipmapper to generate more efficient
// mips.
// the reason it is done on save is to reduce the amount spend in the mipmapper
// (as that is done
// in realtime).
...
// run-length
// encoding and leaves out data for certain kinds of cubes, then zlib removes
// the
// last bits of redundancy. Both passes contribute greatly to the miniscule map
// sizes.

void rlencodecubes(vector<uchar>& f, sqr* s, int len,
                   bool preservesolids) // run-length encoding and serialisation
...
void rldecodecubes(ucharbuf& f, sqr* s, int len, int version,
...
void parseheaderextra(bool clearnonpersist = true,
                      int ignoretypes = 0) // parse all headerextra packets,
// delete the nonpersistent ones
// (like editundos)
{
    ...
    loopv(headerextras) {
        ...
        bool deletethis = false; // (set deletethis for headers that persist
        // outside headerextras and get reinserted by
        // packheaderextras())
...
bool embedconfigfile()
{
    ...
    if (buf) {
        for (int n; (n = findheaderextra(HX_CONFIG)) >= 0;)
            deleteheaderextra(n);
        headerextras.add(new headerextra(len + 1, HX_CONFIG | HX_FLAG_PERSIST,
                                         NULL))->data = buf;
...
void extractconfigfile()
{
    ...
    if (n < 0)
        ...
        backup(mcfname, cbakname); // don't overwrite an existing config file,
        // just rename it
        stream* f = openfile(mcfname, "w");
        if (f) {
...
{
    if (noteditmode("setvantagepoint") || multiplayer())
        return;
    clearvantagepoint();
    short p[5];
    storeposition(p);
...
bool gotovantagepoint()
{
    ...
    if (n >= 0) {
        ...
        if (collide(&d, false))
            n = -1; // don't use out-of-map vantage points
    }
    if (n < 0) // if there is no vantage point set, we go to the first
        // playerstart instead
    {
...
void save_world(char* mname, bool skipoptimise, bool addcomfort)
{
    ...
    if (!validmapname(mname)) {
        conoutf("\f3Invalid map name. It must only contain letters, digits, "
                "'-', '_' and be less than %d characters long",
                MAXMAPNAMELEN);
        return;
    }
    if (!skipoptimise) {
    ...
    ucharbuf hx = packheaderextras(
                      addcomfort ? 0
                      : (1 << HX_EDITUNDO)); // if addcomfort -> add undos/redos
    int writeextra = 0;
    if (hx.maxlen)
        tmp.headersize += writeextra = clamp(hx.maxlen, 0, MAXHEADEREXTRA);
    if (writeextra || skipoptimise)
        tmp.version = 10; // 9 and 10 are the same, but in 10 the headersize is
    // reliable - if we don't need it, stick to 9
...
bool load_world(char* mname) // still supports all map formats that have existed
// since the earliest cube betas!
{
    ...
    loopi(hdr.numents) {
        ...
        if (hdr.version < 10) {
            ...
            case MAPMODEL:
                ...
                e.attr1 = (e.attr1 % 360) * 4; // cut down to less that one
                // rotation and set resolution to
                // 0.25 degree
                break;
            }
        }
    ...
    loopi(256) if (texuse[i])
        lookupworldtexture(i, autodownload ? true : false);
    ...
    if (downloaded > 0)
        clientlogf("downloaded content (%d KB in %d seconds)",
                   downloaded / 1024, watch.elapsed() / 1000);
...
struct xmap {
    ...
    xmap(const char* nnick) { // take the current map and store it in an xmap
        ...
        vector<uchar> tmp; // add undo/redo data in compressed form as temporary
        // headerextra
    ...
    void write(stream* f) { // write xmap as cubescript to a file
        f->printf("restorexmap version %d %d %d\n", XMAPVERSION,
                  isbigendian() ? 1 : 0, sizeof(world)); // it is a non-portable
        // binary file format,
        // so we have to
        // check...
        ...
        uLongf worldsize = cubicsize * sizeof(sqr),
               gzbufsize = (worldsize * 11) / 10; // gzip the world, so the file
        // will only be big instead of
        // huge
        uchar* gzbuf = new uchar[gzbufsize];
        ...
        loopv(ents) { // entities are stored as plain text - you may edit them
            ...
            f->printf("restorexmap ent %d  %d %d %d  %d %d %d %d // %s\n",
                      e.type, e.x, e.y, e.z, e.attr1, e.attr2, e.attr3, e.attr4,
...
#define SPEDIT                                \
    if (noteditmode("xmap") || multiplayer()) \
    return // only allowed in non-coop editmode
#define SPEDITDIFF                                        \
    if (noteditmode("xmap") || multiplayer() || nodiff()) \
    return // only allowed in non-coop editmode after xmap_diff
    ...
    const char* nick) // throw away existing backup and make a new one
{
    ...
    bak = new xmap(text);
    if (unsavededits)
        conoutf("\f3stored backup of unsaved edits on map '%s'; to restore, "
                "type \"/xmap_restore\"",
                bak->name);
}
...
const char* xmapdescstring(xmap* xm, bool shortform = false)
{
    ...
    formatstring(s[toggle])(
        "\"%s\": %s rev %d, %d ents%c size %d, hdrs %d, %d undo steps",
        xm->nick, xm->name, xm->hdr.maprevision, xm->ents.length(),
        shortform ? '\0' : ',', xm->hdr.sfactor, xm->headerextras.length() - 1,
        xm->numundo);
    return s[toggle];
}
         ...
         (const char* nick) // store current map in an xmap buffer
{
    ...
    if (getxmapbynick(nick, &i, false)) {
        xmap* xm = xmaps.remove(i); // overwrite existing same nick
        xmapdelete(xm);
    }
                 ...
                 int numargs) // read an xmap from a cubescript file
{
    const char* cmdnames[] = { "version", "names",       "sizes", "header",
                               "world",   "headerextra", "ent",   "position"
                             };
    ...
    bool abort = false;
    int cmd = -1;
    loopi(sizeof(cmdnames)
          / sizeof(cmdnames[0])) if (!strcmp(cmdnames[i], args[0])) {
        cmd = i;
        break;
    }
    ...
    case 4: { // world
        ...
        if (uncompress((uchar*)xmjigsaw->world, &rawsize, hexbin.getbuf(),
                       hexbin.length()) != Z_OK || rawsize - rawworldsize != 0)
            abort = true;
        break;
    }
    case 5: // headerextra
...
int loadallxmaps() // at game start, load all xmaps from mapediting/xmaps
{
    ...
    loopv(xmapnames) {
        ...
        backup(filename,
               xmapfilename(xmapnames[i], "backups/")); // move to backup folder
        // - we will write new
        // files at exit
        delete[] xmapnames[i];
    }
