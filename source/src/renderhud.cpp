...
void drawequipicon(float x, float y, int col, int row)
{
    ...
    if (!tex)
        tex = textureload("packages/misc/items.png", 4);
    if (tex) {
...
void drawradaricon(float x, float y, float s, int col, int row)
{
    static Texture* tex = NULL;
    if (!tex)
        tex = textureload("packages/misc/radaricons.png", 3);
    if (tex) {
        glEnable(GL_BLEND);
                 ...
                 int alpha)
{
    ...
    if (!ctftex)
        ctftex = textureload("packages/misc/ctficons.png", 3);
    if (!htftex)
    ...
    if (m_htf) {
        if (htftex)
            drawicon(htftex, x, y, s, col, row, ts);
    } else if (m_ktf) {
        if (ktftex)
            drawicon(ktftex, x, y, s, col, row, ts);
    } else {
        if (ctftex)
            drawicon(ctftex, x, y, s, col, row, ts);
    }
}
...
void drawvoteicon(float x, float y, int col, int row, bool noblend)
{
    static Texture* tex = NULL;
    if (!tex)
        tex = textureload("packages/misc/voteicons.png", 3);
    if (tex) {
        if (noblend)
            glDisable(GL_BLEND);
        else
...
void drawsgpat(int w, int h)
{
    ...
    loopi(8+1)
    {
        // if((i%3)==0) { glColor3ub(rgbcv,rgbcv,rgbcv); rgbcv += 4; //rgbcv -=
255/(8+1); }
        if(i%2) glColor3ub(64,64,64); else glColor3ub(32,32,32);
        float c = 0.5f*(1 + border*cosf(i*2*M_PI/8.0f)), s = 0.5f*(1 +
border*sinf(i*2*M_PI/8.0f));
    ...
    loopi(8+1)
    {
        // if((i%3)==0) { glColor3ub(rgbcv,rgbcv,rgbcv); //,128); rgbcv += 8;
//rgbcv -= 255/(8+1); }
...
border*sinf(i*2*M_PI/8.0f));
    ...
    glEnable(GL_BLEND);
    /\*
     // 2011may31: dmg/hits output comes upon each shot, let the pattern be
shown "pure"
     extern int lastsgs_hits;
     extern int lastsgs_dmgt;
     //draw_textf("H: %d DMG: %d", 8, 32, lastsgs_hits, lastsgs_dmgt);
     defformatstring(t2show4hitdmg)("H: %d DMG: %d", lastsgs_hits,
lastsgs_dmgt);
     draw_text(t2show4hitdmg, VIRTW/2-text_width(t2show4hitdmg),
VIRTH/2-3*FONTH/4);
     *\/
}
*/
...
void drawscope(bool preload)
{
    ...
    if (!scopetex)
        scopetex = textureload("packages/misc/scope.png", 3);
    if (preload)
        return;
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    float sz = min(VIRTW, VIRTH), x1 = VIRTW / 2 - sz / 2,
    ...
    glVertex2f(x1 + 0.5f * sz, y1 + 0.5f * sz);
    loopi(8 + 1) {
        float c = 0.5f * (1 + border * cosf(i * 2 * M_PI / 8.0f)),
              s = 0.5f * (1 + border * sinf(i * 2 * M_PI / 8.0f));
        glTexCoord2f(c, s);
        glVertex2f(x1 + c * sz, y1 + s * sz);
    }
    glEnd();
...
void loadcrosshair(char* type, char* filename)
{
    ...
    if (index == CROSSHAIR_DEFAULT) {
        loopi(CROSSHAIR_NUM) if (i != CROSSHAIR_TEAMMATE
                                 && i != CROSSHAIR_SCOPE) crosshairs[i]
...
void drawcrosshair(playerent* p, int n, color* c, float size)
{
    ...
    float chsize
        = s * ((p->weaponsel->type == GUN_ASSAULT && p->weaponsel->shots > 3)
               && (crosshairfx == 1 || crosshairfx == 3)
               ? 1.4f
               : 1.0f) * (n == CROSSHAIR_TEAMMATE ? 2.0f : 1.0f);
    glBegin(GL_TRIANGLE_STRIP);
    glTexCoord2f(0, 0);
...
{
    if (hidedamageindicator || !damageindicatorsize)
        return;
    float bestdist = 0.0f;
    int bestdir = -1;
    loopi(4) {
...
void drawequipicons(playerent* p)
{
    ...
    glColor4f(1.0f, 1.0f, 1.0f,
              0.2f + (sinf(lastmillis / 100.0f) + 1.0f) / 2.0f);
    ...
    if (p->mag[GUN_GRENADE])
        drawequipicon(oldfashionedgunstats ? (HUDPOS_GRENADE + 25) * 2
                      : HUDPOS_GRENADE * 2,
                      1650, 3, 1);
...
void drawradarent(float x, float y, float yaw, int col, int row, float iconsize,
                  bool pulse, const char* label = NULL, ...)
{
    glPushMatrix();
    if (pulse)
        glColor4f(1.0f, 1.0f, 1.0f,
...
void drawradar_showmap(playerent* p, int w, int h)
{
    ...
    if (showmapbackdrop) {
        ...
        loopi(2) {
            int cg = i ? (showmapbackdrop == 2
                          ? ((int)(255 * (100 - showmapbackdroptransparency)
                                   / 100.0f))
                          : 0)
                         : (showmapbackdrop == 2
                            ? ((int)(255 * (100 - showmapbackdroptransparency)
    ...
    if (team_isactive(p->team))
        drawradarent(ppv.x, ppv.y, p->yaw,
                     ...
                     iconsize, isattacking(p), "%s",
                     colorname(p)); // local player
    loopv(players) { // other players
        playerent* pl = players[i];
        if (!pl || pl == p || !isteam(p->team, pl->team)
            || !team_isactive(pl->team))
            continue;
        ...
        loopi(2) { // flag items
            ...
            if (f.state != CTFF_STOLEN && !(m_ktf && f.state == CTFF_IDLE)) {
                ...
                drawradarent(cpos.x + flgoff, cpos.y - flgoff, 0, 3,
                             ...
                             false); // draw on entity pos
            }
            if (m_ktf && f.state == CTFF_IDLE)
                continue;
...
void drawradar_vicinity(playerent* p, int w, int h)
{
    ...
    if (team_isactive(p->team))
        drawradarent(
            0, 0, p->yaw, p->state == CS_ALIVE ? (isattacking(p) ? 2 : 0) : 1,
            2, iconsize, isattacking(p), "%s", colorname(p)); // local player
    loopv(players) { // other players
        playerent* pl = players[i];
        if (!pl || pl == p || !isteam(p->team, pl->team)
            || !team_isactive(pl->team))
    ...
    if (m_flags) {
        ...
        loopi(2) { // flag items
            ...
            // if(showradarvalues) { conoutf("dist2F[%d]: %.2f|%.2f ||
            // %.2f|%.2f", i, pos.x, pos.y, cpos.x, cpos.y); }
            if (pos.magnitude() < d2s) {
                pos.mul(scaled);
                drawradarent(
                    pos.x, pos.y, 0, m_ktf ? 2 : f.team, 3, iconsize,
...
void drawteamicons(int w, int h, bool spect)
{
    ...
    if (!icons)
        icons = textureload("packages/misc/teamicons.png", 3);
...
void drawscores()
{
    ...
    static Texture* tex = NULL;
    if (!tex)
        tex = textureload("packages/misc/nice_medals.png", 4);
                ...
                bool underwater)
{
    ...
    bool reloading = lastmillis < p->weaponsel->reloading
                     + p->weaponsel->info.reloadtime;
    ...
    if (/*!menu &&*/ (!hideradar || showmap))
        drawradar(p, w, h);
    // if(showsgpat) drawsgpat(w,h); // shotty
    if (!editmode) {
        glMatrixMode(GL_MODELVIEW);
        if (!hideteam && m_teammode)
            drawteamicons(w, h, is_spect);
        glMatrixMode(GL_PROJECTION);
    }
    ...
    if (!hideconsole)
        renderconsole();
    formatstring(enginestateinfo)("%d %d %d %d %d", curfps, lod_factor(),
                                  nquads, curvert, xtraverts);
    if (showstats) {
        if (showstats == 2) {
...
#define TXTCOLRGB    \
    ...
    default:         \
        ...
        break;       \
            ...
            c_num = curvert > 3999
                    ? (curvert > 5999 ? (curvert > 7999 ? 3 : 2) : 1)
                    : 0;
            TXTCOLRGB
            ...
            c_num = xtraverts > 3999
                    ? (xtraverts > 5999 ? (xtraverts > 7999 ? 3 : 2) : 1)
                    : 0;
            TXTCOLRGB
    ...
    if (!intermission && lastgametimeupdate != 0) {
        ...
        if (clockdisplay)
            draw_text(gtime, (VIRTW - 225 - 10) * 2
                      - (text_width(gtime) / 2 + FONTH / 2),
                      20);
    }
    ...
    if (hidevote < 2 && multiplayer(false)) {
        ...
        if (curvote && curvote->millis >= totalmillis
            ...
            default:
                ...
                formatstring(str)("\f3vote %s", curvote->result == VOTE_YES
                                  ? "PASSED"
                                  : "FAILED");
    ...
    if (!hidespecthud && !menu && p->state == CS_DEAD
        ...
        if (!m_botmode)
            draw_textf("SCROLL to change player",
                       left
                       - (text_width("SCROLL to change player") + FONTH / 2),
                       top + 80);
    }
    ...
    if (p->state == CS_ALIVE) {
        ...
        if (!hidehudequipment) {
            pushfont("huddigits");
            draw_textf("%d", HUDPOS_HEALTH + HUDPOS_NUMBERSPACING, 823,
                       p->health);
            if (p->armour)
                draw_textf("%d", HUDPOS_ARMOUR + HUDPOS_NUMBERSPACING, 823,
                           p->armour);
            if (p->weaponsel && p->weaponsel->type >= GUN_KNIFE
                && p->weaponsel->type < NUMGUNS) {
                glMatrixMode(GL_MODELVIEW);
                if (p->weaponsel->type != GUN_GRENADE)
                    ...
                    p->weapons[p->akimbo ? GUN_AKIMBO : GUN_PISTOL]
                    ->renderstats();
                else
        ...
        if (m_flags && !hidectfhud) {
            ...
            if ((flaginfos[0].state == CTFF_STOLEN && flaginfos[0].actor == p
                ...
                drawctficon(VIRTW - 225 - 10, VIRTH * 5 / 8, 225, ft, 1,
                            1 / 2.0f, (sinf(lastmillis / 100.0f) + 1.0f) * 128);
            }
        }
    }
...
void loadingscreen(const char* fmt, ...)
{
    ...
    if (!logo)
        logo = textureload("packages/misc/startscreen.png", 3);
...
static void bar(float bar, int o, float r, float g, float b)
{
    ...
    glBegin(GL_TRIANGLE_STRIP);
    loopk(10) {
        float c = 1.2f * cosf(M_PI / 2 + k / 9.0f * M_PI),
              s = 1 + 1.2f * sinf(M_PI / 2 + k / 9.0f * M_PI);
    ...
    glBegin(GL_TRIANGLE_STRIP);
    loopk(10) {
        float c = cosf(M_PI / 2 + k / 9.0f * M_PI),
              s = 1 + sinf(M_PI / 2 + k / 9.0f * M_PI);
        glVertex2f(x2 - c * FONTH, y1 + s * FONTH);
        glVertex2f(x1 + c * FONTH, y1 + s * FONTH);
    }
    glEnd();
    ...
    const char* text2) // also used during loading
{
    ...
    if (text1)
        draw_text(text1, 2 * FONTH, 1 * FONTH + FONTH / 2);
    if (bar2 > 0)
        draw_text(text2, 2 * FONTH, 3 * FONTH + FONTH / 2);
