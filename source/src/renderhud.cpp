...
void drawequipicon(float x, float y, int col, int row)
{
    ...
    if (!tex)
        tex = textureload("packages/misc/items.png", 3);
    if (tex)
    {
...
void drawradar_showmap(playerent *p, int w, int h)
{
    ...
    if (!(p->isspectating() && spect3rd))
        drawradarent(ppv.x, ppv.y, p->yaw, (p->state == CS_ALIVE || p->state == CS_EDITING) ? (isattacking(p) ? 2 : 0) : 1, 2, iconsize, isattacking(p), "%s", colorname(p)); // local player
    loopv(players)                                                                                                                                                            // other players
    {
        playerent *pl = players[i];
        if (!pl || pl == p || !team_isactive(pl->team))
            continue;
        if (OUTBORD(pl->o.x, pl->o.y))
            continue;
        int pl_baseteam = team_base(pl->team);
        if (p->team < TEAM_SPECT && ((m_teammode && !isteam(p_baseteam, pl_baseteam)) || (!m_teammode && !(spect3rd && d == pl))))
            continue;
...
void drawradar_vicinity(playerent *p, int w, int h)
{
    ...
    if (!(p->isspectating() && spect3rd))
        drawradarent(0, 0, p->yaw, (p->state == CS_ALIVE || p->state == CS_EDITING) ? (isattacking(p) ? 2 : 0) : 1, 2, iconsize, isattacking(p), "%s", colorname(p)); // local player
    loopv(players)                                                                                                                                                    // other players
    {
        playerent *pl = players[i];
        if (!pl || pl == p || !team_isactive(pl->team))
...
COMMANDF(
    getEngineState, "", () { result(enginestateinfo); });
...
void gl_drawhud(int w, int h, int curfps, int nquads, int curvert, bool underwater, int elapsed)
{
    ...
    if (command)
        commandh -= rendercommand(-1, HUDPOS_Y_BOTTOMLEFT, VIRTW - FONTH); // dryrun to get height
    else if (infostr)
    ...
    if (editmode && !hideeditinfopanel)
    {
        ...
        if (dimeditinfopanel > 255)
            dimeditinfopanel = 255;
    }
