...
static int scorecmp(playerent** x, playerent** y)
{
    ...
    if ((*x)->deaths < (*y)->deaths)
        return -1;
    if ((*x)->lifesequence > (*y)->lifesequence)
        return 1;
    if ((*x)->lifesequence < (*y)->lifesequence)
        return -1;
    return 0;
...
void renderdiscscores(int team)
{
    loopv(discscores) if (team == team_group(discscores[i].team)) {
        ...
        if (m_flags)
            line.addcol(sc_flags, "%d", d.flags);
...
void renderscore(playerent* d)
{
    ...
    if (team_isspect(d->team))
        ...
        if (multiplayer(false))
            formatstring(lagping)("%s/%s", colorpj(d->plag),
                                  colorping(d->ping));
        else
    ...
    if (m_flags)
        line.addcol(sc_flags, "%d", d->flagscore);
    line.addcol(sc_frags, "%d", d->frags);
    line.addcol(sc_deaths, "%d", d->deaths);
    line.addcol(sc_ratio, "%.2f", SCORERATIO(d->frags, d->deaths));
    if (multiplayer(false) || watchingdemo) {
...
int renderteamscore(teamscore* t)
{
    ...
    if (m_flags)
        line.addcol(sc_flags, "%d", t->flagscore);
    ...
    static color teamcolors[2]
        = { color(1.0f, 0, 0, 0.2f), color(0, 0, 1.0f, 0.2f) };
...
void renderscores(void* menu, bool init)
{
    ...
    if (m_teammode) {
        ...
        winner = m_flags ? (teamscores[sort].flagscore
                            > teamscores[team_opposite(sort)].flagscore
                            ? sort
                            : -1)
                     : (teamscores[sort].frags
                        > teamscores[team_opposite(sort)].frags
                        ? sort
                        : -1);
    ...
    if ((gamemode > 1
         || (gamemode == 0 && (multiplayer(false) || watchingdemo)))
        && minutesremaining >= 0) {
        if (!minutesremaining) {
    ...
    if (multiplayer(false)) {
        ...
        if (s) {
            if (servstate.mastermode > MM_OPEN)
                concatformatstring(serverline,
                                   servstate.mastermode == MM_MATCH ? "M%d "
                                   : "P ",
                                   servstate.matchteamsize);
            // ft: 2010jun12: this can write over the menu boundary
            // concatformatstring(serverline, "%s:%d %s", s->name, s->port,
            // s->sdesc);
            // for now we'll just cut it off, same as the serverbrowser
            // but we might want to consider wrapping the bottom-line to
            // accomodate longer descriptions - to a limit.
            string text;
            filtertext(text, s->sdesc, FTXT__SERVDESC);
            // for(char *p = text; (p = strchr(p, '\"')); *p++ = ' ');
            // text[30] = '\0'; // serverbrowser has less room - +8 chars here -
            // 2010AUG03 - seems it was too much, falling back to 30 (for now):
            // TODO get real width of menu as reference-width. FIXME: cutoff
...
void addstr(char* dest, const char* src)
{
    if (strlen(dest) + strlen(src) < MAXJPGCOM)
        strcat(dest, src);
}
...
const char* asciiscores(bool destjpg)
{
    ...
    loopv(scores) {
        ...
        formatstring(team)(destjpg ? ", %s" : " %-4s",
                           team_string(d->team, true));
        formatstring(flags)(destjpg ? "%d/" : " %4d ", d->flagscore);
        if (destjpg)
            formatstring(text)("%s%s (%s%d/%d)\n", d->name,
                               m_teammode ? team : "", m_flags ? flags : "",
                               d->frags, d->deaths);
        else
            formatstring(text)(
