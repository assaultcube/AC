...
struct ident {
    ...
    union {
        void (*getfun)(); // ID_SVAR   (called /before/ reading the value
                          // string, as a chance for last-minute updates)
    ...
    bool
        istemp; // ID_ALIAS: if value is changed, "persist" will not be set true

    ident() {
    }

    // ID_VAR
    ident(int type, const char* name, int minval, int maxval, int* i,
          int defval, void (*fun)(), bool persist, int context)
        : type(type)
        , name(name)
        , minval(minval)
        , maxval(maxval)
    ...
    // ID_COMMAND
    ident(int type, const char* name, void (*fun)(), const char* sig,
          int context)
        : type(type)
        , name(name)
        , minval(0)
...
#define COMMANDN(name, fun, sig) \
    static bool __dummy_##fun = addcommand(#name, (void (*)())fun, sig)
