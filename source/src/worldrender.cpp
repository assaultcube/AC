...
// be rendered and how (depending on neighbouring cubes), then calls functions
// in rendercubes.cpp
...
// call itself for lower mip levels, on most modern machines however this
// function will use the higher
                    ...
                    int ys)
{
    ...
    for (int oy = y; oy < ys;
        ...
        for (int ox = x; ox < xs; ox++, s++)
            s->occluded
                = isoccluded(camera1->o.x, camera1->o.y, (float)(ox << mip),
                             (float)(oy << mip), fsize);
    }
...
// loop through the rect 3 times (for floor/ceil/walls seperately, to facilitate
// dynamic stripify)
// for each we skip occluded cubes (occlusion at higher mip levels is a big time
// saver!).
// during the first loop (ceil) we collect cubes that lie within the lower mip
// rect and are
// also deferred, and render them recursively. Anything left (perfect mips and
// higher lods) we
// render here.
...
#define LOOPD                       \
    ...
    if (s->defer && mip && xx >= lx && xx < rx && yy >= ly && yy < ry) {
        ...
        while (xx < xs - 1 && s[1].defer && !s[1].occluded) {
            xx++; // collect 2xN rect of lower mip
            s++;
            if (xx < rx)
                s->occluded = 1;
        }
    ...
    case CHF:
        if (s->floor <= vh && render_floor) {
            sqr* v = SWS(s, 0, 1, mfactor);
            render_flat(s->ftex, xx << mip, yy << mip, 1 << mip, s->floor, s,
                        s + 1, v + 1, v, false);
...
if (s->type == CORNER)
{
    ...
    if (SOLID(z)) {
        ...
        if (z->ceil - z->floor < t->ceil - t->floor) {
            if (wv) {
                render_wall(h1 = s, h2 = v, xx + 1, yy, xx, yy + 1, mip, t, v,
                            false, 4);
                topleft = false;
            } else {
                render_wall(h1 = s, h2 = w, xx, yy, xx + 1, yy + 1, mip, s, u,
                            false, 5);
            }
        } else {
            if (wv) {
                render_wall(h2 = s, h1 = v, xx + 1, yy + 1, xx, yy, mip, u, s,
                            false, 6);
            } else {
                render_wall(h2 = s, h1 = w, xx, yy + 1, xx + 1, yy, mip, v, t,
                            false, 7);
                topleft = false;
            }
        }
