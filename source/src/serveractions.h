  ...
  mapaction(char *map, int mode, int time, int caller, bool q)
    ...
    if (isdedicated) {
      ...
      if (!mapok) {
        ...
        bool flags =
            mode != GMODE_COOPEDIT && m_flags && !m_htf ? ms->hasflags : true;
        if (!spawns || !flags || romap) {  // unsupported mode
          if (strchr(scl.voteperm, 'P'))
            ...
            mapok = false;  // default: no one can vote for unsupported mode/map
                            // combinations
          defformatstring(msg)("\f3map \"%s\" does not support \"%s\": ",
      ...
      mapok = mp == MAP_LOCAL ||
              mp == MAP_OFFICIAL;  // clients can only load from packages/maps
...
struct forceteamaction : playeraction {
  ...
  forceteamaction(int cn, int caller, int team) : playeraction(cn), team(team) {
    ...
    if (isvalid() &&
        !(clients[cn]->state.forced && clients[caller]->role != CR_ADMIN))
