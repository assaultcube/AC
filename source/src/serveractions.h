        ...
        if (isdedicated)
        {
            ...
            if (!mapok)
            {
                if (notify)
                {
                    ...
                    {
                        sendservmsg(ms ? (mode == GMODE_COOPEDIT ? "this map cannot be coopedited in this server" : "sorry, but this map does not satisfy some quality requisites to be played in MultiPlayer Mode") : "the server does not have this map",
                                    caller);
                    }
                }
            }
            ...
            { // check, if map supports mode
                ...
                { // unsupported mode
                    if (strchr(scl.voteperm, 'P'))
                        ...
                        mapok = false; // default: no one can vote for unsupported mode/map combinations
