...
void transformoldentitytypes(int mapversion, uchar &enttype)
{
    ...
    if (mapversion < 8 && enttype < 18)
        enttype = usetab[enttype * 2];
}
        ...
        for (int x = 0; x < _ssize; x++)
        {
            if ((s->type & TAGTRIGGERMASK) != SOLID)
            {
                ...
                if (y < md.y1)
                    md.y1 = y;
                md.y2 = y;
                if (s->floor < md.minfloor)
                    md.minfloor = s->floor;
                if (s->ceil > md.maxceil)
                    md.maxceil = s->ceil;
            }
            s++;
        }
    ...
    for (int j = md.yspan; j > 0; j--, ss += linegap)
        loopirev(md.xspan)
        {
            ...
            if (type == FHF || type == CHF) // only CHF and FHF use vdelta
            {
                ...
                ASSERT(d >= 0);
                if (d >= MAS_VDELTA_TABSIZE)
                    d = MAS_VDELTA_TABSIZE - 1;
                ms.vdd[d]++;
            }
            ss++;
        }
...
#define RAYS(da, db)                          \
    ...
    for (;;)                                  \
    {                                         \
        if ((frac += k) >= MAS_RESOLUTION)    \
            rr += da, frac -= MAS_RESOLUTION; \
        rr += db;                             \
        if (SOLID(rr))                        \
            break;                            \
        if (rr->vdelta != epoch)              \
...
mapstats *loadmapstats(const char *filename, bool getlayout)
{
    ...
    if (f->read(&s.hdr, sizeof_baseheader) != sizeof_baseheader || (strncmp(s.hdr.head, "CUBE", 4) && strncmp(s.hdr.head, "ACMP", 4)))
    {
        delete f;
        return NULL;
    }
...
#if !defined(STANDALONE)
        if (clientlogfile)
            clientlogfile->printf("stacktrace\n");
#endif
        const int BTSIZE = 25;
        void *array[BTSIZE];
...
{
    if (data)
        SDL_DestroySemaphore((SDL_sem *)data);
}

void sl_semaphore::wait()
{
    if (SDL_SemWait((SDL_sem *)data) != 0)
        (*errorcount)++;
}
...
sl_semaphore::sl_semaphore(int init, int *ecnt)
{
    errorcount = ecnt ? ecnt : &sl_sem_errorcountdummy;
    data = (void *)new sem_t;
    if (data == NULL || sem_init((sem_t *)data, 0, init) != 0)
        (*errorcount)++;
}
...
{
    int ret;
    if (sem_getvalue((sem_t *)data, &ret) != 0)
        (*errorcount)++;
    return ret;
}
...
void sl_detachthread(void *ti) // SDL can't actually detach threads, so this is manual cleanup
{
    ...
    if (ti || sem_oldthreads.getvalue() > 0)
    {
        sem_oldthreads.wait();
        if (ti)
            oldthreads.add((sl_threadinfo *)ti);
        loopvrev(oldthreads)
        {
...
{
    if (ti)
        pthread_detach(((sl_threadinfo *)ti)->handle);
    if (ti || sem_oldthreads.getvalue() > 0)
    {
        sem_oldthreads.wait();
        if (ti)
