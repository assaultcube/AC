...
int fixmapheadersize(int version, int headersize) // we can't trust
// hdr.headersize for file
...
void stackdumper(unsigned int type, EXCEPTION_POINTERS* ep)
{
    ...
    while (::StackWalk(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(),
        ...
        DWORD off;
        if (SymGetSymFromAddr(GetCurrentProcess(), (DWORD)sf.AddrPC.Offset,
                              &off, &si.sym)
            && SymGetLineFromAddr(GetCurrentProcess(), (DWORD)sf.AddrPC.Offset,
                                  &off, &li)) {
                 ...
                 int deflt)
{
    ...
    if (*key)
        clientlogf("warning: unknown token \"%s\" (not in list [%s])", key,
                   opts);
    delstring(opts);
#endif
    return deflt;
...
{
    return (*((sl_threadinfo*)info)->fn)(((sl_threadinfo*)info)->data);
}
