...
void pushident(ident& id, char* val, int context = execcontext)
{
    ...
    stack->action = id.executing == id.action ? newstring(id.action)
                    : id.action;
...
COMMANDF(pop, "v", (char** args, int numargs)
{
    if (numargs > 0) {
        ...
        if (beforepopval)
            commandret = newstring(beforepopval);
    }
...
{
    if (!idents)
        idents = new hashtable<const char*, ident>;
    ident v(ID_VAR, name, minval, maxval, storage, cur, fun, persist,
            IEXC_CORE);
                ...
                float* storage, void (*fun)(), bool persist)
{
    if (!idents)
        idents = new hashtable<const char*, ident>;
    ident v(ID_FVAR, name, minval, maxval, storage, cur, fun, persist,
            IEXC_CORE);
    idents->access(name, v);
    return cur;
}
...
void addeq(char* name, int* arg)
{
    modifyvar(name, *arg, '+');
}
void subeq(char* name, int* arg)
{
    modifyvar(name, *arg, '-');
}
void muleq(char* name, int* arg)
{
    modifyvar(name, *arg, '*');
}
void diveq(char* name, int* arg)
{
    modifyvar(name, *arg, '/');
}
void addeqf(char* name, float* arg)
{
    modifyfvar(name, *arg, '+');
}
void subeqf(char* name, float* arg)
{
    modifyfvar(name, *arg, '-');
}
void muleqf(char* name, float* arg)
{
    modifyfvar(name, *arg, '*');
}
void diveqf(char* name, float* arg)
{
    modifyfvar(name, *arg, '/');
}
...
COMMANDF(isIdent, "s", (char* name)
{
    intret(identexists(name) ? 1 : 0);
});
...
char* parseword(const char*& p, int arg,
                int& infix) // parse single argument, including expressions
{
    p += strspn(p, " \t\r");
    if (p[0] == '/' && p[1] == '/')
        p += strcspn(p, "\n\0");
        ...
        for (bool cont = true; cont;) { // for each ; seperated statement
            ...
            if (!id) {
                ...
                case ID_COMMAND: { // game defined commands
                    ...
                    if (strstr(id->sig, "v"))
                        ((void(__cdecl*)(char**, int))id->fun)(&w[1],
                                                               numargs - 1);
...
(id->sig[i - 1] == 'i'                                         \
     ...
     : (id->sig[i - 1] == 'f' ? ((void*)&(fb##i = atof(w[i]))) \
                        ...
                        switch (strlen(id->sig)) // use very ad-hoc function
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            ((void(__cdecl*)(void*, void*))id->fun)(ARG(1),
                                                                    ARG(2));
                            break;
                        case 3:
                        ...
                        case 4:
                            ((void(__cdecl*)(void*, void*, void*,
                                             void*))id->fun)(ARG(1), ARG(2),
                                                             ARG(3), ARG(4));
                            break;
                        case 5:
                            ((void(__cdecl*)(void*, void*, void*, void*, void*))
                             id->fun)(ARG(1), ARG(2), ARG(3), ARG(4), ARG(5));
                            break;
                        case 6:
                            ((void(__cdecl*)(void*, void*, void*, void*, void*,
                                             void*))id->fun)(
                        ...
                        case 8:
                            ...
                            break;
                        default:
                            fatal("command %s has too many arguments "
                                  "(signature: %s)",
                                  id->name, id->sig);
                            break;
                        }
#undef ARG
                ...
                case ID_VAR: // game defined variables
                    if (!w[1][0])
                        conoutf("%s = %d", c, *id->storage.i); // var with no
                    // value just
                    // prints its
                    // current value
                        ...
                        if (i1 < id->minval || i1 > id->maxval) {
                            ...
                            conoutf("valid range for %s is %d..%d", id->name,
                                    id->minval, id->maxval);
                        }
                        *id->storage.i = i1;
                        if (id->fun)
                ...
                case ID_FVAR: // game defined variables
                    if (!w[1][0])
                        conoutf("%s = %s", c,
                                floatstr(*id->storage.f)); // var with no value
                    // just prints its
                    // current value
                        ...
                        if (f1 < id->minvalf || f1 > id->maxvalf) {
                            ...
                            conoutf("valid range for %s is %s..%s", id->name,
                                    floatstr(id->minvalf),
                                    floatstr(id->maxvalf));
                            // scripterr(); // Why throw this error here when
                            // it's not done for ID_VAR above? Only difference
                            // is datatype, both are "valid range errors". //
                            // Bukz 2011june04
                        }
                        *id->storage.f = f1;
                        if (id->fun)
                ...
                case ID_SVAR: // game defined variables
                    if (!w[1][0]) {
                        ...
                        conoutf(strchr(*id->storage.s, '"') ? "%s = [%s]"
                                : "%s = \"%s\"",
                                c, *id->storage.s); // var with no value just
                        // prints its current value
                    } else {
                        *id->storage.s
                            = exchangestr(*id->storage.s, newstring(w[1]));
                        if (id->fun)
                            ((void(__cdecl*)())
                             id->fun)(); // call trigger function if available
                    }
                ...
                case ID_ALIAS: // alias, also used as functions and (global)
                    // variables
                    delete[] w[0];
                    ...
                    for (int i = 1; i < numargs; i++) {
                        ...
                        pushident(*argids[i - 1], w[i]); // set any arguments as
                        // (global) arg values
                        // so functions can
                        // access them
                    }
...
// current context/gamemode
{
    ...
    if (identexists(ident)) {
        ...
        defformatstring(execbody)("%s%c%s", ident, *arglist ? ' ' : '\0',
                                  arglist);
        execute(execbody);
        return true;
    }
    ...
    char* s, bool reversedirection)   // s is required to be of size "string"!
{
    ...
    char* cmd = strrchr(s, ';'); // find last ';' (this will not always work
    // properly, because it doesn't take quoted
    // texts into account)
    if (!cmd)
        cmd = s; // no ';' found: command starts after '/'
    ...
    if (openblock && (!closeblock || closeblock < openblock))
        cmd = openblock; // found opening parenthesis inside the command:
    // assume, a new command starts here

    cmd += strspn(cmd + 1, " ") + 1; // skip blanks and one of "/;( ", cmd now
    // points to the first char of the command

    // check, if the command is complete, and we want argument completion
    // instead
    ...
    if (arg) { // full command is present
        ...
        if (completesize < 0 && cdata
            && cdata->type
            == COMPLETE_FILE) { // get directory contents on first run
...
#ifndef STANDALONE
        if (*elem == '\"')
            filterrichtext(newelem, newelem, strlen(newelem));
#endif
        elems.add(newelem);
        whitespaceskip;
    }
...
void looplist(char* list, char* varlist, char* body)
{
    ...
    bool ok = true;
    loopv(vars) {
        ok = ok && ids.add(newident(vars[i]))->type == ID_ALIAS;
    }
    if (ok) {
        vector<char*> elems;
        ...
        loop_level++;
        for (int i = 0; i <= elems.length() - vars.length();
             i += vars.length()) {
            loopvj(vars) {
                if (ids[j]->action != ids[j]->executing)
                    delete[] ids[j]->action;
...
#endif
                if (int(strlen(e)) == len && !strncmp(e, key, len))
                    return i;
                *s = ' ';
            }
...
COMMANDF(findlist, "ss", (char* s, char* key)
{
    intret(find(s, key));
});
...
void testchar(char* s, int* type)
{
    ...
    case 7:
        ...
        break;
    case 8: // Without this it is impossible to determine if a character === "
        // in cubescript
        if (!strcmp(s, "\"")) {
            istrue = true;
        }
...
char* strreplace(char* dest, const char* source, const char* search,
                 const char* replace)
{
    vector<char> buf;
...
int stringsortrev(const char** a, const char** b)
{
    return strcmp(*b, *a);
}
...
void swapelements(char* list, char* v)
{
    ...
    if (swapi.length()
        && !(swapi.length()
             & 1)) { // swap needs to have an even number of elements
...
COMMAND(format, "v");
COMMAND(result, "s");
COMMANDF(execute, "s", (char* s)
{
    intret(execute(s));
});
COMMANDF(listlen, "s", (char* l)
{
    intret(listlen(l));
});
...
COMMAND(testchar, "si");
COMMANDF(strreplace, "sss",
         (const char* source, const char* search, const char* replace)
{
    string d;
    result(strreplace(d, source, search, replace));
});
...
COMMANDN(-f, subf, "ff");
void divf_(float* a, float* b)
{
    floatret(*b ? (*a) / (*b) : 0);
}
COMMANDN(divf, divf_, "ff");
void modf_(float* a, float* b)
{
    floatret(*b ? fmod(*a, *b) : 0);
}
COMMANDN(modf, modf_, "ff");
void powf_(float* a, float* b)
{
    floatret(powf(*a, *b));
}
...
COMMANDN(>, gt, "ii");
void lte(int* a, int* b)
{
    intret((int)(*a <= *b));
}
COMMANDN(<=, lte, "ii");
void gte(int* a, int* b)
{
    intret((int)(*a >= *b));
}
COMMANDN(>=, gte, "ii");

COMMANDF(round, "f", (float* a)
{
    intret((int)round(*a));
});
COMMANDF(ceil, "f", (float* a)
{
    intret((int)ceil(*a));
});
COMMANDF(floor, "f", (float* a)
{
    intret((int)floor(*a));
});
...
void anda(char* a, char* b)
{
    intret(execute(a) != 0 && execute(b) != 0);
}
void ora(char* a, char* b)
{
    intret(execute(a) != 0 || execute(b) != 0);
}
...
COMMANDF(strcmp, "ss",
         (char* a, char* b)
{
    intret((strcmp(a, b) == 0) ? 1 : 0);
});

COMMANDF(rnd, "i", (int* a)
{
    intret(*a > 0 ? rnd(*a) : 0);
});
...
COMMANDF(escape, "s", (const char* s)
{
...
void writecfg()
{
    ...
    f->printf("// automatically written on exit, DO NOT MODIFY\n// delete this "
              "file to have defaults.cfg overwrite these settings\n// modify "
              "settings in game, or put settings in autoexec.cfg to override "
              "anything\n\n");
    ...
    f->printf("\n");
    f->printf(
        "// crosshairs and kill messages for each weapon\n\nlooplist [\n");
    loopi(NUMGUNS) f->printf("  %-7s %-11s %-12s %s\n", gunnames[i],
                             crosshairs[i] && crosshairs[i] != notexture
                             ? behindpath(crosshairs[i]->name)
                             : "\"\"",
                             escapestring(killmessage(i, false)),
                             escapestring(killmessage(i, true)));
    f->printf("] [ w c f g ] [ loadcrosshair $w $c ; fragmessage $w $f ; "
              "gibmessage $w $g ]\n");
    ...
    loopv(sids) {
        ...
        if (curdef && omitunchangeddefaults)
            continue;
        ...
        if (!groupvariables) {
            if (id.type == ID_VAR)
                f->printf("  // min: %d, max: %d, def: %d", id.minval,
                          id.maxval, id.defaultval);
            if (id.type == ID_FVAR)
                f->printf("  // min: %s, max: %s, def: %s",
                          floatstr(id.minvalf), floatstr(id.maxvalf),
                          floatstr(id.defaultvalf));
            const char* doc = docgetdesc(id.name);
            if (doc)
                f->printf(id.type == ID_SVAR ? "  // %s" : ",  %s", doc);
        }
    }
    f->printf("\n\n// weapon settings\n\n");
    ...
    loopv(sids) {
        ...
        if (strncmp(id.name, "demodesc_", 9)) {
            ...
            for (identstack* s = id.stack; s; s = s->next)
                action = s->action;
            if (action[0])
...
void identnames(vector<const char*>& names, bool builtinonly)
{
    enumeratekt(*idents, const char*, name, ident, id, {
        if (!builtinonly || id.type != ID_ALIAS)
            names.add(name);
    });
}
...
int popscontext()
{
    ...
    if (execcontext < old && old >= IEXC_MAPCFG) // clean up aliases created in
        // the old (map cfg) context
    {
        int limitcontext
            = max(execcontext + 1,
                  (int)IEXC_MAPCFG); // don't clean up below IEXC_MAPCFG
        enumeratekt(*idents, const char*, name, ident, id, {
...
bool allowidentaccess(ident* id) // check if ident is allowed in current context
{
    ...
    if (!id)
        return false;
    if (!contextisolated[execcontext])
        return true; // only check if context is isolated
...
void substr_(char* fs, int* pa, int* len)
{
    ...
    if (ia < 0)
        ia += fslen;
    if (ia > fslen || ia < 0 || ilen < 0)
        return;

    if (!ilen)
        ilen = fslen - ia;
    if (ilen >= 0 && ilen < int(strlen(fs + ia)))
        (fs + ia)[ilen] = '\0';
    result(fs + ia);
}
...
COMMANDF(getmode, "i", (int* acr)
{
    result(modestr(gamemode, *acr != 0));
});
COMMAND(getscrext, "");
...
void listoptions(char* s)
{
    ...
    const char* optionnames[]
        = { "entities",       "ents",               "weapons",
            "teamnames",      "teamnames-abbrv",    "punctuations",
            "crosshairnames", "menufilesortorders", ""
          };
    const char** optionlists[]
        = { optionnames, entnames + 1,   entnames + 1,
            gunnames,    teamnames,      teamnames_s,
            punctnames,  crosshairnames, menufilesortorders
          };
    const char** listp
        = optionlists[getlistindex(s, optionnames, true, -1) + 1];
        ...
        case 4: { // STATE
            const char* statenames[]
                = { "disconnected",             "connecting",
                    "acknowledging connect",    "connection pending",
                    "connection succeeded",     "connected",
                    "disconnect later",         "disconnecting",
                    "acknowledging disconnect", "zombie"
                  };
            if (curpeer->state >= 0
                && curpeer->state
                < int(sizeof(statenames) / sizeof(statenames[0])))
                copystring(r, statenames[curpeer->state]);
            break; // 5 == Connected (compare ../enet/include/enet/enet.h +165)
        }
        // CAUTION: the following are only filled if the serverbrowser was used
        // or the scoreboard shown
        // SERVERNAME
        case 5: {
            serverinfo* si = getconnectedserverinfo();
            if (si)
                copystring(r, si->name);
            break;
        }
        // DESCRIPTION (3)
        case 6: {
            serverinfo* si = getconnectedserverinfo();
            if (si)
                copystring(r, si->sdesc);
            break;
        }
        case 7: {
            serverinfo* si = getconnectedserverinfo();
            if (si)
                copystring(r, si->description);
            break;
        }
        // CAUTION: the following is only the last full-description _seen_ in
        // the serverbrowser!
        case 8: {
            serverinfo* si = getconnectedserverinfo();
            if (si)
                copystring(r, si->full);
            break;
        }
