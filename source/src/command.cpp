...
COMMANDF(
    persistidents, "s", (char *on) {
        if (on && *on)
            persistidents = ATOI(on) != 0;
...
COMMANDF(
    alias, "ss", (const char *name, const char *action) { alias(name, action, false, false); });
COMMANDF(
    tempalias, "ss", (const char *name, const char *action) { alias(name, action, true, false); });
COMMANDF(
    const, "ss", (const char *name, const char *action) { alias(name, action, false, true); });

COMMANDF(
    checkalias, "s", (const char *name) { intret(getalias(name) ? 1 : 0); });
COMMANDF(
    isconst, "s", (const char *name) { ident *id = idents->access(name); intret(id && id->isconst ? 1 : 0); });
...
COMMANDF(
    isIdent, "s", (char *name) { intret(identexists(name) ? 1 : 0); });
...
{
    if (levels > 1)
        n = exchangestr(n, lookup(newstring(n), min(levels - 1, CSLIMIT_LOOKUPNESTING - 1))); // nested ("$$var"), limit to three levels
...
char *parseword(const char *&p, int arg, int *infix, int rec) // parse single argument, including expressions
{
    ...
    if (p[0] == '/' && p[1] == '/')
        p += strcspn(p, "\n\r\0");
    if (*p == '"')
        return parsequotes(p);
    if (*p == '(' && !currentcontextisolated)
        return parseexp(p, ')', rec + 1);
    if (*p == '[' && !currentcontextisolated)
        return parseexp(p, ']', rec + 1);
    ...
    { // $() $[]
        ...
        if (b)
            return lookup(b, lvls);
    }
    const char *word = p;
    p += strcspn(p, "; \t\n\r\0");
    if (p - word == 0)
        return NULL;
    if (arg == 1 && *word == '=' && p - word == 1)
        *infix = *word;
...
#define ARG(i) (id->sig[i - 1] == 'i' ? ((void *)&(ib##i = strtol(w[i], NULL, 0))) : (id->sig[i - 1] == 'f' ? ((void *)&(fb##i = atof(w[i]))) : (void *)w[i]))
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        ((void(__cdecl *)(void *, void *, void *, void *))id->fun)(ARG(1), ARG(2), ARG(3), ARG(4));
                        break;
                    case 5:
                        ((void(__cdecl *)(void *, void *, void *, void *, void *))id->fun)(ARG(1), ARG(2), ARG(3), ARG(4), ARG(5));
                        break;
                    case 6:
                        ((void(__cdecl *)(void *, void *, void *, void *, void *, void *))id->fun)(ARG(1), ARG(2), ARG(3), ARG(4), ARG(5), ARG(6));
                        break;
                    case 7:
                        ((void(__cdecl *)(void *, void *, void *, void *, void *, void *, void *))id->fun)(ARG(1), ARG(2), ARG(3), ARG(4), ARG(5), ARG(6), ARG(7));
                        break;
                    case 8:
                        ((void(__cdecl *)(void *, void *, void *, void *, void *, void *, void *, void *))id->fun)(ARG(1), ARG(2), ARG(3), ARG(4), ARG(5), ARG(6), ARG(7), ARG(8));
                        break;
                    default:
...
{                                                                    // always use one of HOOK_SP_MP, HOOK_SP or HOOK_MP and then OR them (as needed) with HOOK_TEAM, HOOK_NOTEAM, HOOK_BOTMODE, HOOK_FLAGMODE, HOOK_ARENA
    if (multiplayer(NULL) && (context & HOOK_FLAGMASK) != HOOK_MP && (context & HOOK_FLAGMASK) != HOOK_SP_MP)
...
{
    enumerate(
        *idents, ident, id, if (id.type == ID_COMMAND || id.type == ID_VAR) { *d ^= id.type == ID_VAR ? (size_t)"" - (size_t)id.name : (size_t)identhash - (size_t)id.fun; *d *= 16777619; });
}
...
bool nickcomplete(char *s, bool reversedirection)
{
    ...
    if (nickcompletesize > 0 && *cp == ':')
        enumigraphs(matchingigs, cp + 1, nickcompletesize - 1); // find all matching igraphs
...
{ // s is required to be of size "string"!
    ...
    if (openblock && (!closeblock || closeblock < openblock))
        cmd = openblock; // found opening parenthesis inside the command: assume, a new command starts here
...
bool execfile(const char *cfgfile)
{
    ...
    if (!currentcontextisolated && !strstr(s, "docs.cfg"))
        cleancubescript(buf);
    execute(buf);
    delete[] buf;
    resetcontext();
...
COMMANDF(
    execute, "s", (char *s) { intret(execute(s)); });
...
COMMANDF(
    break, "", () { if(loop_level) loop_skip = loop_break = true; });
COMMANDF(
    continue, "", () { if(loop_level) loop_skip = true; });

COMMANDF(
    concat, "c", (const char *s) { result(s); });
COMMANDF(
    concatword, "w", (const char *s) { result(s); });
...
void explodelist(const char *s, vector<char *> &elems)
{
    ...
    while (*s)
    {
        ...
        if (*elem == '"')
            filterrichtext(newelem, newelem, len);
#endif
        elems.add(newelem);
        whitespaceskip;
    }
...
COMMANDF(
    looplist, "sss", (char *list, char *varlist, char *body) { looplist(list, varlist, body, false); });
COMMANDF(
    looplisti, "sss", (char *list, char *varlist, char *body) { looplist(list, varlist, body, true); });
...
COMMANDF(
    at, "si", (char *s, int *pos) { commandret = indexlist(s, *pos); });
...
COMMANDF(
    listlen, "s", (char *l) { intret(listlen(l)); });
...
COMMANDF(
    findlist, "ss", (char *s, char *key) { intret(find(s, key)); });

#ifndef STANDALONE
COMMANDF(
    l0, "ii", (int *p, int *v) { defformatstring(f)("%%0%dd", clamp(*p, 0, 200)); defformatstring(r)(f, *v); result(r); });
COMMANDF(
    h0, "ii", (int *p, int *v) { defformatstring(f)("%%0%dx", clamp(*p, 0, 200)); defformatstring(r)(f, *v); result(r); });
COMMANDF(
    strlen, "s", (char *s) { intret(strlen(s)); });
COMMANDF(
    strstr, "ss", (char *h, char *n) { char *r = strstr(h, n); intret(r ? r - h + 1 : 0); });

void substr_(char *text, int *_start, int *_len)
{
    int start = *_start, len = *_len, textlen = (int)strlen(text);
    if (start < 0)
        start += textlen; // negative positions count from the end
    if (len < 0)
        len += textlen; // negative len subtracts from total
    if (start > textlen || start < 0 || len < 0)
        return;

    if (!len)
        len = textlen - start; // zero len gets all
...
COMMANDF(
    strcmp, "ss", (char *a, char *b) { intret((strcmp(a, b) == 0) ? 1 : 0); });

COMMANDF(
    rnd, "i", (int *a) { intret(*a > 0 ? rnd(*a) : 0); });
...
COMMANDF(
    escape, "s", (const char *s) { result(escapestring(s)); });
...
void writecfg()
{
    ...
    loopv(sids)
    {
        ...
        if (!groupvariables)
        {
            ...
            if (id.type == ID_FVAR)
                f->printf("  // min: %s, max: %s, def: %s", floatstr(id.minvalf), floatstr(id.maxvalf), floatstr(id.defaultvalf));
...
void deletecfg()
{
    ...
    loopj(2) // delete files in homedir and basedir if possible
    {
        loopi(sizeof(configs) / sizeof(configs[0]))
        {
            ...
            if (!file || findfilelocation == FFL_ZIP)
                continue;
            delfile(file);
        }
    }
...
{
    int c = getlistindex(context, contextnames, true, -1);
    if (c >= 0 && c < IEXC_NUM && !contextsealed)
        contextisolated[c] = true;
}
COMMAND(isolatecontext, "s");

COMMANDF(
    sealcontexts, "", () { contextsealed = true; });
